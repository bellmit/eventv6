...
function CloseSearchBtn(){
	...
	function loadMessage(pageNo,pageSize,searchType){
		...
		var postData = 'cata=1&page='+pageNo+'&rows='+pageSize+'&gridId='+gridId+'&corName='+corName+'&economicType='+economicType;
		$.ajax({
			type: "POST",
...
// 				gisPosition(results);
	...
	function gisPosition(rowsObj) {
		if ($('#elementsCollectionStr').val() != "") {
			window.parent.markerArcgisDataOfZhuanTi($('#elementsCollectionStr').val(), rowsObj, "01B029", {
				fieldId : "cbiId",
				fieldName : "corName",
				preMakeFunc : function(rowObj) {
					/*if (rowObj.classCode == '0101') {
						return {
							url : 'http://static.fjsq.org/images/map/gisv0/map_config/unselected/region_videoCamera.png',
							width : 10,
							height : 10
						};
					}*/
