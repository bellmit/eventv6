...
<head>
	...
	<title>南昌(nch)大联动(jointco)办理事件</title>
	...
	<table id="_eventHandleTable" width="100%" border="0" cellspacing="0" cellpadding="0" class="DealProcess">
    	...
    	<tr id="assistOrgDiv" class="hide"> 
		    <td>
			    ...
			    <div class="Check_Radio">
			    	<a href="###" class="NorToolBtn EditBtn" id="assistOrgBtn" onclick="SelectWin4OrgInfo.selectOrgInfoByObj({'nextOrgIdElId': 'assistOrgIds', 'nextOrgNameElId': 'assistOrgNames', 'nextOrgNameHtmlId': 'assistOrgNamesHtml'});">选择单位</a>
				</div>
			...
			success:function(data) {
				...
				if (data.msg && data.msg != "") {
					...
					if(data.isShowInterval) {
						...
						$("#_eventHandleTable .intervalTr").show();
					}
					...
					if(isJointHandled || isJointOperated) {
						if(isJointHandled) {
							handlerLabel = "牵头单位";
							assistOrgLabelHtml = "协办单位"
						} else if(isJointOperated) {
							handlerLabel = "交办单位";
							assistOrgLabelHtml = "配合单位";
						}
