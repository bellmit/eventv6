...
<div id="jqueryToolbar">
	<form id="msgReadingForm">
		<input type="hidden" id="listType" name="listType" class="queryParam" value="${listType!'7'}" />
	                                    ...
	                                    <table width="100%" border="0" cellspacing="0" cellpadding="0">
	                                    	...
	                                    	<tr>
	                                    		<td>
	                                    			...
	                                    			<input type="text" id="_msgSendDayDateRender" startId="msgSendDayStart" endId="msgSendDayEnd" class="inp1 InpDisable dateRenderWidth" value=""/>
	                                    		</td>
	                                    	</tr>
	                                    	<tr>
	                                    		<td>
	                                    			<label class="LabName width65px"><span>事件类型：</span></label>
	                                    			<input type="text" class="hide queryParam" id="reportType" name="reportType" />
	                                    			<input class="inp1" type="text" id="reportTypeName" style="width:248px;"></input>
	                                    		</td>
	                                    	</tr>
	                                    </table>
	                                </div>
...
<#include "/zzgl/reportFocus/base/list_query_base.ftl" />
...
<script type="text/javascript">
	...
	$(function() {
		...
		AnoleApi.initListComboBox("reportTypeName", "reportType", null, null, null, {
			DataSrc : eval('(${reportTypeDictJson!})'),
			IsTriggerDocument: false,
			RenderType : "01",
			ShowOptions: {
				EnableToolbar : true
		...
		init4DateRender('msgReadingForm');
	...
	function detail(reportUUID, reportType, reportTypeName, instanceId, msgId, msgReceiveStatus) {
		if(msgId && msgReceiveStatus && msgReceiveStatus == '0') {
			$.ajax({
				type: "POST",
				url: '${rc.getContextPath()}/zhsq/reportFocus/readMsg.jhtml',
				data: {'msgId' : msgId},
				dataType:"json",
		...
		if(reportUUID) {
			var listType = $("#listType").val(),
				reportTypeObj = {'1': 'reportTwoVioPre', '2': 'reportHHD', '3': 'reportEHD', '4': 'reportEPC', '5': 'reportWQ', '6': 'reportMeeting', '7': 'reportPSV'};
			
			var url = "${rc.getContextPath()}/zhsq/" + reportTypeObj[reportType] + "/toDetail.jhtml?reportUUID=" + reportUUID + "&listType=" + listType + "&reportType=" + reportType;
