...
function CloseSearchBtn(){
	...
	function loadMessage(pageNo,pageSize){
		...
		var postData = 'page='+pageNo+'&rows='+pageSize+'&orgCode='+orgCode+'&storeName='+storeName+'&storeAddr='+storeAddr+'&gridId='+gridId;
		$.ajax({
			type: "POST",
			url: '${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofgoods/storeListData.json?t='+Math.random(),
			data: postData,
			dataType:"json",
			success: function(data){
				...
				if(list && list.length>0) {
					for(var i=0;i<list.length;i++){
					  ...
					  tableBody+='<dl onclick="selected(\''+val.storeId+'\',\''+(val.storeName==null?'':val.storeName)+'\')">';
					  tableBody+='<dt>';
					  tableBody+='<b class="FontDarkBlue">'+(val.storeName==null?'':val.storeName)+'</b>';
					  tableBody+='</dt>';
					  tableBody+='<dd>地址：'+(val.storeAddr==null?'':val.storeAddr)+'</dd>';
					  tableBody+='</dl>';
					  
					  results=results+","+val.storeId;
					}
	...
	function gisPosition(res){
		...
		if (res==""){
	  	...
	  	var gisDataUrl="${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofgoods/getArcgisStoreLocateDataList.jhtml?resTypeCode=${markType!''}&ids="+res;
