	    	...
	    	<ul>
	        	...
	        	<li style="margin-top:5px;">
	        		...
	        		<#if isUseTwoTypesMap?? && isUseTwoTypesMap=='true'>
		        		<div id="showMaptype" style="display:none">
			        		<label class="LabName" style="width:60px;margin-left:10px"><span>地图类型：</span></label>
...
<script type="text/javascript">
			...
			success: function(data){
				...
				if(list && list.length>0) {
					...
					for(var i=0;i<list.length;i++){
				        ...
				        tableBody+='onclick="selectRow('+val.buildingId+',\''+val.buildingName+'\',this)"><div  title="['+val.buildingName+'] : '+buildingAddressFull+'"><span class="FontDarkBlue">'+buildingName+'</span> '+buildingAddress+'</div></li>';
					}
					tableBody+='</ul>';
				} else {
	...
	//左侧列表选择
	function selectRow(buildId,buildName,obj){
		...
		window.MapIframe.document.getElementById("name").value = buildName;
		window.MapIframe.document.getElementById("hs").value = "";
		window.MapIframe.clearBuildingLayer();
		//中心点定位
		window.MapIframe.locateCenterAndLevel(buildId,window.MapIframe.currentArcgisConfigInfo.mapType);
		//获取楼宇轮廓
		window.MapIframe.loadArcgisData();
		//window.MapIframe.getMapArcgisDatas();
	...
	function loadArcgisMap(buildingId,gridId){
		var url = '${rc.getContextPath()}/zhsq/map/arcgis/arcgisdata/toArcgisDrawBuildingPanelBinding.jhtml?buildingId='+buildingId+'&gridId='+gridId;
