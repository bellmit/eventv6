...
<script type="text/javascript">
	...
	function loadMessage(pageNo,pageSize){
		...
		var postData = 'page='+pageNo+'&rows='+pageSize+'&keyWord='+content+'&type='+type+'&eventType=all&infoOrgCode='+infoOrgCode;
// 		var postData = 'handleStatuss='+handleStatuss+'&infoOrgCode='+infoOrgCode+'&page='+pageNo+'&rows='+pageSize+'&eventType='+eventType+'&types='+types+'&keyWord='+keyWord+'&gridId='+gridId+"&happenTimeStart="+startHappenTime+"&happenTimeEnd="+endHappenTime;
			...
			success: function(data){
				...
				if(list && list.length>0) {
					for(var i=0;i<list.length;i++){
					  var val=list[i];
					  var eId = val.eventId+","+val.workFlowId+","+val.instanceId+","+val.taskId+",todo";
