...
<head>
	...
	<style type="text/css">
		...
		.DealMan{width:715px; line-height:24px;}
...
<body>
	<form id="flowSaveForm"	name="flowSaveForm" action="" method="post" enctype="multipart/form-data">
		...
		<input type="hidden" id="formTypeId" name="formTypeId" value="<#if formTypeId??>${formTypeId?c}</#if>" />
		...
		<table id="dealProcessTable" width="100%" border="0" cellspacing="0" cellpadding="0" class="DealProcess">
			...
			<tr id="userDiv" class="hide"> 
				<td>
					...
					<div class="FontDarkBlue fl DealMan Check_Radio" style="margin-left: 76px;"><b name="userNames" id="userNames"></b></div>
        	...
        	<div class="BtnList">
        		<#if operateList??>
					<#list operateList as l>
						<a href="###" onclick="BaseWorkflowNodeHandle.${l.operateEvent}();" class="BigNorToolBtn ${l.operateEvent}" style="padding:0 12px 0 10px;"><img src="${rc.getContextPath()}/images/${l.operateEvent}.png" style="vertical-align:middle; margin:-4px 6px 0 0; width: 17px; height: 17px;" />${l.anotherName}</a>
					</#list>
				</#if>
            </div>
        </div>	
...
<script type="text/javascript">
	...
	(function($) {
		...
		BaseWorkflowNodeHandle = {
			...
			delRecord			: initParamObj.delete.deleteOperate,
		...
		//下一办理环节选中操作
		function radioCheckOperate(obj, opt) {
			opt = opt || {};
			opt = $.extend({
				'isClearNextUser': true
			}, opt);
			
			var isClearNextUser = opt.isClearNextUser;
			
			if(opt.isClearNextUser == true) {
				$('#nextUserIds').val('');
				$('#userNames').html('');
				$('#nextOrgIds').val('');
				$('#htmlUserIds').val('');
			...
			var formId_ = $("#formId").val(),
				ajaxData = {
					...
					nodeId: nodeId_,
					instanceId: $("#instanceId").val(),
					new_regionCode:$("#regionCode").val()
				};
			
			$.extend(ajaxData, {}, opt.ajaxData);
				...
				success:function(data) {
					...
					if (data.msg && data.msg != "") {
                        ...
                        if (isDisplayUser) {//只展示人员信息，不可修改
                            ...
                            //$('#nextOrgNames').val(data.orgNames);
                            ...
                            $('#userDiv').show();
                            $("#userSelectBtn").hide();
                        } else if (isClearNextUser == true && (isSelectUser || isUserRadioStyle)) {// 事件采集、指定到人、指定到组织
                        	var isUserDefaulCheck = data.isUserDefaulCheck,
                        		htmlUserIds = data.userIds,
                        		htmlUserOrgIds = data.orgIds,
                        		htmlUserNames = data.userNames,
                        		htmlUserOrgNames = data.orgNames;
                        	
                        	if(isBlankParam(isUserDefaulCheck)) {
                        		isUserDefaulCheck = true;
                        	}
                        	
                        	if(isUserDefaulCheck == true || isUserDefaulCheck == 'true') {
                        		$('#nextUserIds').val(htmlUserIds);
                        		$('#userNames').html(htmlUserNames);
                        		$('#nextOrgIds').val(htmlUserOrgIds);
                        		$('#nextOrgNames').val(data.orgNames);
                        	} else {
                        		$('#nextUserIds').val('');
                        		$('#userNames').html('');
                            ...
                            if(htmlUserNames!=null && htmlUserNames.length>0){
                                ...
                                htmlUserOrgNameArray = htmlUserOrgNames.split(',');
                                len = htmlUserArray.length;
                                ...
                                if(len > 0) {
                                	if(isUserRadioStyle) {
                                		...
                                		for(var index = 0; index < len; index++) {
                                			...
                                			if(index == 0 && (isUserDefaulCheck == true || isUserDefaulCheck == 'true')) {
                                				radioChecked = 'checked';
                                				
                                				BaseWorkflowNodeHandle.selectRadioUser(index, htmlIdArray[index], htmlUserOrgIdArray[index], htmlUserOrgNameArray[index]);
                                			}
                                			
                                			htmlUserContent += "<input type='radio' name='userRadio' id='"+ userLabelId +"' userid='"+ htmlIdArray[index] +"' orgid='"+ htmlUserOrgIdArray[index] + "' onclick=\"BaseWorkflowNodeHandle.selectRadioUser('" + index + "','" + htmlIdArray[index] + "','" + htmlUserOrgIdArray[index] + "','" + htmlUserOrgNameArray[index] + "')\"" + radioChecked + "/>";
                                		...
                                		htmlUserContent += "<input type='checkbox'  id='htmlUserCheckAll' onclick='BaseWorkflowNodeHandle.checkAllUser()' " + radioChecked + "/>";
                                		...
                                		for(var index = 0; index < len; index++) {
                                			...
                                			htmlUserContent += "<input type='checkbox' name='htmlUserCheckbox' id='"+ userLabelId +"' userid='"+ htmlIdArray[index] +"' orgid='"+ htmlUserOrgIdArray[index] +"' onclick='BaseWorkflowNodeHandle.checkUser()' " + radioChecked + "/>";
		...
		function deleteOperate() {
			var deleteCallBack = initParamObj.delete.deleteCallback;
		...
		function deleteCallBack() {
			var flag = true,
				deleteUrl = initParamObj.delete.deleteUrl;
			
			if(isBlankParam(deleteUrl)) {
				$.messager.alert('警告','请设置删除请求，设置属性为[delete.deleteUrl]！','warning');
				return;
			}
		...
		function selectRadioUser(index,userId,userOrgId,userOrgName) {
			$('#htmlUserIds').val(userId);
			$('#nextUserIds').val(userId);
			$('#nextOrgIds').val(userOrgId);
			$('#nextOrgNames').val(userOrgName);
			try {
				_capDynamicContent();
			} catch(e) {
