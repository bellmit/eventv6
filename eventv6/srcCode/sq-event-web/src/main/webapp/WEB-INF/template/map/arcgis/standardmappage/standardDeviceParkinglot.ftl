...
<body style="border:none;scolling:yes">
    ...
    <div class="" style="display:block;">
        <div class="ListSearch">
        	<div class="condition">
            	<ul>
                	<li class="LC1" style="width:72px;">停车场名称：</li>
                	<li class="LC2" style="width:160px;">
                		<input id="deviceName"  type="text" class="inp1"/>
                	</li>
                </ul>
            	<ul>
                	<li class="LC1">&nbsp;</li>
...
function CloseSearchBtn(){
	...
	function loadMessage(pageNo,pageSize,searchType){
		...
		var postData = 'page='+pageNo+'&rows='+pageSize+'&gridId='+gridId+'&deviceName='+deviceName+'&manufacturersNo=${manufacturersNo!}';
		$.ajax({
			type: "POST",
			url:'${rc.getContextPath()}/zhsq/map/arcgis/arcgisDataOfDeviceController/listData.json?t='+Math.random(),
			data: postData,
			dataType:"json",
			success: function(data){ 
				...
				if(list && list.length>0) {
					for(var i=0;i<list.length;i++){
					  ...
					  var addr=(val.regionName==null?'':val.regionName)
					  tableBody+='<dl onClick="selected(\''+val.deviceId+'\')">';
					  tableBody+='<dt>';
					  tableBody+='<b class="FontDarkBlue" >'+(val.deviceName==null?'':val.deviceName)+'</b>';
					  tableBody+='</dt>';
					  tableBody+='<dd title=\''+(addr)+'\'>'
					  //+(val.gridName==null?'':val.gridName)
					  +(addr.length>48?addr.substring(0, 47):addr)+'</dd>';
					  tableBody+='</dl>';
					  results=results+","+val.deviceId;
					}
	...
	function gisPosition(res){
		if (typeof parent.MMApi != "undefined") {	// 判断是否是高德地图
			...
			opt.ecs = $('#elementsCollectionStr').val();
			opt.gridId = $('#gridId').val();
			opt.url = "${rc.getContextPath()}/zhsq/map/arcgis/arcgisDataOfDeviceController/getArcgisLocateDataListOfDeviceInfos.jhtml?noImgSet=true&ids="+res+"&markerType=${deviceType!}";
		...
		if($('#elementsCollectionStr').val() != "") {
			var url = "${rc.getContextPath()}/zhsq/map/arcgis/arcgisDataOfDeviceController/getArcgisLocateDataListOfDeviceInfos.jhtml?noImgSet=true&ids="+res+"&markerType=${deviceType!}";
			...
			var gisDataUrl = "${rc.getContextPath()}/zhsq/map/arcgis/arcgisDataOfDeviceController/getArcgisLocateDataListOfDeviceInfos.jhtml?noImgSet=true&ids="+res+"&markerType=${deviceType!}";
