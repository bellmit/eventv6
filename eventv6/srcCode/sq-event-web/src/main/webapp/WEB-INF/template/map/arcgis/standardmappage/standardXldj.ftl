...
function CloseSearchBtn(){
			...
			success: function(data){
				...
				if(list && list.length>0) {
					...
					for(var i=0;i<list.length;i++){
					  ...
					  /*(val.dutyLabel==null?'':val.dutyLabel)
					  +'&nbsp;'+*/(val.mobileTelephone==null?'':('<img src="${uiDomain!''}/images/map/gisv0/special_config/images/people_06.png">'+val.mobileTelephone))+'</span>';
	...
	function selected(id){
		...
		setTimeout(function() {
			window.parent.localtionGridAdminOrXldjPoint(id,'xldj');	//locationHouse
		},1000);
	  	setTimeout(function() {
			if($('#elementsCollectionStr').val() != "") {
				window.parent.getDetailOnMapOfListClick($('#elementsCollectionStr').val(),390,300,id)
			}else {
				window.parent.localtionGridAdminOrXldjPoint(id,'xldj');
			}
		},1000);
	}
	//地图定位
	...
	if("1" != window.parent.IS_ACCUMULATION_LAYER) {
		...
		if($('#elementsCollectionStr').val() != "") {
			var url = "${rc.getContextPath()}/zhsq/map/arcgis/arcgisDataOfSituationController/getArcgisLocateDataListOfGridAdmin.jhtml?ids="+res+"&showType=2";
			...
			var gridAdminurl="${rc.getContextPath()}/zhsq/map/arcgis/arcgisDataOfSituationController/getArcgisLocateDataListOfGridAdmin.jhtml?ids="+res+"&showType=2";
		  	window.parent.currentLayerLocateFunctionStr="getArcgisDataOfGridAdminOrXldj('"+gridAdminurl+"','xldj')";
			window.parent.getArcgisDataOfGridAdminOrXldj(gridAdminurl,'xldj');
		}
	}
</script>
</body>
