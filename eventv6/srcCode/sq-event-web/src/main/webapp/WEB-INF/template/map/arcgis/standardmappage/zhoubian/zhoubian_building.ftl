			...
			success : function(data) {
				...
				if (list && list.length > 0) {
					for ( var i = 0; i < list.length; i++) {
						...
						var name = val.BUILDING_NAME;
						if (name != null && name != "" && name.length > 12) {
							name = name.substring(0, 12) + "...";
						}

						var address = val.BUILDING_ADDRESS;
						if (address != null && address != "" && address.length > 12) {
							address = address.substring(0, 12) + "...";
						}

						tableBody += '<dl onclick="selected(\'' + val.BUILDING_ID + '\',\'' + (name == null ? '' : name) + '\')">';
						tableBody += '<dt>';
						tableBody += '<span class="fr"></span>';
						tableBody += '<b class="FontDarkBlue" title=\'' + (name == null ? '' : name) + '\'>' + (name == null ? '' : name) + '</b>';
						tableBody += '</dt>';
						tableBody += '<dd title=\'' + (address == null ? '' : address) + '\'>' + (address == null ? '&nbsp;' : address) + '</dd>';
						tableBody += '</dl>';

						results = results + "," + val.BUILDING_ID;
					}
