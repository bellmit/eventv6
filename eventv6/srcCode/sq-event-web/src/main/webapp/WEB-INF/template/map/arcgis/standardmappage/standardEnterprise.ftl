    ...
    <div class="ListSearch">
        <div class="condition">
            <ul>
                ...
                <li class="LC2"><input id="enterpriseName" name="enterpriseName" type="text" class="inp1" /></li>
            </ul>
            <ul>
                <li class="LC1">企业地址：</li>
                <li class="LC2"><input id="enterpriseAddress" name="enterpriseAddress" type="text" class="inp1" /></li>
            </ul>
            <ul>
                <li class="LC1">&nbsp;</li>
...
<script type="text/javascript">
    ...
    $('#enterpriseName,#gridName').keydown(function(e){
        if(e.keyCode==13){
    ...
    function loadMessage(pageNo,pageSize){
        ...
        if(enterpriseName=="==输入查询内容==") enterpriseName="";
        var pageSize = $("#pageSize").val();
        $.blockUI({message: "加载中..." , css: {width: '150px',height:'50px',lineHeight:'50px',top:'40%',left:'20%',
            background:'url(${rc.getContextPath()}/css/loading.gif) no-repeat',textIndent:'20px'},overlayCSS:{backgroundColor:'#fff'}});
        var postData = 'page='+pageNo+'&rows='+pageSize+'&orgCode='+orgCode+'&enterpriseName='+enterpriseName+'&enterpriseAddress='+enterpriseAddress+'&gridId='+gridId;
        $.ajax({
            type: "POST",
            url: '${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofgoods/enterpriseListData.json?t='+Math.random(),
            data: postData,
            dataType:"json",
            success: function(data){
                ...
                if(list && list.length>0) {
                    for(var i=0;i<list.length;i++){
                        ...
                        tableBody+='<dl onclick="selected(\''+val.enterpriseId+'\',\''+(val.enterpriseName==null?'':val.enterpriseName)+'\')">';
                        tableBody+='<dt>';
                        tableBody+='<b class="FontDarkBlue">'+(val.enterpriseName==null?'':val.enterpriseName)+'</b>';
                        tableBody+='</dt>';
                        tableBody+='<dd>地址：'+(val.enterpriseAddress==null?'':val.enterpriseAddress)+'</dd>';
                        tableBody+='</dl>';

                        results=results+","+val.enterpriseId;
                    }
    ...
    function gisPosition(res){
        ...
        var gisDataUrl="${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofgoods/getArcgisEnterpriseLocateDataList.jhtml?markType=${markType!''}&ids="+res;
