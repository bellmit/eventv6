			...
			success: function(data){
				...
				if(list && list.length>0) {
					for(var i=0;i<list.length;i++){
					  ...
					  tableBody+='<dl onclick="selected(\''+val.resId+'\',\''+(val.resName==null?'':val.resName)+'\')">';
					  tableBody+='<dt>';
					  tableBody+='<span class="fr"></span>';
					  tableBody+='<b class="FontDarkBlue">'+(val.resName==null?'':val.resName)+'</b>';
					  tableBody+='</dt>';
	...
	function selected(id, name){
		...
		setTimeout(function() {
			if($('#elementsCollectionStr').val() != "") {
				window.parent.getDetailOnMapOfListClick($('#elementsCollectionStr').val(),320,170,id)
			}else {
	...
	function gisPosition(res){
		...
		if (res==""){
	  	...
	  	if($('#elementsCollectionStr').val() != "") {
			...
			window.parent.getArcgisDataOfZhuanTi(gisDataUrl,$('#elementsCollectionStr').val(),320,170);
		}else {
			var gisDataUrl="${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofgoods/getArcgisLocateDataListOfResource.jhtml?resTypeCode=${resType.typeCode!''}&ids="+res;
