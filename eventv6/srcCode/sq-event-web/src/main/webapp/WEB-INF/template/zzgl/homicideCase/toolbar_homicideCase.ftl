...
<script type="text/javascript">
	var winHeight = $(window).height();	//页面可见高度
	var defaultPopHeight = 480;	//默认弹窗高度
	var popHeight = defaultPopHeight < winHeight ? defaultPopHeight : undefined;
	var opt = {
		"maxWidth": 1100,
		"padding_left": 0,
		"padding_right": 0,
...
//		});
	...
	function add() {
		...
		//showMaxJqueryWindow("新增命案防控信息", url, fetchWinWidth(opt), popHeight);
		easyuiWinExt({
			title:'新增命案防控信息',
			url:url,
			height:popHeight
	...
	function edit(reId, tabIndex) {
		...
		if(isBlankString(reId)) {
			...
			//showMaxJqueryWindow("编辑命案防控信息", url, fetchWinWidth(opt), popHeight);
			easyuiWinExt({
				title:'编辑命案防控信息',
				url:url,
				height:popHeight
	...
	function detail(reId) {
		...
		//showMaxJqueryWindow("查看命案防控信息", url, fetchWinWidth(opt), popHeight);
		easyuiWinExt({
			title:'查看命案防控信息',
			url:url,
			height:popHeight
