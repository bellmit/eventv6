...
<div class="MainContent">
	<#include "/zzgl/relatedEvents/relatedEventsDataGridToolbar.ftl" />
</div>
...
<script type="text/javascript">
    ...
    function loadDataList(){
    	...
    	$('#list').datagrid({
			...
			singleSelect: true,
			idField:'reId',
			url:'${rc.getContextPath()}/zhsq/relatedEvents/RelatedEventsController/listRelatedEventsData.json',
			columns:[[
				...
				{field:'bizName',title:'所在地段', align:'center',width:fixWidth(0.2),sortable:true,
					formatter:function(value,rowData,rowIndex){
						var oldval=value;
						if(value!=null && value.length>10){
							value = value.substring(0,10);
						}
						return '<span title="'+ oldval +'" >'+ value +'</span>';
					}
				},
