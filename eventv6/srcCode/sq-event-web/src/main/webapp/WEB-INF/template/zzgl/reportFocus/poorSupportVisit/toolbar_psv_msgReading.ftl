...
<div id="jqueryToolbar">
	<form id="msgReadingForm">
		...
		<input type="hidden" id="reportType" name="reportType" value="${reportType!'7'}" class="queryParam" />
	                            ...
	                            <div class="RightShadow">
	                                <div class="list NorForm" style="position:relative;">
	                                    <table width="100%" border="0" cellspacing="0" cellpadding="0">
	                                    	<tr>
	                                    		<td><label class="LabName width65px"><span>报告编号：</span></label><input class="inp1 queryParam" type="text" id="reportUUID" name="reportUUID" style="width:248px;"></input></td>
	                                    	</tr>
	                                    	<tr>
	                                    		<td>
...
<script type="text/javascript">
	...
	function detail(reportUUID, instanceId, msgId, msgReceiveStatus) {
		if(msgId && msgReceiveStatus && msgReceiveStatus == '0') {
			$.ajax({
				type: "POST",
				url: '${rc.getContextPath()}/zhsq/reportPSV/readMsg.jhtml',
				data: {'msgId' : msgId},
				dataType:"json",
		...
		if(reportUUID) {
			...
			var url = "${rc.getContextPath()}/zhsq/reportPSV/toDetail.jhtml?reportUUID=" + reportUUID + "&listType=" + listType + "&reportType=" + $('#reportType').val();
