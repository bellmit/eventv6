...
function CloseSearchBtn(){
	...
	function loadMessage(pageNo,pageSize,searchType){
		...
		if(stationName=="==输入查询内容==") stationName="";
		var pageSize = $("#pageSize").val();
		$.blockUI({message: "加载中..." , css: {width: '150px',height:'50px',lineHeight:'50px',top:'40%',left:'20%',
    	background:'url(${rc.getContextPath()}/css/loading.gif) no-repeat',textIndent:'20px'},overlayCSS:{backgroundColor:'#fff'}});
		var postData = 'page='+pageNo+'&rows='+pageSize+'&regionCode='+infoOrgCode+'&name='+stationName;
		$.ajax({
			type: "POST",
			url: '${rc.getContextPath()}/zhsq/map/arcgis/Lzly/twoStationListData.json?t='+Math.random(),
			data: postData,
			dataType:"json",
			success: function(data){
				...
				if(list && list.length>0) {
                    for(var i=0;i<list.length;i++){
                        var val=list[i];
                        tableBody+='<dl onclick="selected(\''+val.tsId+'\')">';
                        tableBody+='<dt>';
                        tableBody+='<b class="FontDarkBlue">'+(val.stationName==null?'':val.stationName)+'</b>';
                        tableBody+='</dt>';
//                        if(bizType!=0){
...
//                        }
                        tableBody+='<dd>两站类别：'+(val.stationTypeCN==null?'':val.stationTypeCN)+'</dd>';
                        tableBody+='</dl>';

                        results=results+","+val.tsId;
                    }
	...
	function gisPosition(res){
		...
		if (res==""){
	  	...
	  	if($('#elementsCollectionStr').val() != "") {
			var url = "${rc.getContextPath()}/zhsq/map/arcgis/Lzly/twoStationListGisData.jhtml?ids="+res+"&markType=TWO_STATION";
