<script type="text/javascript">
	function add(opt) {
		var url = "${rc.getContextPath()}/zhsq/reportHHD/toAdd.jhtml?listType=" + $('#listType').val();
		opt = opt || {};
	...
	function edit() {
		...
		if(reportUUID == "") {
			...
			var url = "${rc.getContextPath()}/zhsq/reportHHD/toAdd.jhtml?listType=" + $('#listType').val() + "&reportUUID=" + reportUUID;
		...
		if(reportUUID == "") {
			...
			$.messager.confirm('提示', '您确定删除选中的记录吗？', function(r) {
				if(r){
					...
					$.ajax({
						type: "POST",
						url: '${rc.getContextPath()}/zhsq/reportHHD/delReportFocus.jhtml',
						...
						error:function(data){
							$.messager.alert('错误','删除房屋安全隐患信息失败！','error');
						}
					});
				}
			});
	...
	function detail(reportUUID, instanceId, listType) {
		if(reportUUID) {
			...
			var url = "${rc.getContextPath()}/zhsq/reportHHD/toDetail.jhtml?reportUUID=" + reportUUID + "&listType=" + listType + "&reportType=" + $('#reportType').val();
