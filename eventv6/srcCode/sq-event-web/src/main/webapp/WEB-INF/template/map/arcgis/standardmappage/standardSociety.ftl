...
<title>巡逻队、警务室、治保组织等社会组织列表</title>
        ...
        <div class="ListSearch">
        	<div class="condition">
            	<ul>
                	<li class="LC1">名称：</li>
                	<li class="LC2">
                		<input id="name" value="" type="text" class="inp1" />
                	</li>
                </ul>
            	<ul>
                	<li class="LC1">&nbsp;</li>
...
function CloseSearchBtn(){
	...
	function loadMessage(pageNo,pageSize,searchType){
    	...
    	if($('#elementsCollectionStr').val() != "") {
			postData = 'page='+pageNo+'&rows='+pageSize+'&gridId='+gridId+'&resName='+name+'&type='+type+'&elementsCollectionStr='+$('#elementsCollectionStr').val();
		}else {
			postData = 'page='+pageNo+'&rows='+pageSize+'&gridId='+gridId+'&resName='+name+'&type='+type;
		}
		$.ajax({
			type: "POST",
			url:'${rc.getContextPath()}/zhsq/map/arcgis/arcgisDataOfSocietyController/societyListData.json?t='+Math.random(),
			data: postData,
			dataType:"json",
			success: function(data){ 
				...
				if(list && list.length>0) {
					for(var i=0;i<list.length;i++){
					  ...
					  tableBody+='<dl onClick="selected(\''+val.orgId+'\',\''+(val.resName==null?'':val.resName)+'\')">';
					  tableBody+='<dt>';
					  tableBody+='<span class="fr">'+'</span>';
					  tableBody+='<b class="FontDarkBlue" >'+(val.orgName==null?'':val.orgName)+'</b>';
					  tableBody+='</dt>';
					  tableBody+='<dd>'+
					   (val.mobileTelephone==null?'':
					  		('<span class="fr">'+(val.telephoe==null?'':val.telephoe))+'</span>')+ (val.gridName==null?'':val.gridName) + '</dd>';
					  tableBody+='</dl>';
	...
	function selected(id, name) {
		...
		setTimeout(function() {
			if($('#elementsCollectionStr').val() != "") {
				window.parent.getDetailOnMapOfListClick($('#elementsCollectionStr').val(),650,320,id)
			}else {
				window.parent.localtionSocietyPoint(id, type);
			}
		},1000);
	}
	//--定位
	...
	if("1" != window.parent.IS_ACCUMULATION_LAYER) {
		...
		if($('#elementsCollectionStr').val() != "") {
			var url = "${rc.getContextPath()}/zhsq/map/arcgis/arcgisDataOfSocietyController/getArcgisLocateDataListOfSocietyOrg.jhtml?ids="+res;
			...
			var gisDataUrl = "${rc.getContextPath()}/zhsq/map/arcgis/arcgisDataOfSocietyController/getArcgisLocateDataListOfSocietyOrg.jhtml?type=${type!''}&ids="+res;
			var type = '${type}';
