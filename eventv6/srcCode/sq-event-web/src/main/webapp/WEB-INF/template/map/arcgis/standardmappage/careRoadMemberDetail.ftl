        	...
        	<div class="ManagerInfo">
                <ul>
                    ...
                    <li style="width:245px;">
                        ...
                        <p><code>所在队伍：</code>${teamMember.teamName!''}</p>
                        <p><code>联系电话：</code>${teamMember.tel!''}</p>
                        <p><code>负责地段：</code>${prvetionTeam.lotName!''}</p>
                    </li>
                </ul>
                <div class="clear"></div>
            </div>
                	...
                	<ul>
                    	<li>
                    		<input type="text" id="start" 
									...
									class="Wdate" style="width:135px; height:26px; *width:100px; *height:24px;font-size:11px;" value="${startTime}"/>
                   		</li>
                        <li>至</li>
                    	<li>
                    		<input type="text" id="end" 
									onclick="WdatePicker({startDate:'${endTime}', dateFmt:'yyyy-MM-dd HH:mm', readOnly:true, alwaysUseStartDate:true})" 
									class="Wdate" style="width:135px; height:26px; *width:100px; *height:24px;font-size:11px;" value="${endTime}"/>
                    	</li>
...
function trajectoryQuery(){
	...
	var params = "?locateTimeBegin="+start+"&memberId="+memberId+"&locateTimeEnd="+end+"&imsi="+imsi+"&mobileTelephone="+mobileTelephone+"&userId="+userId+"&t="+Math.random();
...
function dynamicTrajectoryQuery(){
	...
	var params = "?locateTimeBegin="+start+"&memberId="+memberId+"&imsi="+imsi+"&mobileTelephone="+mobileTelephone+"&userId="+userId+"&t="+Math.random();
	var url = "${rc.getContextPath()}/zhsq/map/arcgis/arcgisdata/getArcgisDataTrajectoryOfCareRoadMember.jhtml"+params;
