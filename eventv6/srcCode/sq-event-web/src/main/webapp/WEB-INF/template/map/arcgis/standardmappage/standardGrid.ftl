    ...
    <div class="" style="display:block;">
        ...
        <div class="ListSearch">
        	<div class="condition">
                ...
                <ul>
                	<li class="LC1">层级显示：</li>
                	<li class="LC2">
                		<select name="gridLevel" id="gridLevel" class="sel1">
                			<option value="0">-------不  限-------</option>
                			<option value="2">市</option>
                			<option value="3">县（区）</option>
                			<option value="4">乡镇（街道）</option>
                			<option value="5">村（社区）</option>
                			<option value="6">网格</option>
                		</select>
                	</li>
                </ul>
...
function CloseSearchBtn(){
	...
	function loadMessage(pageNo,pageSize,searchType){
		...
		//var gridLevel = 0;
		var gridLevel = $("#gridLevel option:selected").val();
		var pageSize = $("#pageSize").val();
		$.blockUI({message: "加载中..." , css: {width: '150px',height:'50px',lineHeight:'50px',top:'40%',left:'20%',
    	background:'url(${rc.getContextPath()}/css/loading.gif) no-repeat',textIndent:'20px'},overlayCSS:{backgroundColor:'#fff'}});
    	var postData = 'page='+pageNo+'&rows='+pageSize+'&gridId='+gridId+'&gridName='+gridName+'&gridLevel='+gridLevel;
		$.ajax({
			type: "POST",
			url: '${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofregion/gridListData.json?t='+Math.random(), 
			data: postData,
			dataType:"json",
			success: function(data){
				...
				if(list && list.length>0) {
					...
					for(var i=0;i<list.length;i++){
					  ...
					  var dutyTelnum = val.dutyTelnum;
					  
					  if (dutyTelnum != null) {
						  tableBody+='<span class="fr" title=\''+dutyTelnum+'\'><img src="${uiDomain!''}/images/map/gisv0/special_config/images/people_06.png">' + (val.dutyTelnum==null?'':(''+val.dutyTelnum))+'</span>';
					  }
					  
					  tableBody+='<b class="FontDarkBlue" title=\''+val.gridName+'\'>'+(val.gridName==null?'':val.gridName)+'</b>';
					  tableBody+='</dt>';
					  tableBody+='<dd>'+(val.gridArea==null?'':val.gridArea+"平方米")+'</dd>';
					  tableBody+='</dl>';
	...
	function gisPosition(res){
		if (typeof parent.MMApi != "undefined") {	// 判断是否是高德地图
			...
			opt.ecs = $('#elementsCollectionStr').val();
			opt.gridId = $('#gridId').val();
			opt.url = "${rc.getContextPath()}/zhsq/map/arcgis/arcgisdata/getArcgisDataOfGridsListByIds.jhtml?ids="+res+"&showType=2";
			return parent.MMApi.markerIcons(opt, "grid");
		}
		...
		if($('#elementsCollectionStr').val() != "") {
			...
			window.parent.getArcgisDataOfZhuanTi(url,$('#elementsCollectionStr').val(),342,245);
		}else {
