...
function CloseSearchBtn(){
	...
	function loadMessage(pageNo,pageSize,searchType){
    	...
    	var postData = 'page='+pageNo+'&rows='+pageSize+'&gridId='+gridId+'&name='+name+'&type='+type;
		...
		$.ajax({
			type: "POST",
			url: '${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofregion/serviceOutletsListData.json?t='+Math.random(), 
			data: postData,
			dataType:"json",
			success: function(data){
				...
				if(list && list.length>0) {
					for(var i=0;i<list.length;i++){
					  ...
					  tableBody+='<dl onclick="selected(\''+val.soId+'\',\''+(val.name==null?'':val.name)+'\')">';
					  tableBody+='<dt>';
					  tableBody+='<span class="fr">'+(val.typeName==null?'':val.typeName)+'</span>';
					  tableBody+='<b class="FontDarkBlue" title=\''+(val.name==null?'':val.name)+'\'>'+name+'</b>';
					  tableBody+='</dt>';
					  tableBody+='<dd>'+(val.gridName==null?'':val.gridName)+'</dd>';
					  tableBody+='</dl>';
					
					  results=results+","+val.soId;
					}
	...
	var currentPageNum=1;
     ...
     function change(_index){
		...
		window.parent.currentPageNum = pagenum;
		...
		if(window.parent.qnviccub != undefined) {
	  	...
	  	setTimeout(function() {
			if($('#elementsCollectionStr').val() != "") {
				window.parent.getDetailOnMapOfListClick($('#elementsCollectionStr').val(),322,170,id)
			}else {
				window.parent.localtionServiceOutletsPoint(id,'');
			}
		},1000);
	}
	//地图定位
	...
	if("1" != window.parent.IS_ACCUMULATION_LAYER) {
		...
		if($('#elementsCollectionStr').val() != "") {
			var url = "${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofregion/getArcgisLocateDataListOfServiceOutlets.jhtml?ids="+res+"&showType=2";
			window.parent.currentLayerLocateFunctionStr="getArcgisDataOfZhuanTi('"+url+"','"+$('#elementsCollectionStr').val()+"')";
			window.parent.getArcgisDataOfZhuanTi(url,$('#elementsCollectionStr').val(),322,170);
		}else {
			var serviceOutletsurl="${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofregion/getArcgisLocateDataListOfServiceOutlets.jhtml?ids="+res;
		  	window.parent.currentLayerLocateFunctionStr="getArcgisDataOfServiceOutletsListByIds('"+serviceOutletsurl+"','')";
			window.parent.getArcgisDataOfServiceOutletsListByIds(serviceOutletsurl,'');
		}
