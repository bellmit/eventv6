<script type="text/javascript">
	function add(opt) {
		var url = "${rc.getContextPath()}/zhsq/reportPSV/toAdd.jhtml?listType=" + $('#listType').val();
		opt = opt || {};
	...
	function edit() {
		...
		if(reportUUID == "") {
			...
			var url = "${rc.getContextPath()}/zhsq/reportPSV/toAdd.jhtml?listType=" + $('#listType').val() + "&reportUUID=" + reportUUID;
		...
		if(reportUUID == "") {
			...
			$.messager.confirm('提示', '您确定删除选中的记录吗？', function(r) {
				if(r){
					...
					$.ajax({
						type: "POST",
						url: '${rc.getContextPath()}/zhsq/reportPSV/delReportFocus.jhtml',
						...
						error:function(data){
							$.messager.alert('错误','删除扶贫走访信息失败！','error');
						}
					});
				}
			});
