...
<script type="text/javascript">
	...
	function loadList() {
		$('#list').datagrid({
			...
			singleSelect: true,
			fit: true,
			url: '${rc.getContextPath()}/zhsq/repairTask/listData.jhtml?indexType=fq',
	...
	//新增
	function add() {
		var url = '${rc.getContextPath()}/zhsq/repairTask/form.jhtml';
	...
	function edit() {
		...
		if (rows.length != 1) {
			...
			if(wfInstanceId!=null&&wfInstanceId!=''){
				...
				var url = '${rc.getContextPath()}/zhsq/repairTask/form.jhtml?id=' + rows[0].drtId;
			...
			if(wfInstanceId!=null&&wfInstanceId!=''){
				...
				$.messager.confirm('提示', '您确定删除选中的信息吗?', function(r) {
					if (r) {
						...
						$.ajax({
							type: 'POST',
							url: '${rc.getContextPath()}/zhsq/repairTask/del.json',
							data: {
								drtId: rows[0].drtId
							},
							dataType: 'json',
