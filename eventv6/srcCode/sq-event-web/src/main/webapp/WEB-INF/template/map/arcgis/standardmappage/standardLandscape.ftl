    ...
    <div class="" style="display:block;">
        ...
        <div class="ListSearch">
        	<div class="condition">
            	<ul>
                	<li class="LC1">名称：</li>
                	<li class="LC2"><input id="storeName" name="name" type="text" class="inp1" /></li>
                </ul>
                <ul>
                	<li class="LC1">地址：</li>
                	<li class="LC2"><input id="storeAddr" name="roomAddress" type="text" class="inp1" /></li>
                </ul>
            	<ul>
                	<li class="LC1">网格：</li>
...
function CloseSearchBtn(){
	...
	function loadMessage(pageNo,pageSize,searchType){
		...
		if(storeName=="==输入查询内容==") storeName="";
		var pageSize = $("#pageSize").val();
		$.blockUI({message: "加载中..." , css: {width: '150px',height:'50px',lineHeight:'50px',top:'40%',left:'20%',
    	background:'url(${rc.getContextPath()}/css/loading.gif) no-repeat',textIndent:'20px'},overlayCSS:{backgroundColor:'#fff'}});
		var postData = 'page='+pageNo+'&rows='+pageSize+'&orgCode='+orgCode+'&storeName='+storeName+'&gridName='+gridName+'&storeAddr='+storeAddr;
		$.ajax({
			type: "POST",
			url: '${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofgoods/landscapeListData.json?t='+Math.random(),
			data: postData,
			dataType:"json",
			success: function(data){
				...
				if(list && list.length>0) {
					for(var i=0;i<list.length;i++){
					  ...
					  tableBody+='<dl onclick="selected(\''+val.gardenId+'\',\''+(val.name_==null?'':val.name_)+'\')">';
					  tableBody+='<dt>';
					  tableBody+='<b class="FontDarkBlue">'+(val.name_==null?'':val.name_)+'</b>';
					  tableBody+='</dt>';
					  tableBody+='<dd>地址：'+(val.placeAddr==null?'':val.placeAddr)+'</dd>';
					  tableBody+='</dl>';
					  
					  results=results+","+val.gardenId;
					}
...
// 		gisPosition(id);
	...
	function gisPosition(res){
		...
		if (res==""){
	  	...
	  	var gisDataUrl="${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofgoods/getArcgisLandscapeLocateDataList.jhtml?resTypeCode=${markType!''}&ids="+res;
