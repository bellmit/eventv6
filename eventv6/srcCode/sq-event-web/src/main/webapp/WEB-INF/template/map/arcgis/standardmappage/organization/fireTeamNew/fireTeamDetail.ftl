		      ...
		      <ul>
		          <li style="width:120px; height:145px;">
			      	...
			      	<#if fireTeam.photoPath??>
					    <img id="preview" src="${fireTeam.photoPath}"  style="width:120px; height:145px;margin-top: 5px;margin-bottom: 15px;"/>
					<#else>
		          ...
		          <li style="width:220px;">
		               <p class="FontDarkBlue" style="font-size:14px;width:200px;">
			               <b style="cursor:default" title="<#if fireTeam.name?exists>${fireTeam.name}</#if>">
			               	<a class="FontDarkBlue"  style="font-size:14px;" href="javascript:void(0);" onclick="showDetail('<#if fireTeam.name?exists>${fireTeam.name}</#if>','<#if fireTeam.fireTeamId?exists>${fireTeam.fireTeamId}</#if>')">
								<#if fireTeam.name?exists>${fireTeam.name}</#if>
							</a>
			               </b>
		               </p>
		               <p>所属网格：
...
function showDetail(title,id) {
	...
	var url = sq_zzgrid_url+"/zzgl/fireGrid/fireTeamNew/map/detail.jhtml?fireTeamId="+id;
	window.parent.showMaxJqueryWindow("详情", url, 600, 320);	
}
</script>
