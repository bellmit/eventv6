...
<script type="text/javascript">
	...
	function loadMessage(pageNo,pageSize,searchType){
		...
		var postData = 'page=' + pageNo + '&rows=' + pageSize + '&infoOrgCode=' + infoOrgCode + '&name=' + name;
		$.ajax({
			type : "POST",
			url : '${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofgoods/findArcgisDataListOfFireTeam.json?t=' + Math.random(),
			data : postData,
			dataType : "json",
			success : function(data) {
				...
				if (list && list.length > 0) {
					for ( var i = 0; i < list.length; i++) {
						...
						tableBody += '</dt>';
						tableBody += '<dd>地址：' + (val.addr == null ? '' : val.addr) + '</dd>';
						tableBody += '</dl>';
		...
		setTimeout(function() {
			if($('#elementsCollectionStr').val() != "") {
				...
				window.parent.localtionPointForCommon({
					id : id,
					title : "消防队",
					imgUrl : "${uiDomain!''}/images/map/gisv0/map_config/unselected/things_fireteam.png",
					layerName : "fireTeamLayer",
					url : "${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofgoods/fireTeamDetail.jhtml?resId=" + id,
					func1 : function (id, title) {
						...
						window.parent.showMaxJqueryWindow("详情", url, 400, 250);
					}
				});
			}
	...
	function gisPosition(res) {
		...
		if($('#elementsCollectionStr').val() != "") {
			var url = "${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofgoods/getArcgisLocateDataListOfFireTeam.jhtml?ids=" + res + "&markType=${markType!''}";
