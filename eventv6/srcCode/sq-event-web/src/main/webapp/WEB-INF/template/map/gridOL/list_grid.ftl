...
<script type="text/javascript">
			...
			success: function(data){
				...
				if(list && list.length>0) {
					...
					for(var i=0;i<list.length;i++){
						...
						tableBody+='onclick="selectRow(\''+ td_hs +'\','+val.gridId+','+ i +',this)">';
						if(val.td_hs != null && val.td_hs != ''){
	...
	//左侧列表选择
	function selectRow(hs,gridId, selectRowNum,obj){
		window.MapIframe.selectRowNum = selectRowNum;
		window.MapIframe.drawPointFlag = true;
		$("#targetType").val('grid');
		window.MapIframe.blindGridFlag = true;
		window.MapIframe.drawGridHSFlag = true;
		$('#selectGridHS').val(hs);
		...
		window.MapIframe.initButton();
		window.MapIframe.removeAllTools();
		window.MapIframe.getGridHSById();
	...
	function loadArcgisMap(gridId){
		var url = '';
		if(gridId != null && gridId != ""){
			url = '${rc.getContextPath()}/zhsq/map/gridOL/gridOLController/olwfs.jhtml?gridId='+gridId;
		}else{
			url = '${rc.getContextPath()}/zhsq/map/gridOL/gridOLController/olwfs.jhtml';
		}
