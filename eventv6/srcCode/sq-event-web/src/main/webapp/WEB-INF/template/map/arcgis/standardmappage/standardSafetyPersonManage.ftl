...
function CloseSearchBtn(){
	...
	function loadMessage(pageNo,pageSize,searchType){
    	...
    	var postData = 'page='+pageNo+'&rows='+pageSize+'&gridId='+gridId+'&name='+name+'&order='+order;
		
		$.ajax({
			type: "POST",
			url: '${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofpoplocal/safetyPersonManageListData.json?t='+Math.random(),
			data: postData,
			dataType:"json",
			success: function(data){
				...
				if(list && list.length>0) {
					...
					for(var i=0;i<list.length;i++){
					  ...
					  var corPlaceName = val.corPlaceName;
					  if(corPlaceName!=null && corPlaceName!="" && corPlaceName.length>15){
						corPlaceName = corPlaceName.substring(0,15)+"...";
					  }else{
						corPlaceName = corPlaceName;
					  }
					  
					  tableBody+='<dl onclick="selected(\''+val.corpStaffId+'\')">';
					  tableBody+='<dt>';
					  tableBody+='<span class="fr" style="width: 66px;">' + (val.residentMobile==null?'':('<img src="${uiDomain!''}/images/map/gisv0/special_config/images/people_06.png">'+val.residentMobile))+'</span>';
					  tableBody+='<b class="FontDarkBlue">'+(val.name==null?'':val.name)+'</b>'+
					  (personType==null?'':'<label>【' + personType + '】</label>');
					  tableBody+='</dt>';
					  tableBody+='<dd>'+(val.gridName==null?'':val.gridName)+'</dd>';
					  tableBody+='<dd title=\''+(val.corPlaceName==null?'':val.corPlaceName)+'\'>'+corPlaceName+'</dd>';
					  tableBody+='</dl>';
					  			
					  results=results+","+val.corpStaffId;
					}
			        tableBody+='</ul>';
	...
	function selected(id){
		...
		setTimeout(function() {
			if($('#elementsCollectionStr').val() != "") {
				window.parent.getDetailOnMapOfListClick($('#elementsCollectionStr').val(),360,240,id);
			}else {
				window.parent.localtionSafetyPersonManagePoint(id,'');
			}
		},1000);
	}
	//地图定位
	function gisPosition(res){
		...
		if($('#elementsCollectionStr').val() != "") {
			var url = "${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofpoplocal/getArcgisLocateDataListOfSafetyPersonManage.jhtml?ids="+res;
			window.parent.currentLayerLocateFunctionStr="getArcgisDataOfZhuanTi('"+url+"','"+$('#elementsCollectionStr').val()+"')";
			window.parent.getArcgisDataOfZhuanTi(url,$('#elementsCollectionStr').val(),360,240);
		}else {
			var safetyPersonManageUrl="${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofpoplocal/getArcgisLocateDataListOfSafetyPersonManage.jhtml?ids="+res;
		  	window.parent.currentLayerLocateFunctionStr="getArcgisDataOfSafetyPersonManage('"+safetyPersonManageUrl+"','')";
			window.parent.getArcgisDataOfSafetyPersonManage(safetyPersonManageUrl,'');
		}
