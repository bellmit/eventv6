...
<body>
					...
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
						...
						<tr>
							<td class="LeftTd">
	            				...
	            				<input type="text" id="receivePerson" class="comboselector" data-options="dType:'orgTeam', afterSelect: selectGridAdmin, multiple: true, panelHeight: 220, value: <#if gridAdmin.gridAdminId??>${gridAdmin.gridAdminId?c}</#if>" query-params="mapType=${mapType!''}&x=${x!''}&y=${y!''}&distance=${distance!''}&bizType=1<#if infoOrgCode??>&infoOrgCode=${infoOrgCode}</#if>" style="width: 460px; height: 28px;" />
						...
						<tr>
							<td id="sendTypeTd" class="LeftTd">
				    			...
				    			<div class="Check_Radio"><input type="checkbox" id="sendOutside" name="_sendTypeCheckbox" value="1" checked /><label for="sendOutside" style="cursor: pointer;">短信消息</label></div>
					        </td>
						</tr>
					</table>
				</div>
	...
	<script type="text/javascript">
								...
								if(userIds.length) {
									$.ajax({
										...
										success: function(data) {
											...
											if((successLen + failLen) == checkedBoxLen) {
												modleclose();
												
												if(successLen == checkedBoxLen) {
		...
		function selectGridAdmin(gridAdminList) {
			...
			if(gridAdminList && gridAdminList.length) {
				$(gridAdminList).each(function(index, gridAdmin) {
					...
					if(gridAdminId) {
						var isNotItemExist = $('#' + gridAdminId + ' > input[value="'+ phoneNum +'"]').length == 0;
		...
		function selectGridAdmin(preventionList) {
			$("#receivePersonList > span").remove();
			
			if(preventionList && preventionList.length) {
				$(preventionList).each(function(index, prevention) {
					var gridAdminSpan = "",
						gridAdminId = prevention.teamId || "",
						partyName = prevention.manager || "",
						phoneNum = prevention.managerTel || "";
					
					if(gridAdminId && (partyName || phoneNum)) {
						var isNotItemExist = $('#receivePersonList input[id="phoneNums"][value="'+ phoneNum +'"]').length == 0;
