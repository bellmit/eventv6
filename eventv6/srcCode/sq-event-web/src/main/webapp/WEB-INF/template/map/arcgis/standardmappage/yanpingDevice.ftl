...
function CloseSearchBtn(){
	...
	function loadMessage(pageNo,pageSize,searchType){
		...
		var postData = 'page='+pageNo+'&rows='+pageSize+'&gridId='+gridId+'&deviceName='+deviceName+'&deviceType=${deviceType!}';
		$.ajax({
			type: "POST",
			url:'${rc.getContextPath()}/zhsq/map/arcgis/arcgisDataOfYanpingDeviceController/listData.json?t='+Math.random(),
			data: postData,
			dataType:"json",
			success: function(data){ 
				...
				if(list && list.length>0) {
					for(var i=0;i<list.length;i++){
					  ...
					  var addr=(val.deviceInstallAddress==null?'':val.deviceInstallAddress)
	...
	function gisPosition(res){
		if (typeof parent.MMApi != "undefined") {	// 判断是否是高德地图
			...
			opt.ecs = $('#elementsCollectionStr').val();
			opt.gridId = $('#gridId').val();
			opt.url = "${rc.getContextPath()}/zhsq/map/arcgis/arcgisDataOfYanpingDeviceController/getArcgisLocateDataListOfDeviceInfos.jhtml?ids="+res+"&markerType=${deviceType!}";
		...
		if($('#elementsCollectionStr').val() != "") {
			var url = "${rc.getContextPath()}/zhsq/map/arcgis/arcgisDataOfYanpingDeviceController/getArcgisLocateDataListOfDeviceInfos.jhtml?ids="+res+"&markerType=${deviceType!}";
			...
			var gisDataUrl = "${rc.getContextPath()}/zhsq/map/arcgis/arcgisDataOfYanpingDeviceController/getArcgisLocateDataListOfDeviceInfos.jhtml?ids="+res+"&markerType=${deviceType!}";
