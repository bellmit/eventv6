...
<body style="border:none;scolling:yes">
	...
	<input type="hidden" id="resTypeId" value="${resTypeId?c}" />
...
function CloseSearchBtn(){
	...
	function loadMessage(pageNo,pageSize,searchType){
		...
		var postData = 'page='+pageNo+'&rows='+pageSize+'&gridId='+gridId+'&resName='+name+'&resTypeCode=${resType.typeCode!''}&resTypeId='+resTypeId;
		$.ajax({
			type: "POST",
			url: '${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofgoods/municipalResourceListData.json?t='+Math.random(),
			data: postData,
			dataType:"json",
			success: function(data){
				...
				if(list && list.length>0) {
					for(var i=0;i<list.length;i++){
					  ...
					  tableBody+='<dl onclick="selected(\''+val.roadResId+'\',\''+(val.roadName==null?'':val.roadName)+'\')">';
					  tableBody+='<dt>';
					  tableBody+='<span class="fr">'+ val.roadLength + '*' + val.roadWide + '</span>';
					  tableBody+='<b class="FontDarkBlue" title="'+(val.roadName==null?'':val.roadName)+'">'+(val.roadName==null?'':val.roadName)+'</b>';
					  tableBody+='</dt>';
					  tableBody+='<dt>'+(val.roadStartEnd==null?'':val.roadStartEnd)+'</dt>';
					  tableBody+='</dl>';
					  
					  results=results+","+val.roadResId;
					}
	...
	function selected(id, name){
		...
		setTimeout(function() {
			if($('#elementsCollectionStr').val() != "") {
				window.parent.getDetailOnMapOfListClick($('#elementsCollectionStr').val(),650,350,id)
			}else {
				window.parent.localtionResourcePoint('${resType.typeCode!''}',id);
			}
		},1000);
	}
	//地图定位
	function gisPosition(res){
		if (typeof parent.MMApi != "undefined") {	// 判断是否是高德地图
			...
			opt.ecs = $('#elementsCollectionStr').val();
			opt.gridId = $('#gridId').val();
			opt.url = "${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofgoods/getArcgisLocateDataListOfResource.jhtml?resTypeCode=${resType.typeCode!''}&ids="+res;
		...
		if (res==""){
	  	...
	  	if($('#elementsCollectionStr').val() != "") {
			var gisDataUrl="${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofgoods/getRoadMarkersList.jhtml?resTypeCode=${resType.typeCode!''}&ids="+res;
			window.parent.currentLayerLocateFunctionStr="getArcgisDataOfZhuanTi('"+gisDataUrl+"','"+$('#elementsCollectionStr').val()+"')";
			window.parent.getArcgisDataOfZhuanTi(gisDataUrl,$('#elementsCollectionStr').val(),650,350);
		}else {
			var gisDataUrl="${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofgoods/getArcgisLocateDataListOfResource.jhtml?resTypeCode=${resType.typeCode!''}&ids="+res;
		  	window.parent.currentLayerLocateFunctionStr="getArcgisDataOfResource('${resType.typeCode!''}','"+gisDataUrl+"')";
		  	window.parent.getArcgisDataOfResource('${resType.typeCode!''}',gisDataUrl);
		}
	}
