...
function CloseSearchBtn(){
	...
	function loadMessage(pageNo,pageSize,searchType){
		...
		var postData = 'page='+pageNo+'&rows='+pageSize+'&gridId='+gridId+'&corName='+corName+'&corType='+corType+'&category='+category+'&economicType='+economicType+'&cateVals='+cateVals;
		$.ajax({
			type: "POST",
			url:'${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofregion/anjianCorBaseListData.json?t='+Math.random(),  
			data: postData,
			dataType:"json",
	...
	function selected(id, name){
		setTimeout(function() {
			if($('#elementsCollectionStr').val() != "") {
				...
				window.parent.getAnjianCorDetailOnMapOfListClick(id);
			}
	...
	if("1" != window.parent.IS_ACCUMULATION_LAYER) {
		...
		if($('#elementsCollectionStr').val() != "") {
			...
			window.parent.currentLayerLocateFunctionStr="getArcgisDataOfAnjianCor('"+corurl+"')";
			window.parent.getArcgisDataOfAnjianCor(corurl);
		}
	}
</script>
</body>
