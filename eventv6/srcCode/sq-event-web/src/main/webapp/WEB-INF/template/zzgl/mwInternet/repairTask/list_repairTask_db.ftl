...
<body class="easyui-layout">
	...
	<div id="jqueryToolbar">
		...
		<div class="ToolBar">
			<div class="tool fr">
				<a href="javascript:void(0)" class="NorToolBtn EditBtn" onclick="edit();">办理</a>
			</div>
		</div>
	</div>
</body>
<script type="text/javascript">
	$(function() {
		...
		AnoleApi.initListComboBox("faultTypeName", "faultType", "${faultType}", null, null, {
		 ...
		 AnoleApi.initListComboBox("stateName", "state", null, null, [""], {
        	...
        	DataSrc: [{"name":"待派发", "value":"1"},{"name":"待反馈", "value":"2"},{"name":"待审核", "value":"3"}]
        });
		loadList(); //加载列表
	});
	...
	function loadList() {
		$('#list').datagrid({
			...
			singleSelect: true,
			fit: true,
			url: '${rc.getContextPath()}/zhsq/repairTask/listData.jhtml?indexType=db',
	...
	function edit() {
		...
		if (rows.length != 1) {
			...
			var url = '${rc.getContextPath()}/zhsq/repairTask/todo.jhtml?id=' + rows[0].drtId+'&taskId='+rows[0].wfDbid_+'&instanceId='+rows[0].wfInstanceId;
