    ...
    <div class="" style="display:block;">
        ...
        <div class="ListSearch">
        	<div class="MetterCondition">
            	<ul>
                	<li>
                		<input name="keyWord" id="keyWord" type="text" class="inp1" onkeydown="if (event.keyCode==13) {}" onblur="if(this.value=='')value='输入事件名称、地点进行查询';" onfocus="if(this.value=='输入事件名称、地点进行查询')value='';" value="输入事件名称、地点进行查询" style="width:240px;" />
                	</li>
                </ul>
                <ul>
                	<li>
                		<input type="text" id="eventLevel" name="eventLevel" style="display:none;"/>
            			<input type="text" class="inp1" style="width:263px;" id="eventLevelName" />
                	</li>
                </ul>
                <ul>
                	<li>
                		<input type="text" id="eventType" name="eventType" style="display:none;"/>
            			<input type="text" class="inp1" style="width:263px;" id="eventTypeName" />
                	</li>
                </ul>
...
function CloseSearchBtn(){
	...
	function loadMessage(pageNo,pageSize,searchType){
		...
		if (keyWord == "输入事件名称、地点进行查询") keyWord = "";
		...
		if (eventType != "") postData.eventType = eventType;
		if (eventLevel != "") postData.eventLevel = eventLevel;
		$.ajax({
			type: "POST",
			...
			success: function(data){
				...
				if(list && list.length>0) {
					for(var i=0;i<list.length;i++){
					  ...
					  tableBody+='</dt>';
					  tableBody+='<dd style="overflow:hidden;height:20px;">地点：'+(val.OCCU_ADDR==null?'':val.OCCU_ADDR)+'</dd>';
