...
<script type="text/javascript">
	...
	function loadList() {
		$('#list').datagrid({
			...
			singleSelect: true,
			fit: true,
			url: '${rc.getContextPath()}/zhsq/corpLink/listData.jhtml',
			//queryParams: $('#searchForm').serializeJson(),
			columns: [[
				{field:'linkageUnitId', title:'联动单位ID', align:'center', width:100},
				{field:'linkMan', title:'联络员', align:'center', width:100},
				{field:'linkManTel', title:'联络员联系方式', align:'center', width:100},
				{field:'leaderName', title:'分管领导', align:'center', width:100},
				{field:'leaderTel', title:'分管领导联系方式', align:'center', width:100}
			]],
			pagination: true,
			pageSize: 20,
	...
	//新增
	function add() {
		var url = '${rc.getContextPath()}/zhsq/corpLink/form.jhtml';
	...
	function edit() {
		...
		if (rows.length != 1) {
			...
			var url = '${rc.getContextPath()}/zhsq/corpLink/form.jhtml?id=' + rows[0].cluId;
	...
	//详情
	function detail(id) {
		var url = "${rc.getContextPath()}/zhsq/corpLink/detail.jhtml?id=" + id;
		...
		if (rows.length != 1) {
			...
			$.messager.confirm('提示', '您确定删除选中的信息吗?', function(r) {
				if (r) {
					...
					$.ajax({
						type: 'POST',
						url: '${rc.getContextPath()}/zhsq/corpLink/del.json',
						data: {
							cluId: rows[0].cluId
						},
						dataType: 'json',
