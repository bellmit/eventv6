			...
			success : function(data) {
				...
				if (list && list.length > 0) {
					for ( var i = 0; i < list.length; i++) {
						...
						tableBody += '<dl onclick="selected(\'' + val.FIRE_TEAM_ID + '\',\'' + (val.NAME == null ? '' : val.NAME) + '\')">';
						tableBody += '<dt>';
						tableBody += '<span class="fr"></span>';
						tableBody += '<span class="fr">' + (val.PH_==null?'':('<img src="${uiDomain!''}/images/map/gisv0/special_config/images/people_06.png">'+val.PH_))+'</span>';
						tableBody += '<b class="FontDarkBlue" title=\'' + (val.NAME == null ? '' : val.NAME) + '\'>' + (name == null ? '暂无' : name) + '</b>';
						tableBody += '</dt>';
						tableBody += '<dd title=\'' + (val.ADDR_ == null ? '暂无' : val.ADDR_) + '\'>地址：' + (addr == null ? '暂无' : addr) + '</dd>';
						tableBody += '<dd>队伍人数：'+(val.PERSONAL_NUM==null?'0':val.PERSONAL_NUM)+ '人&nbsp;&nbsp;车辆：' + (val.CAR_NUM==null?'0':val.CAR_NUM) + '辆</dd>';
						tableBody += '</dl>';

						results = results + "," + val.FIRE_TEAM_ID;
					}
