...
<body style="border:none;">
	...
	<input type="hidden" id="geoString" value="${geoString}" />
	<input type="hidden" id="mapt" value="${mapt}" />
...
function CloseSearchBtn(){
	...
	function loadMessage(pageNo,pageSize,searchType){
		...
		var postData = 'page='+pageNo+'&rows='+pageSize+'&geoString='+geoString+'&platformName='+platformName+'&mapt='+mapt;
		$.ajax({
			type: "POST",
			url: '${rc.getContextPath()}/zhsq/map/arcgis/arcgisDataOfSituationController/boxglobalEyesListData.json?t='+Math.random(), 
			data: postData,
			dataType:"json",
			success: function(data){
				...
				if(list && list.length>0) {
					for(var i=0;i<list.length;i++){
					  ...
					  tableBody+='<dl onclick="selected(\''+val.monitorId+'\',\''+(val.platformName==null?'':val.platformName)+'\')">';
					  tableBody+='<dt>';
