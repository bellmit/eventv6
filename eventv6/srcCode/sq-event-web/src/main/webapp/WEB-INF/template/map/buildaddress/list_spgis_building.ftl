   		...
   		<div class="search" style="height:60px;">
	    	<ul>
	        	<li>
	        		...
	        		<input id="buildingName" name="buildingName" style="width:155px;" class="inp1 InpDisable" type="text" onkeydown="if (event.keyCode==13) {}" onblur="if(this.value=='')value='查询楼宇名称';" onfocus="if(this.value=='查询楼宇名称')value='';" value="查询楼宇名称">
	        		
	        	</li>
	        	
	        	<li style="margin-top:5px;">
	        		
	        		<label class="LabName" style="width:90px;float:left;"><span>是否已经标注：</span></label>
	        		<select name="isMarker" id="isMarker" class="sel1" style="width:65px;float:left;">
	        			<option value="-1" <#if isMarker?? && isMarker == '-1'>selected</#if>>不限</option>
	        			<option value="0" <#if isMarker?? && isMarker == '0'>selected</#if>>否</option>
	        			<option value="1" <#if isMarker?? && isMarker == '1'>selected</#if>>是</option>
	        		</select>
	        		<span class="SearchBox" style="float:left;margin-left:5px">
...
<script type="text/javascript">
	...
	$(function(){
		...
		AnoleApi.initGridZtreeComboBox("gridName", "gridId", function (gridId, items){
			if(items!=undefined && items!=null && items.length>0){
				...
				$("#targetType").val('grid');
				//地图定位
				window.MapIframe.setMapcenter(null, grid.gridId);
			} 
		});
			...
			success: function(data){
				...
				if(list && list.length>0) {
					...
					for(var i=0;i<list.length;i++){
				        ...
				        tableBody+='onclick="selectRow(\''+ td_hs +'\','+ val.gridId +','+ val.buildingId +',\''+ val.orgCode +'\',\'' + val.buildingName + '\',\'' + val.useNatureLabel + '\',\'' + val.buildingAddress + '\','+ i +',this)">';
                        if(val.td_hs != null && val.td_hs != ''){
                            tableBody+='<span id="markerImg_'+ i +'"><img src="${rc.getContextPath()}/js/map/spgis/lib/img/marker-gold.png" class="FontDarkBlue" style="float:left" title="已绑定"></span>';
                        }else{
	...
	//左侧列表选择
	function selectRow(hs, gridId, buildId, orgCode, buildingName, useNatureLabel, buildingAddress, selectRowNum, obj){
        window.MapIframe.selectRowNum = selectRowNum;
		window.MapIframe.document.getElementById("ssqy").value = orgCode;//所属区域
		window.MapIframe.document.getElementById("name").value = buildingName;//名称
		window.MapIframe.document.getElementById("xingzhi").value = useNatureLabel;//性质
		window.MapIframe.document.getElementById("adress").value = buildingAddress;//地址
        //$('#selectBuildingHS').val(hs);
        window.MapIframe.document.getElementById("hs").value=hs;
        window.MapIframe.document.getElementById("savePointLocate").style.display="none";
        window.MapIframe.document.getElementById("cancleDrawOrEditBuilding").style.display="none";
		...
		//添加标注功能
        window.MapIframe.addDrawPointTool();
		$(obj).addClass("current");
		//window.MapIframe.querySPGISDate(buildId);
		window.MapIframe.setMapcenter(buildId, gridId);
	...
	function loadArcgisMap(gridId){
		var url = '${rc.getContextPath()}/zhsq/map/buildaddress/buildAddress/spgisMap.jhtml?gridId='+gridId;
