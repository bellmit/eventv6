...
<body style="border:none;scolling:yes">
    ...
    <div class="" style="display:block;">
        <div class="ListSearch">
            ...
            -->
            	<ul>
                	<li class="LC1">车牌号码：</li>
                	<li class="LC2">
                		<input id="carNo"  type="text" class="inp1"/>
                	</li>
                </ul>
                <ul>
                	<li class="LC1">设备ID：</li>
                	<li class="LC2">
                		<input id="devId"  type="text" class="inp1"/>
                	</li>
                </ul>
            	<ul>
                	<li class="LC1">&nbsp;</li>
...
function CloseSearchBtn(){
	...
	function loadMessage(pageNo,pageSize,searchType){
		...
		var carZbh = $('#carZbh').val();
		var carNo = $('#carNo').val();
		var devId = $('#devId').val();
		$.blockUI({message: "加载中..." , css: {width: '150px',height:'50px',lineHeight:'50px',top:'40%',left:'20%',
    	background:'url(${rc.getContextPath()}/css/loading.gif) no-repeat',textIndent:'20px'},overlayCSS:{backgroundColor:'#fff'}});
		var postData = 'page='+pageNo+'&rows='+pageSize+'&gridId='+gridId+'&carNo='+carNo+'&carZbh='+carZbh+'&devId='+devId;
		if(type!=null && type!=""){
			postData+=('&type='+type);
		}
		$.ajax({
			...
			success: function(data){ 
				...
				if(list && list.length>0) {
					for(var i=0;i<list.length;i++){
					  ...
					  tableBody+='<dl onClick="selected(\''+val.carId+'\')">';
					  tableBody+='<dt>';
					  tableBody+='<b class="FontDarkBlue" >'+(val.carNo==null?'':val.carNo)+'</b>';
					  tableBody+='</dt>';
					  tableBody+='<dd title="">设备ID:'+val.devId+',自编号:'+val.carZbh+',登记时间:'+val.carDjrq+'</dd>';
					  tableBody+='</dl>';
	...
	function selected(id){
		...
		if(window.parent.qnviccub != undefined) {
	  	...
	  	var url = '${rc.getContextPath()}/zhsq/map/taix/taxiDetail.jhtml?carId='+id;
