			...
			success : function(data) {
				...
				if (list && list.length > 0) {
					for ( var i = 0; i < list.length; i++) {
						...
						tableBody += '<dl onclick="selected(\'' + val.CBI_ID + '\',\'' + (val.COR_NAME == null ? '' : val.COR_NAME) + '\')">';
						tableBody += '<dt>';
						tableBody += '<span class="fr"></span>';
						tableBody += '<b class="FontDarkBlue" title=\'' + (val.COR_NAME == null ? '' : val.COR_NAME) + '\'>' + (name == null ? '' : name) + '</b>';
						tableBody += '</dt>';
						tableBody += '<dd title=\'' + (val.BUILDING_ADDRESS == null ? '' : val.BUILDING_ADDRESS) + '\'>' + (address == null ? '&nbsp;' : address) + '</dd>';
						tableBody += '</dl>';

						results = results + "," + val.CBI_ID;
					}
