...
<input type="hidden" id="resTypeId" value="${resTypeId}" />
...
//var temp1= $(".ListSearch").is(":visible");//是否可见
    ...
    function loadMessage(pageNo,pageSize,searchType){
       ...
       <#--  var postData = 'page='+pageNo+'&rows='+pageSize+'&orgCode='+orgCode+'&platformName='+platformName; -->
        var postData = 'page='+pageNo+'&rows='+pageSize+'&resTypeId='+resTypeId+'&startGridId='+gridId;
        $.ajax({
            type: "POST",
            url: '${rc.getContextPath()}/zhsq/map/arcgis/arcgisDataOfSituationController/pollutionListData.json?t='+Math.random(),
             data: postData,
            <#--data:{
                page:pageNo,
                rows:pageSize,
                ...
                if(list && list.length>0) {
                    for(var i=0;i<list.length;i++){
                       ...
                       <#-- if(val.useType!=null) {
                        ...
                        tableBody+='<dl onclick="selected(\''+val.resId+'\',\''+(val.gridPath==null?'':val.gridPath)+'\',\''+(val.companyType==null?'':val.companyType)+'\',\''+(val.resName==null?'':val.resName)+'\')">';
                        tableBody+='<dt>';
                        tableBody+='<span class="fr">'+'</span>';
                        tableBody+='<b class="FontDarkBlue" style="color: #fff">'+(val.resName==null?'':val.resName)+'</b>';
                        tableBody+='</dt>';
                        tableBody+='<dd style="color: #fff">'+(val.gridPath==null?'':val.gridPath)+'</dd>';
                        tableBody+='</dl>';
                       <#--  if(val.companyType != 20){
                            results=results+","+val.resId;
                        }else{//ffcs全球眼类型的是获取设备号
                            results2=results2+",'"+val.resName+"'";
                        }-->
                        results=results+","+val.resId;
                        results2=results2+",'"+val.resName+"'";
                    }
...
//            }
    ...
    function gisPosition(res, res20){
        if (typeof parent.MMApi != "undefined") {	// 判断是否是高德地图
            ...
            opt.h = 406;
            opt.ecs = $('#elementsCollectionStr').val();
            opt.url = "${rc.getContextPath()}/zhsq/map/arcgis/arcgisDataOfSituationController/getPollutionDataListOfGlobalEyes.jhtml?ids="+res+"&showType=5&typeCode="+typeCode;
        ...
        if($('#elementsCollectionStr').val() != "") {
            var url = "${rc.getContextPath()}/zhsq/map/arcgis/arcgisDataOfSituationController/getPollutionDataListOfGlobalEyes.jhtml?ids="+res+"&showType=5&typeCode="+typeCode+"&ids20="+res20;
            ...
            var qqyurl= "${rc.getContextPath()}/zhsq/map/arcgis/arcgisDataOfSituationController/getPollutionDataListOfGlobalEyes.jhtml?ids="+res+"&showType=5&typeCode="+typeCode;
