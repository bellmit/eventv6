...
<script type="text/javascript">
	...
	function loadList() {
		$('#list').datagrid({
			...
			singleSelect: true,
			fit: true,
			url: '${rc.getContextPath()}/zhsq/warnTask/listData.jhtml?indexType=db',
	...
	function edit() {
		...
		if (rows.length != 1) {
			...
			var url = '${rc.getContextPath()}/zhsq/warnTask/todo.jhtml?id=' + rows[0].dwtId+'&taskId='+rows[0].wfDbid_+'&instanceId='+rows[0].wfInstanceId;
