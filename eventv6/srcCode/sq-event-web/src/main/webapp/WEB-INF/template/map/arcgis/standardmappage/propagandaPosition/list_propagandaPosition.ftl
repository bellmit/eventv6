    ...
    <div class="" style="display:block;">
        ...
        <div class="ListSearch">
        	<div class="condition">
                ...
                <ul>
                	<li class="LC1">电话号码：</li>
                	<li class="LC2"><input id="phone" name="phone" type="text" class="inp1" /></li>
                </ul>
                 <ul>
                	<li class="LC1">负责人：</li>
                	<li class="LC2"><input id="personInCharge" name="personInCharge" type="text" class="inp1" /></li>
                </ul>
...
function CloseSearchBtn(){
	...
	function loadMessage(pageNo,pageSize,searchType){
		...
		var postData = 'page='+pageNo+'&rows='+pageSize+'&regionCode='+infoOrgCode+'&name='+name+'&personInCharge='+personInCharge+'&phone='+phone;
		$.ajax({
			type: "POST",
			url: '${rc.getContextPath()}/zhsq/map/arcgis/Lzly/pp/propagandaPositionListData.json?t='+Math.random(),
			data: postData,
			dataType:"json",
			success: function(data){
				...
				if(list && list.length>0) {
                    for(var i=0;i<list.length;i++){
                        var val=list[i];
                        tableBody+='<dl onclick="selected(\''+val.ppId+'\')">';
                        tableBody+='<dt>';
                        tableBody+='<b class="FontDarkBlue">'+(val.name==null?'':val.name)+'</b>';
                        tableBody+='</dt>';

                           tableBody+='<dd>负责人：'+(val.personInCharge==null?'':val.personInCharge)+'</dd>';
                           tableBody+='<dd>联系电话：'+(val.phone==null?'':val.phone)+'</dd>';                       
                           tableBody+='<dd>阵地类型：'+(val.typeCN==null?'':val.typeCN)+'</dd>';                                            
                        tableBody+='</dl>';

                        results=results+","+val.ppId;
                    }
	...
	function gisPosition(res){
		...
		if (res==""){
	  	...
	  	if($('#elementsCollectionStr').val() != "") {
			var url = "${rc.getContextPath()}/zhsq/map/arcgis/Lzly/pp/ppListGisData.jhtml?ids="+res+"&markType=PROPAGANDA_POSITION";
