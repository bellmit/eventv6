...
<!-- 			<li><lable class="nawid">设备编号：</lable><span class="blue" id="deviceNum" data="deviceNum"></span></li> -->
			<li><lable class="nawid">设备地址：</lable><span style="display:inline-block;width:160px;" class="blue" data="remark">111</span></li>
			<li><lable class="nawid">进入视野时间：</lable><span class="blue" id="timestampStartStr" data="timestampStartStr">2013-12-10 17：46：22</span></li>
	  ...
	  <div class="faceinfo">
		  ...
		  <ul>
			...
			<li><lable class="nawid">证件号：</lable><span class="blue" id="person_id" data="person_id">350422197801235678</span></li>
			  <#--<li>-->
				  <#--<a href="###" class="NorToolBtn" onclick="eventDispatch();">事件派遣</a>-->
				  <#--<a href="###" class="NorToolBtn" onclick="emergencyPlan()">应急预案</a>-->
			  <#--</li>-->
<!-- 			<li id="tags"> -->
<!-- 				<lable class="nawid"></lable><span class="blue"></span> -->
<!-- 			</li> -->
<!-- 			<li id="tags2"> -->
<!-- 			</li> -->
		  </ul>
...
var path = "http://oaknt.tpddns.cn:19249/";
$(function() {
	$("#content-d").mCustomScrollbar({theme:"minimal-dark"});
	var data = '${data!}';
	var dataJson = JSON.parse(data);
	console.log(dataJson.request_id);
	console.log(dataJson.data[0].camera_id);
	...
	for(var i=0;i<datas.length;i++){
		...
		var content = '<li'+st+' onclick="selectBlack(this,'+i+')"><img src="'+path + datas[i].candidatepic_image_url+'" width="47" height="44" alt=""/><span class="bno">'+datas[i].similarity+'</span></li>';
		$('#blacklist').append(content);
	}
	$("#deviceNum").html(dataJson.data[0].camera_id);
	$("#timestampStartStr").html(dataJson.data[0].check_time);
	$("#similarity").html(dataJson.data[0].similarity);
	$("#name").html(dataJson.data[0].name);
	$("#person_id").html(dataJson.data[0].card_number);
	$("#photoImg").attr("src",path+dataJson.data[0].candidatepic_image_url);
	$("#blackImg").attr("src",path+dataJson.data[0].facerecognition_image_url);
// 	getData();
});
