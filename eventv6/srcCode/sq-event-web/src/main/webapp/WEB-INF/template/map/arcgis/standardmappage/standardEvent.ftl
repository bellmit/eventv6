...
function CloseSearchBtn(){
				...
				if(list && list.length>0) {
					for(var i=0;i<list.length;i++){
						...
						if(val.remindStatus=='2' || val.remindStatus=='3'){
						    console.log("supervisionType: " + val.supervisionType);
                            tableBody+='<div class="duban2"></div>';
							if(val.supervisionType == '1'){
                                tableBody+='<div class="duban1"></div>';
                            }else if(val.supervisionType == '3'){
                                tableBody+='<div class="duban3"></div>';
                            }else {
	...
	function showDetailRow(obj,eventId,instanceId,workFlowId,type, urgencyDegree,urgencyResults,results){
		...
		/*if(!eventId){
			...
			$(obj).addClass("cik");
			var url = "${rc.getContextPath()}/zhsq/event/eventDisposalController/detailEvent/"+ type +".jhtml?eventType=${eventType}&instanceId="+instanceId+"&workFlowId="+workFlowId+"&eventId="+eventId+"&cachenum=" + Math.random();
		    //window.parent.showMaxJqueryWindow("查看事件信息", url, 820);	
		    window.parent.showCustomEasyWindow("查看事件信息", url, 1024);
		}
		if(eventType != "todo"){
			window.parent.localtionEventPoint('${eventType}',eventId,instanceId,workFlowId,type,urgencyDegree);
		}else{
			window.parent.localtionEventPointNoInfo('${eventType}',eventId,instanceId,workFlowId,type,urgencyDegree);
		}*/
	}
...
//					}
				...
				if(statusImg != ''){
					url += "&statusImg=" + statusImg;
				}
				window.parent.getArcgisDataOfUrgencyEvent(url,res,35,35);
			}
		}
...
//					}
			...
			<#if multiple??>
				url +="&multiple=true";
			window.parent.getArcgisDataOfEvent(url,res,30,39);
			<#elseif statusImg??>
				url += "&statusImg=" + statusImg;
				window.parent.getArcgisDataOfEvent(url,res,35,35);
			<#else>
