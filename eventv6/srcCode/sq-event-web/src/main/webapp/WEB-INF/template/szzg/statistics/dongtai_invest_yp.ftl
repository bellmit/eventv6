...
var orgCode = '${orgCode}';
 ...
 function findYears(){
	var option = {tooltip:{trigger:'axis'},title:{ text: '固定资产投资额累计(万元)',x:'center',textStyle:{color:'#fff',fontSize:21}},
				legend:{data:['投资额累计'],show:false},toolbox:{show:true},grid:{x:50,y:30,x2:10,y2:50},calculable : false,
				...
				series : [
					{name:'投资额累计',type:'bar',stack:'投资额',data:[],itemStyle:{normal:{label:{show:true,position:'insideRight'},color:(function(a,b,c){return colorObj[3];})()}},barWidth:30}]
				};
				
	var option1 = {tooltip:{trigger:'axis'},title:{ text: '固定资产投资额增长(%)',x:'center',textStyle:{color:'#fff',fontSize:21}},
	...
	$.ajax({
		...
		success: function(data){ 
			...
			for(var i=data.list.length-1;i>=0;i--){
				...
				option.series[0].data.push({value:isNum(d.GDP_SUM),year:d.YEAR_,column:'GDP'});
 ...
 function findItem(param){
	var option = {title:{text: param.year+'年每月投资额增量',x:'center',textStyle:{color:'#fff',fontSize:21}},tooltip:{trigger:'axis',axisPointer:{type:'shadow'},
				...
				legend:{data:['投资额'],x:'center',y:'bottom',textStyle:{color:'#fff'}},toolbox:{},xAxis:[{type:'category',splitLine:{show:false},data:[],axisLabel:{textStyle:{color:"#fff"}}}],
				...
				{name:'投资额',type:'bar',stack:'总量',itemStyle:{normal:{label:{show:true,position:'top'},color:(function(a,b,c){return colorObj[1];})()}},data:[]},
				{name:'支出',type:'bar',stack:'总量',itemStyle:{normal:{label:{show:true,position:'bottom'}}},data:[]}]};	
				
	var option1 = {tooltip:{trigger:'axis'},title:{ text: param.year+'年每月增长(%)',x:'center',textStyle:{color:'#fff',fontSize:21}},
				legend:{data:['增长'],x:'center',y:'bottom',textStyle:{color:'#fff'}},
				toolbox:{show:true},grid:{x:40,y:30,x2:5,y2:50},calculable : false,
				xAxis : [{type:'category',data:[],axisLabel:{textStyle:{color:"#fff"}}}],
				yAxis : [{type : 'value',axisLine:{lineStyle:{color: '#fff'}},axisLabel:{textStyle:{color:"#fff"}}}],
				series : [
					{name:'增长',type:'bar',stack: '值',data:[],barWidth:30,itemStyle:{normal:{label:{show:true,position:'insideRight'},color:(function(a,b,c){return colorObj[2];})()}}}]
				};	
	$.ajax({
		url: '${rc.getContextPath()}/zhsq/szzg/zgStatisticsController/findGDPDate.json?t='+Math.random(),
		type: 'POST',
		...
		success: function(data){ 
			...
			for(var i=0,l=data.list.length;i<l;i++){
				var d = data.list[i],sum = isNum(d.GDP_SUM),cha = sum-lastSum;
