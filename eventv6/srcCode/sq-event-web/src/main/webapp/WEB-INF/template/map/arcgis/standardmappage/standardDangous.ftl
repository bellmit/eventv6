...
<body style="border:none;scolling:yes">
    ...
    <div class="" style="display:block;">
        ...
        <div class="ListSearch">
        	<div class="condition">
            	<ul>
                	<li class="LC1">隐患描述：</li>
                	<li class="LC2">
                		<input id="checkDetail"  type="text" class="inp1" />
                	</li>
                </ul>
            	<ul>
                	<li class="LC1">&nbsp;</li>
...
function CloseSearchBtn(){
	...
	function loadMessage(pageNo,pageSize,searchType){
		...
		if(checkDetail!=null && checkDetail!=""){
			postData+=('&checkDetail='+checkDetail);
		}
		$.ajax({
			type: "POST",
			...
			success: function(data){ 
				...
				if(list && list.length>0) {
					for(var i=0;i<list.length;i++){
					  ...
					  tableBody+='<dl onclick="selected(\''+val.safeCheckDetailId+'\')">';
					  tableBody+='<dt>';
					  tableBody+='<span class="fr">'+'</span>';
					  tableBody+='<b class="FontDarkBlue" >'+(val.checkDetail==null?'':val.checkDetail)+'</b>';
					  tableBody+='</dt>';
					  tableBody+='<dd>'+
					  	(val.checkTimeStr==null?'':val.checkTimeStr)+'</dd>';
					  tableBody+='</dl>';
	...
	function selected(id){
		setTimeout(function() {
			if($('#elementsCollectionStr').val() != "") {
				window.parent.getDetailOnMapOfListClick($('#elementsCollectionStr').val(),645,320,id)
			}else {
	...
	if("1" != window.parent.IS_ACCUMULATION_LAYER) {
		...
		if($('#elementsCollectionStr').val() != "") {
			var url = "${rc.getContextPath()}/zhsq/map/arcgis/arcgisDataOfSocietyController/getArcgisLocateDataListOfDangous.jhtml?ids="+res;
			window.parent.currentLayerLocateFunctionStr="getArcgisDataOfZhuanTi('"+url+"','"+$('#elementsCollectionStr').val()+"')";
			window.parent.getArcgisDataOfZhuanTi(url,$('#elementsCollectionStr').val(),645,320);
		}else {
