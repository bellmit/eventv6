...
@Service(value="kuangXuanStatOfXss")
public class KuangXuanStatOfXssServiceImpl implements IKuangXuanStatService {
	...
	public int statOfKuangXuan(Map<String, Object> params) {
		...
		int zjsbsCount = gisInfoService.getZKSListboxChose(geoString, "K", mapType, null,infoOrgCode);//重精神病数
		return zjsbsCount;
	}
	...
	public EUDGPagination statOfKuangXuanList(int pageNo, int pageSize, Map<String, Object> params) {
		...
		params.put("peopleTypeTableName", "K");
		params.put("empWay", null);
		if (pageNo == 1) {
			pagination.setTotal(gisInfoService.getZKSListboxChose(geoString, "K", mapType, null,infoOrgCode));
		}
