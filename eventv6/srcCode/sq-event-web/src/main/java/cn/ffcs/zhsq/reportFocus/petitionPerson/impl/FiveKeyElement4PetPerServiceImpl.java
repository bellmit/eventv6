 ...
 */
@Service(value = "fiveKeyElement4PetPerService")
public class FiveKeyElement4PetPerServiceImpl extends FiveKeyElement4TwoVioPreServiceImpl {
	...
	public Map<String, Object> getNodeInfoForEvent(
		...
		String reportType = ReportTypeEnum.petitionPerson.getReportTypeIndex(), formTypeIdStr = null;
		...
		if((FocusReportNode362Enum.DISTRIBUTE_NODE_NAME.getTaskNodeName().equals(curnodeName)
				...
				&&FocusReportNode362Enum.FORM_TYPE_ID.getTaskNodeName().equals(formTypeIdStr)) {//处理中
			//不自动取人
		...
		}else if(FocusReportNode362Enum.DISTRIBUTE_NODE_NAME3.getTaskNodeName().equals(curnodeName)
				&&FocusReportNode362Enum.FIND_NODE_NAME.getTaskNodeName().equals(nodeName)
				&&FocusReportNode362Enum.FORM_TYPE_ID.getTaskNodeName().equals(formTypeIdStr)){
			//不自动取人
			params.put("isAnalysisOperateUser", false);
			resultMap = getNodeInfoForEventV2(userInfo, curnodeName, nodeName, nodeCode, nodeId, params);
			...
			resultMap.put("isUserRadioStyle", true);
			resultMap.put("isSelectUser", false);
		}else if(FocusReportNode36201Enum.DISTRIBUTE_NODE_NAME.getTaskNodeName().equals(curnodeName)
				&&FocusReportNode36201Enum.FEEDBACK_NODE_NAME.getTaskNodeName().equals(nodeName)
				&&FocusReportNode36201Enum.FORM_TYPE_ID.getTaskNodeName().equals(formTypeIdStr)){
			//不自动取人
			params.put("isAnalysisOperateUser", false);
			nodeCode = INodeCodeHandler.ORG_UINT + INodeCodeHandler.STREET + nodeCode.substring(2);
			resultMap = getNodeInfoForEventV2(userInfo, curnodeName, nodeName, nodeCode, nodeId, params);
			...
			resultMap.put("isSelectUser", false);
		}else if(FocusReportNode36201Enum.FEEDBACK_NODE_NAME.getTaskNodeName().equals(curnodeName)
				&&FocusReportNode36201Enum.FORM_TYPE_ID.getTaskNodeName().equals(formTypeIdStr)){
			//不自动取人
			params.put("isAnalysisOperateUser", false);
			resultMap = getNodeInfoForEventV2(userInfo, curnodeName, nodeName, nodeCode, nodeId, params);
			...
			tempParams.put("taskName", "task1");
			List<Map<String, Object>> tempList = reportPetPerMapper.findToDoOrgId(tempParams);
			if(!tempList.isEmpty()) {
				//乡镇分管领导(信访)：xzfgldxinfang
				PositionInfoBO positionInfoBO = positionInfoOutService.queryPositionByCode("xzfgldxinfang");
				if(positionInfoBO!=null) {
	...
	protected StringBuffer capAdviceNoteCondition(ProInstance proInstance, Map<String, Object> params) {
		...
		trigCondition.append(ConstantValue.PETITION_PERSON_NOTE).append("-").append(proInstance.getProName()).append("-").append(curTaskName);
	...
	protected String capAdviceNote(String adviceNote, UserInfo userInfo, Map<String, Object> params) {
		...
		if(adviceNote.contains("@dataSourceName@")) {
			...
			if(CommonFunctions.isNotBlank(params, "dataSource")){
				dataSourceName = PetPerDataSourceEnum.getValue(params.get("dataSource").toString());
			}
			adviceNote = adviceNote.replace("@dataSourceName@", dataSourceName);
		}
		
		
		if(adviceNote.contains("@partyName_@")) {
			String partyName_ = "";
			if(CommonFunctions.isNotBlank(params, "partyName_")){
				partyName_ = params.get("partyName_").toString();
			}
			adviceNote = adviceNote.replace("@partyName_@", partyName_);
		}
		
		if(adviceNote.contains("@partyName_2@")) {
			adviceNote = adviceNote.replace("@partyName_2@", "@partyName_@");
		}
		
		if(adviceNote.contains("@petitionTypesName@")) {
			String petitionTypesName = "";
			if(CommonFunctions.isNotBlank(params, "petitionTypesName")){
				petitionTypesName = params.get("petitionTypesName").toString();
			}
			adviceNote = adviceNote.replace("@petitionTypesName@", petitionTypesName);
		}
		
		if(adviceNote.contains("@petitionTypesName2@")) {
			adviceNote = adviceNote.replace("@petitionTypesName2@", "@petitionTypesName@");
		}
		
		if(adviceNote.contains("@petitionTrend@")) {
			String petitionTrend = "";
			if(CommonFunctions.isNotBlank(params, "petitionTrend")){
				petitionTrend = params.get("petitionTrend").toString();
			}
			adviceNote = adviceNote.replace("@petitionTrend@", petitionTrend);
		}
		
		if(adviceNote.contains("@petitionTrend2@")) {
			adviceNote = adviceNote.replace("@petitionTrend2@", "@petitionTrend@");
		}
		
		if(adviceNote.contains("@controlTypeName@")) {
			String controlTypeName = "";
			if(CommonFunctions.isNotBlank(params, "controlTypeName")){
				controlTypeName = params.get("controlTypeName").toString();
			}
			adviceNote = adviceNote.replace("@controlTypeName@", controlTypeName);
		}
		
		if(adviceNote.contains("@controlTypeName2@")) {
			adviceNote = adviceNote.replace("@controlTypeName2@", "@controlTypeName@");
		}
		
		if(adviceNote.contains("@petitionPartyTypeName@")) {
			String petitionPartyTypeName = "";
			if(CommonFunctions.isNotBlank(params, "petitionPartyTypeName")){
				petitionPartyTypeName = params.get("petitionPartyTypeName").toString();
			}
			adviceNote = adviceNote.replace("@petitionPartyTypeName@", petitionPartyTypeName);
		}
		
		if(adviceNote.contains("@petitionPartyTypeName2@")) {
			adviceNote = adviceNote.replace("@petitionPartyTypeName2@", "@petitionPartyTypeName@");
		}
		
		if(adviceNote.contains("@nowDayTimeZH@")) {
			adviceNote = adviceNote.replace("@nowDayTimeZH@", DateUtils.getToday("yyyy年MM月dd日HH时"));
		}
		
		if(adviceNote.contains("@amTime2@")) {
			String amTime2 = "";
			if(CommonFunctions.isNotBlank(params, "amTime")){
				amTime2 = params.get("amTime").toString();
			}
			adviceNote = adviceNote.replace("@amTime2@", amTime2);
		}
		
		if(adviceNote.contains("@pmTime2@")) {
			String pmTime = "";
			if(CommonFunctions.isNotBlank(params, "pmTime")){
				pmTime = params.get("pmTime").toString();
			}
			adviceNote = adviceNote.replace("@pmTime2@", pmTime);
		}
	...
	public Map<String, Object> getNodeInfoForEventV2(
		...
		String reportType = ReportTypeEnum.petitionPerson.getReportTypeIndex();
		...
		if(CommonFunctions.isNotBlank(reportFocusMap, "regionCode")) {
							...
							if(ConstantValue.COMMUNITY_ORG_LEVEL.equals(orgChiefLevel)){
								...
								if(StringUtils.isNotBlank(userOrgType)){
									if(String.valueOf(ConstantValue.ORG_TYPE_UNIT).equals(userOrgType)){
										...
										if(CommonFunctions.isNotBlank(reportFocusMap, "dataSource")) {
											String dataSource = reportFocusMap.get("dataSource").toString();
											if(PetPerDataSourceEnum.PUBLIC_SECURITY.getDataSource().equals(dataSource)
													&&FocusReportNode362Enum.FIND_NODE_NAME.getTaskNodeName().equals(curnodeName)
													&&FocusReportNode362Enum.DISTRIBUTE_NODE_NAME2.getTaskNodeName().equals(nodeName)) {
												nodeCodeTmp = INodeCodeHandler.ORG_UINT + INodeCodeHandler.STREET + INodeCodeHandler.OPERATE_SEND + (INodeCodeHandler.STREET - nodeCodeHandler.getToLevel()) + nodeCode.substring(4);
												//nodeCodeTmp="U4R0U4";
											}else if(PetPerDataSourceEnum.OFFICE_SPECIALIST.getDataSource().equals(dataSource)
													&&FocusReportNode362Enum.DISTRIBUTE_NODE_NAME3.getTaskNodeName().equals(curnodeName)
													&&FocusReportNode362Enum.TODO_NODE_NAME.getTaskNodeName().equals(nodeName)) {
												nodeCodeTmp = INodeCodeHandler.ORG_UINT + INodeCodeHandler.STREET + INodeCodeHandler.OPERATE_REPORT + (INodeCodeHandler.STREET - nodeCodeHandler.getToLevel()) + nodeCode.substring(4);
												//nodeCodeTmp="U4R0U4";
											}
										}
									}
			...
			String adviceNoteInitial = null, adviceNote = null,task1AdviceNote = null;
			...
			task1AdviceNote = capAdviceNote(adviceNote, userInfo, adviceNoteMap);
			...
			if(StringUtils.isNotBlank(adviceNote)) {
				...
				if(adviceNote.contains(splitStr)) {
					...
					//导入特殊处理
					resultMap.put("task1AdviceNote", task1AdviceNote);
				}
			}
	...
	private List<UserBO> findExtraCCUserV2(String curTaskName, String nextTaskName, ProInstance proInstance, Map<String, Object> params) {
		...
		if(FocusReportNode362Enum.FORM_TYPE_ID.toString().equals(formTypeIdStr)) {
			if(PetPerDataSourceEnum.GRID_INSPECT.getDataSource().equals(dataSource)) {
				if(FocusReportNode362Enum.DISTRIBUTE_NODE_NAME.getTaskNodeName().equals(nextTaskName)) {
					//乡镇（街道、开发区）党委（党工委）主要领导
				}else if(FocusReportNode362Enum.END_NODE_NAME.getTaskNodeName().equals(nextTaskName)
						||FocusReportNode362Enum.DOING_NODE_NAME.getTaskNodeName().equals(nextTaskName)) {
					//乡镇（街道、开发区）社会治理办公室负责人
					String professionName = "乡镇社会治理办公室";
					ProfessionBO professionBO = professionService.findProfessionByName(professionName);
					if(professionBO!=null) {
						List<OrgSocialInfoBO> orgList = orgSocialInfoService.findOrgSocialListByParentIdAndProId(curUserInfo.getOrgId(),professionBO.getProfessionId());
						if(orgList!=null&&!orgList.isEmpty()) {
							//乡镇业务科室负责人
							PositionInfoBO positionInfoBO = positionInfoOutService.queryPositionByCode("xzywksfzr");
							if(positionInfoBO!=null) {
								allList.addAll(userManageService.getUserListByUserExampleParamsOut(null, positionInfoBO.getPositionId(), orgList.get(0).getOrgId()));
							}
						}
					}
				}
			}else if(PetPerDataSourceEnum.PUBLIC_SECURITY.getDataSource().equals(dataSource)) {
				if(FocusReportNode362Enum.END_NODE_NAME.getTaskNodeName().equals(nextTaskName)
						||FocusReportNode362Enum.DOING_NODE_NAME.getTaskNodeName().equals(nextTaskName)) {
					//乡镇（街道、开发区）社会治理办公室负责人
					String professionName = "乡镇社会治理办公室";
					ProfessionBO professionBO = professionService.findProfessionByName(professionName);
				...
				if(FocusReportNode362Enum.DISTRIBUTE_NODE_NAME3.getTaskNodeName().equals(curTaskName)) {
					//乡镇分管领导、主要领导
					OrgSocialInfoBO orgSocialInfoBO = orgSocialInfoService.findByOrgId(curUserInfo.getOrgId());
					OrgSocialInfoBO pOrgSocialInfoBO = orgSocialInfoService.findByOrgId(orgSocialInfoBO.getParentOrgId());
					if(pOrgSocialInfoBO!=null) {
						//乡镇分管领导(信访)：xzfgldxinfang
						PositionInfoBO positionInfoBO = positionInfoOutService.queryPositionByCode("xzfgldxinfang");
						if(positionInfoBO!=null) {
							allList.addAll(userManageService.getUserListByUserExampleParamsOut(null, positionInfoBO.getPositionId(), pOrgSocialInfoBO.getOrgId()));
						}
						//乡镇主要领导：xzzyld
						positionInfoBO = positionInfoOutService.queryPositionByCode("xzzyld");
						if(positionInfoBO!=null) {
							allList.addAll(userManageService.getUserListByUserExampleParamsOut(null, positionInfoBO.getPositionId(), pOrgSocialInfoBO.getOrgId()));
						}
					}
				}else if(FocusReportNode362Enum.FEEDBACK_NODE_NAME.getTaskNodeName().equals(nextTaskName)
						||FocusReportNode362Enum.END_NODE_NAME.getTaskNodeName().equals(nextTaskName)) {
					//乡镇（街道、开发区）社会治理办公室负责人、分管领导、主要领导
					//乡镇分管领导(信访)：xzfgldxinfang
					PositionInfoBO positionInfoBO = positionInfoOutService.queryPositionByCode("xzfgldxinfang");
					if(positionInfoBO!=null) {
					...
					if(professionBO!=null) {
						...
						if(orgList!=null&&!orgList.isEmpty()) {
							//乡镇业务科室负责人
							positionInfoBO = positionInfoOutService.queryPositionByCode("xzywksfzr");
							if(positionInfoBO!=null) {
								allList.addAll(userManageService.getUserListByUserExampleParamsOut(null, positionInfoBO.getPositionId(), orgList.get(0).getOrgId()));
							}
						}
			...
			if(FocusReportNode36201Enum.DISTRIBUTE_NODE_NAME.getTaskNodeName().equals(curTaskName)) {
				//乡镇（街道、开发区）社会治理办公室负责人、分管领导、主要领导
				//乡镇分管领导(信访)：xzfgldxinfang
				PositionInfoBO positionInfoBO = positionInfoOutService.queryPositionByCode("xzfgldxinfang");
				if(positionInfoBO!=null) {
			...
			}else if(FocusReportNode36201Enum.FEEDBACK_NODE_NAME.getTaskNodeName().equals(curTaskName)) {
				//乡镇（街道、开发区）社会治理办公室负责人、分管领导、主要领导
				List<OrgSocialInfoBO> orgList = orgSocialInfoService.findBySelfIdAndLevel(curUserInfo.getOrgId(), 4);
				if(orgList!=null&&!orgList.isEmpty()) {
					...
					ProfessionBO professionBO = professionService.findProfessionByName(professionName);
					if(professionBO!=null) {
						orgList = orgSocialInfoService.findOrgSocialListByParentIdAndProId(orgList.get(0).getOrgId(),professionBO.getProfessionId());
						if(orgList!=null&&!orgList.isEmpty()) {
							//乡镇业务科室负责人
							positionInfoBO = positionInfoOutService.queryPositionByCode("xzywksfzr");
							if(positionInfoBO!=null) {
