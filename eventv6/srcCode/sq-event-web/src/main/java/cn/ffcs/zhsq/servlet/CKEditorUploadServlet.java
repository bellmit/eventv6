...
public class CKEditorUploadServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String currentDirPath = ConstantValue.RESOURSE_SAVE_ROOT_PATH + currentPath;
		// 判断文件夹是否存在，不存在则创建
		...
		String fileUrl = currentPath;
		currentPath = ConstantValue.RESOURSE_SAVE_ROOT_PATH + currentPath;
		if (debug)
		...
		fileUrl = App.IMG_ZZGRID.getDomain(request.getSession())+fileUrl;
		// CKEditorFuncNum是回调时显示的位置，这个参数必须有
		String callback = request.getParameter("OMEditorFuncNum");
        out.println("<script type=\"text/javascript\">");
        out.println("window.parent.OMEDITOR.tools.callFunction(" + callback
                + ",'" + fileUrl + "',''" + ")");
        out.println("</script>");  
		//out.println("图片地址："+fileUrl);
		out.flush();
		out.close();
		if (debug)
