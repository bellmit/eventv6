...
@Controller
@RequestMapping("/zhsq/personProb")
public class PersonProbController extends ZZBaseController {
	
	@Autowired
	private IPersonProbService personProbService;
	
	@RequestMapping(value="index")
	public String indexPersion(HttpSession session, ModelMap map){
		...
		return "zzgl/prob/list_personProb.ftl";
	}
	
	@RequestMapping(value="windowPrint")
	public String windowPrint(){
		return "zzgl/prob/windowPrint_person.ftl";
	}
	
	@ResponseBody
	@RequestMapping(value = "personListData",method = RequestMethod.POST)
	public EUDGPagination persionListData(HttpSession session, ModelMap map, int page, int rows,@RequestParam Map<String, Object> params){
		Map<String, Object> defaultGridInfo = this.getDefaultGridInfo(session);
		page = page < 1 ? 1 : page;
		rows = rows < 1 ? 20 : rows;
		...
		EUDGPagination accountEn = personProbService.searchList(page,rows,params);
		return accountEn;
	}
	
	@RequestMapping("/caseBriefDetail")
	public Object showCaseBriefDetail( HttpSession session, ModelMap map,int probId) {
		Map<String, Object> bo = personProbService.searchById(probId);
	...
	public String printPage(HttpSession session, ModelMap map,String regionCode,Integer page, Integer rows,@RequestParam Map<String, Object> params){
		...
		EUDGPagination pagination = personProbService.searchList(page,rows,params);
		map.addAttribute("pagination",pagination);
		return "zzgl/prob/print_page_persion.ftl";
	}
