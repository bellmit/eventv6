...
@Service(value="kuangXuanStatOfXjs")
public class KuangXuanStatOfXjsServiceImpl implements IKuangXuanStatService {
	...
	public int statOfKuangXuan(Map<String, Object> params) {
		...
		int zjsbsCount = gisInfoService.getZKSListboxChose(geoString, "O", mapType, null,infoOrgCode);//重精神病数
		return zjsbsCount;
	}
	...
	public EUDGPagination statOfKuangXuanList(int pageNo, int pageSize, Map<String, Object> params) {
		...
		params.put("peopleTypeTableName", "O");
		params.put("empWay", null);
		if (pageNo == 1) {
			pagination.setTotal(gisInfoService.getZKSListboxChose(geoString, "O", mapType, null,infoOrgCode));
		}
