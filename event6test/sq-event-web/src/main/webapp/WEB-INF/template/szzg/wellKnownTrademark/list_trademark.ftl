        ...
        <form id="searchForm">
            <div class="fl">
                <ul>
                    ...
                    <li>单位名称：</li>
                    <li>
                        <input class="inp1" type="text" id="unitName" name="unitName" />
                    </li>
                </ul>
            </div>
            <div class="btns">
...
<script type="text/javascript">
    ...
    function loadList() {
        $('#list').datagrid({
            ...
            singleSelect : true,
            fit : true,
            url : '${rc.getContextPath()}/zhsq/szzg/trademark/listData.json',
            columns : [ [
                {	field : 'gridName',
                    title : '所属网格',
                    align : 'center',
                ...
                },
                {
                    ...
                    formatter : function(value, rec, index) {
                        ...
                        var f = '<a href="###" onclick="detail('+ rec.trademarkId+ ')"><span style="color:lightred; text-decoration:underline;">'+ value+ '</span></a>&nbsp;';
                        return f;
                    }
                },
                {	field : 'brand',
    ...
    //新增
    function add() {
        var url = '${rc.getContextPath()}/zhsq/szzg/trademark/add.jhtml';
        showMaxJqueryWindow('新增商标信息', url, 600, 450);
    ...
    function edit() {
        ...
        if (rows.length != 1) {
            ...
            var id = rows[0].trademarkId;
            var url = '${rc.getContextPath()}/zhsq/szzg/trademark/edit.jhtml?id='+ id;
            showMaxJqueryWindow('编辑商标信息', url, 600, 450);
    ...
    function detail(id) {
        ...
        if (!id) {
            ...
            var url = "${rc.getContextPath()}/zhsq/szzg/trademark/detail.jhtml?id="+ id;
            showMaxJqueryWindow('商标详情', url, 650, 400);
        ...
        if (rows.length != 1) {
            ...
            $.messager.confirm('提示','您确定删除选中的信息吗?',function(r) {
                if (r) {modleopen(); //打开遮罩层
                    $.ajax({
                        type : 'POST',
                        url : '${rc.getContextPath()}/zhsq/szzg/trademark/del.json',
                        data : {trademarkId : rows[0].trademarkId},
                        dataType : 'json',
