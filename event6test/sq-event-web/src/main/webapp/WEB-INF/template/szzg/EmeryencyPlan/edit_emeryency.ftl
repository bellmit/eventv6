            ...
            <form id="tableForm" name="tableForm"
                ...
                <table width="100%" border="0" cellspacing="0" cellpadding="0">
                    ...
                    <input type="hidden" name="id"
                           value="<#if emeryencyPlanContent.id??>${emeryencyPlanContent.id}</#if>"/>
                    <input type="hidden" name="treeId"
                           value="<#if emeryencyPlanContent.treeId??>${emeryencyPlanContent.treeId}</#if>"/>
                    <tr>
                        <td class="LeftTd" colspan="2"><label class="LabName"><span><b style="color: red;">*</b>标题：</span></label>
                            <input name="title" id="title" type="text"
                                   value="<#if emeryencyPlanContent.title??>${emeryencyPlanContent.title}</#if>"
                                   style="height: 30px;width: 80.5%;" class="inp1 easyui-validatebox"
                    ...
                    <tr>
                        <td class="LeftTd"><label class="LabName"><span>排序：</span></label>
                            <input name="priority" id="priority" style="height: 30px;width: 160px;"
                                   value="<#if emeryencyPlanContent.priority??>${emeryencyPlanContent.priority}</#if>"
...
<#--                            <div class="Check_Radio"><input type="checkbox" name="status" id="status"-->
<#--                                                            <#if (emeryencyPlanContent.status?? && emeryencyPlanContent.status=='1')>checked</#if>-->
<#--                                                            onclick="changeStdValue()"/><label for="isStd"-->
...
<#--                        </td>-->
                        <td class="LeftTd">
                            ...
                            <div class="zz-form" zz-form-filter="careRoadsIsDetection">
                                <input type="checkbox" zz-form-filter="_careRoadsIsDetection" name="status" id="status" onclick="changeStdValue()" value="1" <#if (emeryencyPlanContent.status?? && emeryencyPlanContent.status=='1')>checked</#if>>
                    ...
                    <tr>
                        <td colspan="3" class="LeftTd"><label class="LabName"><span><b style="color: red;">*</b>内容：</span></label>
                            <textarea rows="4" style="width:300px;margin:10px auto" id="content" name="content">
                            <#if emeryencyPlanContent.txtStr??>${emeryencyPlanContent.txtStr}</#if>
                            </textarea>
                        </td>
                    </tr>
                </table>
