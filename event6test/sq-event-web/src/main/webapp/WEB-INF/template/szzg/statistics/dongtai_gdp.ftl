...
var orgCode = '${orgCode}';
 ...
 function findYears(){
	var option = {tooltip:{trigger:'axis'},title:{ text: '地区生产总值累计值(万元)',x:'center',textStyle:{color:'#fff',fontSize:21}},
				legend:{data:['GDP','一产值','二产值','三产值'],x:'center',y:'bottom',textStyle:{color:'#fff'}},
				...
				series : [
					{name:'GDP',type:'bar',stack:'GDP',data:[],itemStyle:{normal:{color:(function(a,b,c){return colorObj[3];})()}},barWidth:30},
					{name:'三产值',type:'bar',stack:'值',data:[],itemStyle:{normal:{color:(function(a,b,c){return colorObj[0];})()}},barWidth:30},
					{name:'二产值',type:'bar',stack:'值',data:[],itemStyle:{normal:{color:(function(a,b,c){return colorObj[1];})()}},barWidth:30},
					{name:'一产值',type:'bar',stack:'值',data:[],itemStyle:{normal:{color:(function(a,b,c){return colorObj[2];})()}},barWidth:30}]
				};
				
	var option1 = {tooltip:{trigger:'axis'},title:{ text: '生产总值增幅(%)',x:'center',textStyle:{color:'#fff',fontSize:21}},
		legend:{data:['GDP增幅','一产增幅','二产增幅','三产增幅'],x:'center',y:'bottom',textStyle:{color:'#fff'}},
		...
		series : [
			{name:'GDP增幅',type:'bar',stack: 'GDP',data:[],barWidth:30,itemStyle:{normal:{color:(function(a,b,c){return colorObj[3];})()}}},
			{name:'三产增幅',type:'bar',stack: '值',data:[],barWidth:30,itemStyle:{normal:{color:(function(a,b,c){return colorObj[0];})()}}},
			{name:'二产增幅',type:'bar',stack: '值',data:[],barWidth:30,itemStyle:{normal:{color:(function(a,b,c){return colorObj[1];})()}}},
			{name:'一产增幅',type:'bar',stack: '值',data:[],barWidth:30,itemStyle:{normal:{color:(function(a,b,c){return colorObj[2];})()}}}]
		};
	$.ajax({
		url: '${rc.getContextPath()}/zhsq/szzg/zgStatisticsController/findGDPDate.json?t='+Math.random(),
		type: 'POST',
		data: {orgCode:orgCode,table:'gdp'},
		dataType:"json",
		error: function(data){
		...
		success: function(data){ 
			...
			for(var i=data.list.length-1;i>=0;i--){
				...
				option.series[0].data.push({value:isNum(d.GDP_SUM),year:d.YEAR_,column:'GDP',colName:'GDP'});
 ...
 function findItem(param){
	var option = {tooltip:{trigger:'axis'},title:{ text: param.year+'年'+param.colName+'季度总值(亿元)',x:'center',textStyle:{color:'#fff',fontSize:21}},
				legend:{show:false,data:['总值']},toolbox:{show:true},grid:{x:50,y:30,x2:10,y2:30},calculable : false,
				xAxis : [{type:'category',data:['第一季度','第二季度','第三季度','第四季度'],axisLabel:{textStyle:{color:"#fff"}}}],
				yAxis : [{name:'',type:'value',axisLine:{lineStyle:{color: '#fff'}},axisLabel:{rotate:50,textStyle:{color:"#fff"}}}],
				series : [
					{name:'总值',type:'bar',stack:'GDP',data:[],barWidth:30,itemStyle:{normal:{label:{show:true,position:'insideLeft'}}}}]
				};
				
	var option1 = {tooltip:{trigger:'axis'},title:{ text: param.year+'年'+param.colName+'季度总值增幅(%)',x:'center',textStyle:{color:'#fff',fontSize:21}},
		legend:{show:false,data:['增幅']},toolbox:{show:true},grid:{x:40,y:30,x2:5,y2:30},calculable : false,
		xAxis : [{type:'category',data:['第一季度','第二季度','第三季度','第四季度'],axisLabel:{textStyle:{color:"#fff"}}}],
		yAxis : [{type : 'value',axisLine:{lineStyle:{color: '#fff'}},axisLabel:{textStyle:{color:"#fff"}}}],
		series : [
			{name:'增幅',type:'bar',stack: 'GDP',data:[],barWidth:30,itemStyle:{normal:{label:{show:true,position:'insideRight'}}}}]
		};
	$.ajax({
		url: '${rc.getContextPath()}/zhsq/szzg/zgStatisticsController/findGDPDate.json?t='+Math.random(),
		type: 'POST',
		data: {orgCode:orgCode,year:param.year,table:'gdp'},
		dataType:"json",
		error: function(data){
			$.messager.alert('提示','信息获取异常!','warning');
		...
		success: function(data){ 
			...
			var monthSUM = {3:0,6:0,9:0,12:0},monthINC = {3:0,6:0,9:0,12:0},month=[3,6,9,12];
			...
			for(var i=0,l=month.length;i<l;i++){
				option.series[0].data.push({value:isNum(monthSUM[month[i]]),month:month[i],column:param.column,itemStyle:{normal:{color:(function(a,b,c){return colorObj[i];})()}}});
				option1.series[0].data.push({value:isNum(monthINC[month[i]]),itemStyle:{normal:{color:(function(a,b,c){return colorObj[i];})()}}});
			}
 ...
 function findYearJiDu(param){
	var jidu = {3:'一',6:'二',9:'三',12:'四'};
	var option = {tooltip:{trigger:'axis'},title:{ text: '第'+jidu[param.month]+'季度总值趋势(亿元)',x:'center',textStyle:{color:'#fff',fontSize:21}},
				legend:{show:false,data:['总值']},toolbox:{show:true},grid:{x:50,y:30,x2:10,y2:30},calculable : false,
				xAxis : [{type:'category',data:[],axisLabel:{textStyle:{color:"#fff"}}}],
				yAxis : [{name:'',type:'value',axisLine:{lineStyle:{color: '#fff'}},axisLabel:{rotate:50,textStyle:{color:"#fff"}}}],
				series : [
					{name:'总值',type:'line',stack:'GDP',data:[],barWidth:30,itemStyle:{normal:{label:{show:true}}}}]
				};
				
	var option1 = {tooltip:{trigger:'axis'},title:{ text: '第'+jidu[param.month]+'季度总值增幅趋势(%)',x:'center',textStyle:{color:'#fff',fontSize:21}},
		legend:{show:false,data:['增幅']},toolbox:{show:true},grid:{x:40,y:30,x2:5,y2:30},calculable : false,
		xAxis : [{type:'category',data:[],axisLabel:{textStyle:{color:"#fff"}}}],
		yAxis : [{type : 'value',axisLine:{lineStyle:{color: '#fff'}},axisLabel:{textStyle:{color:"#fff"}}}],
		series : [
			{name:'增幅',type:'line',stack: 'GDP',data:[],barWidth:30,itemStyle:{normal:{label:{show:true}}}}]
		};
	$.ajax({
		url: '${rc.getContextPath()}/zhsq/szzg/zgStatisticsController/findGDPDate.json?t='+Math.random(),
		type: 'POST',
		data: {orgCode:orgCode,month:param.month,table:'gdp'},
		dataType:"json",
		error: function(data){
			$.messager.alert('提示','信息获取异常!','warning');
		...
		success: function(data){ 
			...
			for(var i=0,l=data.list.length;i<l;i++){
				...
				option.series[0].data.push(isNum(d[param.column+'_SUM']));
				option1.series[0].data.push(isNum(d[param.column+'_INC']));
			}
