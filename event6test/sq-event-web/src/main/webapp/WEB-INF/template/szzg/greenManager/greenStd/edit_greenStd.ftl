                   ...
                   <input type="hidden" name="seqid" id="seqid" value="<#if greenStd.seqid??>${greenStd.seqid}</#if>">
                    ...
                    <tr>
                        <td colspan="2" class="LeftTd"><label class="LabName"><span>指标名称：</span></label>
                            <input name="name" id="name" maxLength="100" type="text" class="inp1 easyui-validatebox" style="width: 510px;" value="<#if greenStd.name??>${greenStd.name}</#if>" data-options="tipPosition:'bottom',required:true,validType:['charts','length[0,20]']"/></td>
                    </tr>

                    <tr>
                        <td class="LeftTd"><label class="LabName"><span>所属年份：</span></label>
                            <input class="inp1 inp2 InpDisable  Wdate timeClass easyui-validatebox" type="text" id="syear" style="width:160px" value="<#if greenStd.syear??>${greenStd.syear}</#if>"
                        ...
                        <td><label class="LabName"><span>类型：</span></label>
                            <input id="type" name="type" type="hidden" value="<#if greenStd.type??>${greenStd.type}</#if>"/>
                            <input id="typeName"  data-options="required:true"  name="typeName" type="text" class="inp1 InpDisable easyui-validatebox"  value="<#if greenStd.typeName??>${greenStd.typeName}</#if>" style="width:160px;"
                        </td>
                    </tr>
                    <tr>
                        <td class="LeftTd"><label class="LabName"><span>指标标准：</span></label>
                            <input name="stdval" id="stdval" type="text" style="height: 28px;width: 160px;"  value="<#if greenStd.stdval??>${greenStd.stdval}</#if>" class="inp1 inp2 easyui-validatebox" data-options="tipPosition:'bottom',required:true,validType:['maxLength[15]','characterCheck']"/>
                        </td>
                        <td><label class="LabName"><span>我市标准：</span></label>
                            <input name="actval" id="actval" style="height: 28px;width: 160px;" value="<#if greenStd.actval??>${greenStd.actval}</#if>" type="text" class="inp1 inp2 easyui-validatebox"  data-options="tipPosition:'bottom',required:true,validType:['maxLength[15]','characterCheck']"/>
                        </td>
                    </tr>
                    ...
                    <tr>
                        <td colspan="2" class="LeftTd RightTd"><label class="LabName"><span>指标说明：</span></label>
                            <textarea name="content" id="content" cols="45" rows="60" style="width: 510px;height: 160px;"class="area1 easyui-validatebox" maxLength="251" data-options="tipPosition:'bottom',validType:['maxLength[250]','characterCheck']"><#if greenStd.content??>${greenStd.content}</#if></textarea></td>
                    </tr>
