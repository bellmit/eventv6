			...
			<table width="100%" border="0" cellspacing="0" cellpadding="0" class="select-table" style="height:255px;">
				...
				<tr>
					<td width="25%" style="background-color: #FFFFFF;" valign="top" class="select-td">
					 	<div id="leftPanel" class="easyui-panel"  data-options="title:'角色列表'" style="overflow:hidden;">
	...
	***************************************************************/
	function selectUserByObj(obj) {
		...
		var setting = {
			async : {
				enable : true,
				url : "${rc.getContextPath()}/zhsq/warnTask/selectRole.jhtml?type="+nodeType_,
				autoParam : [ 'id' ],
				otherParam : {
				},
				type : "get"
			...
			callback : {
				onAsyncSuccess : function(event, treeId, treeNode, msg) {
					...
					if(leftHtml.length == 0){
						$.messager.alert('警告','没有可使用的角色列表，请先配置！','warning');
					}
				},
				onClick : function(event, treeId, treeNode, clickFlag) {
					var roleId = parseInt(treeNode.id);
					if (!isNaN(roleId)) {
	...
	function selectUser(_hiddenId, _fileldId, _curOrgIds, _curOrgNames, _nodeType) {
		var obj = {
				...
				.datagrid(
						{
							...
							singleSelect : true,
							rownumbers : true,
							url : "${rc.getContextPath()}/zhsq/warnTask/selectUser.jhtml",
							idField : 'userId',
