...
<script type="text/javascript">
			...
			success: function(data){
				...
				if(list && list.length>0) {
					...
					for(var i=0;i<list.length;i++){
				        ...
				        tableBody+='onclick="selectRow('+val.gridId+',this)"><div  title="'+val.gridName+'"><span class="FontDarkBlue">'+gridName+'</span></div></li>';
					}
					tableBody+='</ul>';
				} else {
	...
	function loadArcgisMap(gridId){
		var url = '${rc.getContextPath()}/zhsq/map/arcgis/arcgisdata/toArcgisDrawGridPanel.jhtml?gridId='+gridId;
