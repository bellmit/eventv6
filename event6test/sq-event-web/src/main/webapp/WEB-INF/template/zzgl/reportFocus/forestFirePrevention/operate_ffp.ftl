<script type="text/javascript">
	function add(opt) {
		...
		var url = "${rc.getContextPath()}/zhsq/reportFFP/toAdd.jhtml?listType=" + $('#listType').val();
	...
	function edit() {
		...
		if(reportUUID == "") {
			...
			var url = "${rc.getContextPath()}/zhsq/reportFFP/toAdd.jhtml?listType=" + $('#listType').val() + "&reportUUID=" + reportUUID;
	...
	function del() {
		...
		if(reportUUID == "") {
			...
			$.messager.confirm('提示', '您确定删除选中的森林防灭火信息吗？', function(r) {
				if(r){
					modleopen();
					
					$.ajax({
						type: "POST",
						url: '${rc.getContextPath()}/zhsq/reportFFP/delReportFocus.jhtml',
						...
						error:function(data){
							$.messager.alert('错误','删除森林防灭火信息失败！','error');
						}
					});
				}
			});
