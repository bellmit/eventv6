...
<html>
	<head>
		...
		<style>
	    	...
	    	.btn-bon {background-color:#5294e8!important;;color: rgb(255, 255, 255)!important}
	    </style>
	</head>
	<body class="xiu-body">
		...
		<div class="container-detail">
			<div class="form-warp-sh">
				...
				<input type="hidden" id="orgCode" name="orgCode" value="<#if ORG_CODE??>${ORG_CODE}</#if>">
				<!--办理意见-->
						...
						<div class="fw-det-toggle">
							<ul class="fw-xw-from clearfix mr30">
								...
								<li class="xw-com1">
									...
									<textarea name="content" id="content" cols="" rows="" class="area1 easyui-validatebox from flex1 bg-btm-arrow1" style="width:400px; height:64px;resize: none;" data-options="required:true,tipPosition:'bottom',validType:['minLength[20]','maxLength[1024]','characterCheck']" ><#if event.content??>${event.content}</#if></textarea>
								</li>
								<li class="xw-com2">
	...
	$(function(){
		...
		AnoleApi.initListComboBox("involvedNumName", "involvedNum", "${involvedNumPcode}", null, 
				[<#if (event.involvedNum == 1)>"01"
				<#elseif (event.involvedNum == 2)>"02"
				<#elseif (event.involvedNum == 3) || (event.involvedNum == 4)>"03"
				<#elseif (event.involvedNum >= 5) && (event.involveNum < 10)>"04"
				<#elseif (event.involvedNum >= 10) && (event.involvedNum < 30)>"05"
				<#elseif (event.involvedNum >= 30)>"06"
				<#else>"00"
				</#if>]);
