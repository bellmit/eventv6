...
<div id="jqueryToolbar">
	<form id="msgReadingForm">
		...
		<input type="hidden" id="reportType" name="reportType" value="${reportType!'1'}" class="queryParam" />
...
<script type="text/javascript">
	...
	function detail(reportUUID, instanceId, msgId, msgReceiveStatus) {
		if(msgId && msgReceiveStatus && msgReceiveStatus == '0') {
			$.ajax({
				type: "POST",
				url: '${rc.getContextPath()}/zhsq/reportTwoVioPre/readMsg.jhtml',
				data: {'msgId' : msgId},
				dataType:"json",
		...
		if(reportUUID) {
			...
			var url = "${rc.getContextPath()}/zhsq/reportTwoVioPre/toDetail.jhtml?reportUUID=" + reportUUID + "&listType=" + listType + "&reportType=" + $('#reportType').val();
