			...
			<div class="fl">
				<ul>
                    ...
                    <li>
					    ...
					    <input type="text" id="startTime" name="violationDateStart" value="${yesterday!''}" style="display:none;"/>
					    <input type="text" id="endTime" name="violationDateEnd" value="${yesterday!''}" style="display:none;"/>
                    </li>
				</ul>
			</div>
...
<script type="text/javascript">
	...
	function loadList() {
		$('#list').datagrid({
			...
			columns: [[
				...
				{field:'procType', title:'问责处理<br/>类型', align:'center', width:100},
				...
				{field:'violationMoneyType', title:'违纪违法<br/>资金类别', align:'center', width:120,
			      formatter:function (value,rec,rowIndex) {
				           var f = '<span>'+formatText(value)+'</span>';
					       return f;
                }},
				{field:'violationType1', title:'违规违纪<br/>类别统计', align:'center', width:120,
			      formatter:function (value,rec,rowIndex) {
				           var f = '<span>'+formatText(value)+'</span>';
					       return f;
                }},
                {field:'violationType2', title:'作风建设<br/>类别统计', align:'center', width:120,
			      formatter:function (value,rec,rowIndex) {
				           var f = '<span>'+formatText(value)+'</span>';
					       return f;
                }},
				{field:'violationType0', title:'扫黑除恶<br/>类别统计', align:'center', width:120,
			      formatter:function (value,rec,rowIndex) {
				           var f = '<span>'+formatText(value)+'</span>';
					       return f;
                }},
				{field:'shape', title:'四种形态', align:'center', width:100},
				{field:'amountOfRecovery', title:'追缴资金<br/>(万元)', align:'center', width:110}
	...
	function detail(probId) {
		var url = "${rc.getContextPath()}/zhsq/unitProb/caseBriefDetail.jhtml?probId=" + probId;
	...
	function windowPrint() {
	    var url = '${rc.getContextPath()}/zhsq/unitProb/windowPrint.jhtml';
	...
	function exportExcel() {
		...
		var url = "${rc.getContextPath()}/common/export/excel.jhtml?beanId=unitProbService" + pm;
		window.open(url);
	}
...
var pm = "";		
	...
	pm+="&page="+page+"&rows="+rows;
	 var url = '${rc.getContextPath()}/zhsq/unitProb/printPage.jhtml?'+pm;
	window.open(url);
}
</script>
</html>
