...
<script type="text/javascript">
	...
	function detail(reportUUID, instanceId, msgId, msgReceiveStatus) {
		if(msgId && msgReceiveStatus && msgReceiveStatus == '0') {
			$.ajax({
				type: "POST",
				url: '${rc.getContextPath()}/zhsq/reportFFP/readMsg.jhtml',
				data: {'msgId' : msgId},
				dataType:"json",
		...
		if(reportUUID) {
			...
			var url = "${rc.getContextPath()}/zhsq/reportFFP/toDetail.jhtml?reportUUID=" + reportUUID + "&listType=" + listType + "&reportType=" + $('#reportType').val();
