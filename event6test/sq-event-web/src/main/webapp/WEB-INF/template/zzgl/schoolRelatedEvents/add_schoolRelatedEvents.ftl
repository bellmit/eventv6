			...
			<div class="NorForm" style="width:784px;">
				<div class="fl" >
					<table  style="width:798px;"  border="0" cellspacing="0" cellpadding="0">
						<tr>
							<td class="LeftTd" <#if relatedEvents.reNo??><#else> colspan="2"</#if>>
								<label class="LabName"><span><label style="color:red;">*</label>案件名称：</span></label><input type="text" class="inp1 easyui-validatebox" data-options="required:true,validType:['maxLength[100]','characterCheck']" name="reName" id="reName" value="<#if relatedEvents.reName??>${relatedEvents.reName}</#if>" style="<#if relatedEvents.reNo??> width:400px;<#else> width:660px;</#if>" />
							</td>
						...
						<tr>
							<td class="LeftTd" style="width:520px;">
								<label class="LabName"><span><label style="color:red;">*</label>所在学校：</span></label>
								<input class="comboselector" type="text" id="lotId" data-options="required:true, dType: 'campus', afterSelect: combo_afterSelect<#if relatedEvents.bizId??>,value: ${relatedEvents.bizId?c}</#if>" style="width: 155px; height:28px" query-params="gridId=${gridId?c}" />
							</td>
							<td>
				    	...
				    	<tr>
							<td class="LeftTd">
								<label class="LabName"><span>发生日期：</span></label><input type="text" class="easyui-datetimebox easyui-validatebox" editable="false" style="width: 155px;height: 28px;" id="occuDateStr" name="occuDateStr" value="<#if relatedEvents.occuDateStr??>${relatedEvents.occuDateStr}</#if>"/>
							</td>
							<td>
						...
						<tr>
							...
							<td >
				    			<label class="LabName"><span><label style="color:red;">*</label>证件号码：</span></label><input type="text" style="width:130px;"  class="inp1 easyui-validatebox" data-options="required:true,validType:['maxLength[50]','characterCheck']" name="prisonersDocNo" id="prisonersDocNo" value="<#if relatedEvents.prisonersDocNo??>${relatedEvents.prisonersDocNo}</#if>" />
				    		</td>
				        </tr>
				    	<tr>
							<td class="LeftTd">
				    	...
				    	<tr>
							...
							<td>
				    			<label class="LabName"><span>作案人数：</span></label><input type="text" class="inp1 easyui-numberspinner" style="width:135px;"  data-options="min:0,validType:'maxLength[6]'" name="crimeNum" id="crimeNum" value="<#if relatedEvents.crimeNum??>${relatedEvents.crimeNum?c}<#else>0</#if>" style="width:114px; height:28px;" /></li>
				    		</td>
						</tr>
				    </table>
				</div>
...
<script type="text/javascript">
	...
	$(function(){
        ...
        layer.load(0);// 加载遮罩层
		$.excuteFieldCfg({
			moduleCode: "school_related_events"// 必传，模块编码
	...
	function tableSubmit(m){
		...
		if(isValid && isNoValid){
			...
			$("#tableForm").attr("action","${rc.getContextPath()}/zhsq/relatedEvents/SchoolRelatedEventsController/"+m+".jhtml");
	      	modleopen();
