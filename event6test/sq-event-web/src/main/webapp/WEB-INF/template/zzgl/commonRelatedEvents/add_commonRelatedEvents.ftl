			...
			<div class="NorForm" style="width:784px;">
				<div class="fl" style="width:520px;">
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
						...
						<tr>
							<td class="LeftTd">
								<label class="LabName"><span><font color="red">*</font>发生地点：</span></label><input type="text" class="inp1 easyui-validatebox" data-options="required:true,validType:['maxLength[100]','characterCheck']" name="occuAddr" id="occuAddr" value="<#if relatedEvents.occuAddr??>${relatedEvents.occuAddr}</#if>" style="width:400px;" />
							</td>
						</tr>
				    	<tr>
							<td class="LeftTd">
				    	...
				    	<tr>
							<td class="LeftTd">
								<label class="LabName"><span><font color="red">*</font>案件情况：</span></label><textarea name="situation" id="situation" cols="" rows="" class="area1 easyui-validatebox" data-options="required:true,validType:['maxLength[4000]','characterCheck']" style="width:400px;height:64px;resize:none;"><#if relatedEvents.situation??>${relatedEvents.situation}</#if></textarea>
							</td>
						</tr>
				    	<tr>
							<td class="LeftTd">
								<label class="LabName"><span><font color="red">*</font>侦破情况：</span></label><textarea name="detectedOverview" id="detectedOverview" cols="" rows="" class="area1 easyui-validatebox" data-options="required:true,validType:['maxLength[4000]','characterCheck']" style="width:400px;height:64px;resize:none;"><#if relatedEvents.detectedOverview??>${relatedEvents.detectedOverview}</#if></textarea>
							</td>
						</tr>
				    </table>
				</div>
				<div class="fr" style="width:264px;">
					<table width="100%" border="0" cellspacing="0" cellpadding="0">
						<tr>
							<td>
						...
						<tr>
				    		<td>
				    			<label class="LabName"><span><font color="red">*</font>主犯姓名：</span></label><input type="text" class="inp1 easyui-validatebox" data-options="required:true,validType:['maxLength[32]','characterCheck']" name="prisonersName" id="prisonersName" value="<#if relatedEvents.prisonersName??>${relatedEvents.prisonersName}</#if>" />
				    		</td>
				    	</tr>
				    	<tr>
				    		<td>
					    		...
					    		<input type="text" class="inp1" id="prisonersDocTypeName" value="<#if relatedEvents.prisonersDocTypeName??>${relatedEvents.prisonersDocTypeName}</#if>" />
				    		</td>
				    	</tr>
				    	<tr>
				    		<td style="border-bottom:none;">
				    			<label class="LabName"><span><font color="red">*</font>证件号码：</span></label><input type="text" class="inp1 easyui-validatebox" data-options="required:true,validType:['maxLength[50]','characterCheck']" name="prisonersDocNo" id="prisonersDocNo" value="<#if relatedEvents.prisonersDocNo??>${relatedEvents.prisonersDocNo}</#if>" />
				    		</td>
				    	</tr>
				    	<tr>
				    		<td>
				    			<label class="LabName"><span><font color="red">*</font>在逃人数：</span></label><input type="text" class="inp1 easyui-numberspinner" data-options="required:true,min:0,validType:'maxLength[5]',onSpinUp:function(){countCrimeNum();},onSpinDown:function(){countCrimeNum();},onChange:function(){countCrimeNum();}" name="ecapeNum" id="ecapeNum" value="<#if relatedEvents.ecapeNum??>${relatedEvents.ecapeNum?c}<#else>0</#if>"  style="width:114px; height:28px;" />
				    		</td>
				    	</tr>
				    	<tr>
				    		<td style="border-bottom:none;">
				    	...
				    	<tr>
				    		<td>
				    			<label class="LabName"><span><font color="red">*</font>作案人数：</span></label><input type="text" class="inp1 easyui-numberspinner" data-options="min:0,validType:'maxLength[6]'" name="crimeNum" id="crimeNum" value="<#if relatedEvents.crimeNum??>${relatedEvents.crimeNum?c}<#else>0</#if>" style="width:114px; height:28px;" /></li>
				    		</td>
				    	</tr>
				    </table>
				</div>
...
<script type="text/javascript">
	...
	function tableSubmit(m){
		...
		if(isValid && isNoValid){
			...
			$("#tableForm").attr("action","${rc.getContextPath()}/zhsq/relatedEvents/CommonRelatedEventsController/"+m+".jhtml");
	      	modleopen();
