...
<script type="text/javascript">
	...
	function loadList() {
		$('#list').datagrid({
			...
			singleSelect: true,
			fit: true,
			url: '${rc.getContextPath()}/zhsq/warnTask/listData.jhtml?indexType=end',
			//queryParams: $('#searchForm').serializeJson(),
			columns: [[
				{field:'deviceName', title:'设备名称', align:'center', width:120},
				{field:'deviceTypeStr', title:'设备类型', align:'center', width:80},
				{field:'deviceAddr', title:'设备地址', align:'center', width:120,formatter : function(val, rec) {
					return '<span title="'+val+'">'+val+'</span>';
				}},
				{field:'warnTimeStr', title:'告警时间', align:'center', width:80},
				{field:'warnInfo', title:'告警信息', align:'center', width:120,formatter : function(val, rec) {
