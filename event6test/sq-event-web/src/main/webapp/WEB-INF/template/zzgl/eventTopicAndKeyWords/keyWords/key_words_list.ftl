...
<script type="text/javascript">
	...
	function loadList() {
		$('#list').datagrid({
			...
			columns: [[
				...
				{field:'topicName', title:'词云名称', align:'center', width:100,formatter:topicNameFmt},
				{field:'isReleaseStr', title:'发布状态', align:'center', width:100}
			]],
			pagination: true,
			pageSize: 20,
	...
	function add() {
		...
		showMaxJqueryWindow('新增', url, 500, 200);
	}
	...
	function edit() {
		...
		if (rows.length != 1) {
			...
			var url = '${rc.getContextPath()}/zhsq/eventTopic/form.jhtml?id=' + rows[0].id_ + '&bizType=' + bizType;
			showMaxJqueryWindow('编辑', url, 500, 200);
		}
	}
	...
	function detail(id) {
		...
		showMaxJqueryWindow('详情', url, 500, 200);
	}
		...
		$.ajax({
			...
			success: function(data) {
				if(data != null && data == -1){
					...
					$.messager.alert('提示', '最多只能发布20条记录！', 'info');
					return;
				}else{
					release(id);
				}
