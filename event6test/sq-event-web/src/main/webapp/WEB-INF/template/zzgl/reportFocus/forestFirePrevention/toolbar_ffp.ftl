...
<div id="jqueryToolbar">
	<form id="ffpQueryForm">
		...
		<input type="hidden" id="reportType" name="reportType" value="${reportType!'9'}" class="queryParam iconParam" />
...
<script type="text/javascript">
	$(function() {
		...
		if($('#reportStatus').length > 0) {
			AnoleApi.initTreeComboBox("reportStatusName", "reportStatus", "B210009001", null, null, {
				RenderType : "01",
				ShowOptions: {
					EnableToolbar : true
				}
		...
		if($('#dataSourceArrayName').length > 0) {
			AnoleApi.initTreeComboBox("dataSourceArrayName", "dataSourceArray", "B210009004", null, null, {
				RenderType : "01",
				ShowOptions: {
					EnableToolbar : true
				}
...
<@block name="operateFunction">
	<#include "/zzgl/reportFocus/forestFirePrevention/operate_ffp.ftl" />
</@block>
