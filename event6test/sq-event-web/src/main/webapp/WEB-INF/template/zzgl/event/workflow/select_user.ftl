...
<body>
	<!-- 组织、角色、职位人员选择面板 -->
	<div id="userSelectorWin" class="easyui-window hide">
...
<script type="text/javascript">
	...
	function alterDatagrid() {//调整表格行高、列宽
		...
		var btnWidth = $('#userSelectorBtnList').find("a").length > 1 ? 2 : 0;
		$('#userSelectorBtnList').find("a").each(function() {
			btnWidth = btnWidth + $(this).outerWidth(true) + 1;
		});
		$('#userSelectorBtnList').css("width", btnWidth);
	}
	...
	********************************************************************/
	function selectUserByObj(obj) {
		...
		if(isShowOrgNameFuzzyQuery == true) {
			$('#leftTree').height(220 - 10 - $('#orgInfoSearchCondition4User').outerHeight(true) - 2);
			$('#orgInfoSearchCondition4User').show();
