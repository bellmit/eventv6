	                            ...
	                            <div class="RightShadow">
	                                <div class="list NorForm" style="position:relative;">
	                                    <table width="100%" border="0" cellspacing="0" cellpadding="0">
	                                    	...
	                                    	<tr>
	                                    		<td>
	                                    			<label class="LabName width65px"><span>办理状态：</span></label>
	                                    			<input type="text" id="state" name="state" class="hide queryParam" />
	                								<input type="text" id="stateStr" value="" class="inp1 w150" />
	                                    		</td>
	                                    	</tr>
	                                    	<tr>
	                                    		<td>
	                                    			<label class="LabName width65px"><span>提醒状态：</span></label>
	                                    			<input type="text" id="childState" name="childState" class="hide queryParam" />
	                								<input type="text" id="childStateStr" value="" class="inp1 w150" />
	                                    		</td>
	                                    	</tr>
	                                    	<tr>
	                                    		<td>
...
<script type="text/javascript">
	$(function() {
		...
		AnoleApi.initListComboBox("stateStr", "state", null, null, [""], {
			ShowOptions : {
				EnableToolbar : true
			},
        	DataSrc: [{"name":"受理中", "value":"1"},{"name":"单位处理", "value":"2"},{"name":"退回发起人", "value":"3"}]
        });
		AnoleApi.initListComboBox("childStateStr", "childState", null, null, [""], {
			ShowOptions : {
				EnableToolbar : true
			},
        	DataSrc: [{"name":"仅退回", "value":"1"},{"name":"仅待评价", "value":"2"},{"name":"退回、待评价", "value":"3"}]
        });
	...
	function loadList() {
		$('#list').datagrid({
			...
			singleSelect: true,
			fit: true,
			url: '${rc.getContextPath()}/zhsq/requestion/listDataDB_Main.jhtml',
			...
			columns: [[
				{field:'code', title:'案件编号', align:'center', width:100,formatter : function(val, rec) {
					...
					}else if(rec.childState=="2"){
						temp += '<i title="有待评价信息" class="childState_2" style="padding-right: 5px;"></i>';
					}else if(rec.childState=="3"){
						temp += '<i title="有退回、待评价信息" class="childState_3" style="padding-right: 5px;"></i>';
					}else{
	...
	function todo() {
		...
		if (rows.length != 1) {
			...
			var nodeName = rows[0].wfActivityName_;
