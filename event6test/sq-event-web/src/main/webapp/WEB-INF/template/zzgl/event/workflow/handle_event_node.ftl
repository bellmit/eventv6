...
<body>
	...
	<input type="hidden" id="formTypeId" name="formTypeId" value="<#if curNode.formTypeId??>${curNode.formTypeId?c}<#elseif formTypeId??>${formTypeId}</#if>"> 
...
<script type="text/javascript">
		...
		if($("#sendSms_").attr("checked")){
			...
			if(flag){
				...
				$.ajax({
					...
					data: <@block name="smsContentDataOption">{'formId': $("#formId").val(), 'taskId': $("#taskId").val(), 'curnodeName': $("#curNodeName").val(), 'nodeName': $("#nodeName_").val(), 'advice': $("#advice").val(), 'receiverIds': $('#userIds').val(), 'nodeId': nodeId<@block name="smsContentDataExtraOption"></@block>}</@block>,
					dataType:"json",
					success: function(data){
	...
	<@block name="extraHandleFunction"></@block>
</script>
