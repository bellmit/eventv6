<script type="text/javascript">
	function add(opt) {
		...
		var url = "${rc.getContextPath()}/zhsq/reportTOT/toAdd.jhtml?listType=" + $('#listType').val();
	...
	function edit() {
		...
		if(reportUUID == "") {
			...
			var url = "${rc.getContextPath()}/zhsq/reportTOT/toAdd.jhtml?listType=" + $('#listType').val() + "&reportUUID=" + reportUUID;
	...
	function del() {
		...
		if(reportUUID == "") {
			...
			$.messager.confirm('提示', '您确定删除选中的整治信息吗？', function(r) {
				if(r){
					modleopen();
					
					$.ajax({
						type: "POST",
						url: '${rc.getContextPath()}/zhsq/reportTOT/delReportFocus.jhtml',
						...
						error:function(data){
							$.messager.alert('错误','删除整治信息失败！','error');
						}
					});
				}
			});
	...
	function detail(reportUUID, instanceId, listType) {
		if(reportUUID) {
			...
			var url = "${rc.getContextPath()}/zhsq/reportTOT/toDetail.jhtml?reportUUID=" + reportUUID + "&listType=" + listType + "&reportType=" + $('#reportType').val();
