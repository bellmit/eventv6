...
<div class="MainContent">
	<#include "/zzgl/reportFocus/reportFocusExtend/toolbar_reportFocusExtend_msgReading.ftl" />
</div>
...
<script type="text/javascript">
    ...
    function loadDataList(){
    	...
    	$('#list').datagrid({
			...
			singleSelect: true,
			idField:'msgId',
			url:'${rc.getContextPath()}/zhsq/reportFocus/listIntegrationMsgReadingData.json',
			columns:[[
				...
				{field:'reportTypeName', title:'模块类型', align:'left', width:fixWidth(0.08)},
			...
			onDblClickRow:function(index,rec) {
				...
				detail(rec.reportUUID, rec.reportType, rec.reportTypeName, instanceId, rec.msgId, rec.msgReceiveStatus);
			},
	...
	function clickFormatter(value, rec, rowIndex) {
		...
		if(value) {
			...
			title += '<a href="###" title="'+ value +'" onclick="detail(\'' + rec.reportUUID + '\', \'' + rec.reportType + '\', \'' + rec.reportTypeName + '\', \'' + instanceId + '\', \'' + rec.msgId + '\', \'' + rec.msgReceiveStatus + '\')">'+ value +'</a>';
		}
