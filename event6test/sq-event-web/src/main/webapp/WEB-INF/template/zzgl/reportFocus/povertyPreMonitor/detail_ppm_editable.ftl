...
<div class="container_fluid">
    ...
    <div id="formDiv" class="form-warp-sh form-warp-sh-min"><!-- 外框 -->
        ...
        <div id="mainDiv" class="fw-main tabContent" style="border-bottom: 1px solid #e5e5e5">
            <form id="ppmDetailForm" name="ppmDetailForm" action="" method="post">
                ...
                <input type="hidden" id="ppmRiskOri" name="ppmRiskOri" value="${reportFocus.ppmRisk!}" />
                <!--用于地图-->
                        ...
                        <table width="100%" border="0" cellspacing="0" cellpadding="0">
                            <tr>
                                ...
                                <td class="LeftTd">
                                    ...
                                    <#else >
                                        <input type="text" id="reportTime" name="reportTime" class="inp1 Wdate easyui-validatebox singleCellClass isEdit"
                                               style="width:220px; cursor:pointer;" data-options="required:true"
                                               onclick="WdatePicker({readOnly:true, maxDate:'${(reportFocus.reportTimeStr!maxReportTimeStr)?substring(0,10)} 23:59:59', dateFmt:'yyyy-MM-dd HH:mm:ss', isShowClear:false, isShowToday:false})"
                                               value="${reportFocus.reportTimeStr!}" readonly="readonly"></input>
                                    </#if>-->
                                    <div class="Check_Radio FontDarkBlue">${reportFocus.reportTimeStr!}</div>
                                </td>
                            </tr>
                            <tr>
                                ...
                                <td class="LeftTd" id="povBackReasonTd">
                                    ...
                                    <#else >
                                        ...
                                        <input  id="povBackReasonName" type="text" class="inp1 easyui-validatebox singleCellInpClass isEdit"
                                                style="width: 220px;" data-options="tipPosition:'bottom',validType:['maxLength[50]','characterCheck']" />
                                    </#if>
                                </td>
                                <#--<td class="LeftTd">
                            ...
                            <tr>
                                <td class="LeftTd">
                                    ...
                                    <#else >
                                        ...
                                        <input  id="resident" type="text" class="inp1 easyui-validatebox singleCellInpClass isEdit"
                                                style="cursor: pointer;width: 220px;" data-options="required:true,tipPosition:'bottom',validType:['maxLength[100]','characterCheck']"
                            ...
                            <#if reportFocus.ppmRisk?? &&( reportFocus.dataSource == '01' || reportFocus.dataSource == '05' ||  reportFocus.dataSource == '06' )>
                                <tr id="ppmRiskTr" style="display: none">
                                    <td colspan="2" id="ppmRiskTd">
                                        <label class="LabName"><span style="display: block;width: 85px;"><label class="Asterik">*</label>存在致贫返贫风险：</span></label>
                                        <div class="Check_Radio FontDarkBlue doubleCellClass">${reportFocus.ppmRiskName!}</div>
                            ...
                            <tr id="povBackDescTr" class="hide">
                                <td class="LeftTd" colspan="2">
                                    ...
                                    <#else >
                                        <textarea name="povBackDesc" id="povBackDesc" cols="" rows="" class="area1 autoWidth easyui-validatebox" style="height:64px;resize: none;width: 700px;" data-options="tipPosition:'bottom',validType:['maxLength[500]','characterCheck']">${reportFocus.povBackDesc!}</textarea>
                                    </#if>
                                </td>
                            </tr>
                            <tr>
            ...
            <#if isAble2Handle?? && isAble2Handle>
                <div class="fw-det-tog fw-det-tog-n mt20">
                    <div class="fw-det-tog-top" id="fw-det-tog-top">
                        <h5><i style="background:#4f91ff;"></i>处置流程</h5>
                    </div>
                    <div class="fw-chul" id="fw-chul">
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $('#ppmDetailForm .autoWidth').each(function() {
        ...
        if(dataSource == '01'){
            ...
            $('#povBackReason').val('');
            $('#povBackReasonName').validatebox({required:false});
        }else{
            $('#povBackReasonTd').show();
        ...
        BaseWorkflowNodeHandle.initParam({
            subTask: {
                subTaskUrl: '${rc.getContextPath()}/zhsq/reportPPM/subWorkflow4ReportFocus.jhtml?reportType=' + reportType,
            ...
            delete: {
                deleteUrl: '${rc.getContextPath()}/zhsq/reportPPM/delReportFocus.jhtml?reportType=' + reportType + '&reportUUID=' + $('#reportUUID').val()
            },
            evaluate: {
                isShowEva: false
            },
        ...
        detailFormJsonObject = $('#ppmDetailForm').serializeArray();
    ...
    function startWorkflow() {//启动流程
        ...
        if(reportId) {
            $("#ppmDetailForm").attr("action", "${rc.getContextPath()}/zhsq/reportPPM/startWorkflow4ReportFocus.jhtml");

            modleopen();

            $("#ppmDetailForm").ajaxSubmit(function(data) {
                modleclose();
    ...
    function radioCheckCallback(data) {//下一环节选中回调方法
        ...
        if(curNodeName == 'task1' && nextNodeName == 'task4'){
            $('#povBackReasonTd').show();
            $('#povBackReasonName').validatebox({required: true});
        }else if(curNodeName == 'task1' && nextNodeName == 'end1'){
            $('#povBackReasonTd').hide();
            $('#povBackReason').val('');
            $('#povBackReasonName').validatebox({required:false});
        }
    ...
    function _subTaskOperate() {
        ...
        if($('#povBackReasonTd').length == 1 && $('#povBackReasonTd').is(':visible')) {
            $('#povBackReasonName').validatebox({
                required: true
            });
            isValid =  $("#ppmDetailForm").form('validate');
        }
    ...
    function savePPM() {
        var isValid =  $("#ppmDetailForm").form('validate'),
            longitude = $('#x').val(),
            latitude = $('#y').val(),
            mapType = $('#mapt').val(),
            invalidTimeStamp = $('#invalidTimeStamp').val();
		
        isValid = isValid && checkAttachmentStatus4BigFileUpload('bigFileUploadDiv') && checkAttachmentStatus4BigFileUpload('bigVideoUploadDiv');
        ...
        if(isValid) {
            ...
            $("#ppmDetailForm").attr("action", "${rc.getContextPath()}/zhsq/reportPPM/saveReportFocus.jhtml");
            ...
            $("#ppmDetailForm").ajaxSubmit(function(data) {
                ...
                if(data.success && data.success == true) {
                	...
                	if(isEditableNode == 'true') {
                        ...
                        //回笼处置编辑了 存在致贫返贫风险时 会影响到下一环节的获取 所以在此处刷新页面
                        /*var ppmRiskOri = $('#ppmRiskOri').val();
                        var ppmRisk = $('#ppmRisk').val();
                        if(isNotBlankString(ppmRiskOri) && isNotBlankString(ppmRisk) && ppmRiskOri != ppmRisk){
                            var url = '${rc.getContextPath()}/zhsq/reportPPM/toDetail.jhtml?reportUUID='+ $('#reportUUID').val() + '&listType=2&reportType=11'+'&instanceId=' +$('#instanceId').val();
    ...
    function formCompare4PPM() {
    	var detailFormJsonObjectNow = $('#ppmDetailForm').serializeArray();
