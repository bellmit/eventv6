...
function CloseSearchBtn(){
	...
	function loadMessage(pageNo,pageSize,searchType){
		...
		$.ajax({
			type: "POST",
			url:'${rc.getContextPath()}/zhsq/jointDefence/findTenTeam.json',  
			data: postData,
			dataType:"json",
			success: function(data){
				...
				if(list && list.length>0) {
					for(var i=0;i<list.length;i++){
					  var val=list[i];
					  var imageGenderSpan="";
					  tableBody+='<dl onclick="selected(\''+val.INFO_ORG_CODE+'\',\''+val.USER_ID+'\')">';
					  tableBody+='<dt>';
					  tableBody+='<b class="FontDarkBlue">'+isNull(val.GRID_NAME)+'</b>';
					  tableBody+='</dt>';
					  tableBody+='<dd>'+isNull(val.PARTY_NAME)+'</dd>';
					  tableBody+='</dl>';
					  results.push({wid:(val.USER_ID+'&orgCode='+val.INFO_ORG_CODE),x:val.CEN_LON,y:val.CEN_LAT,gridName:val.PARTY_NAME,name:val.PARTY_NAME,elementsCollectionStr:'menuSummaryUrl_,_'+obj.menuSummaryUrl});
					}
				} else {
	...
	function selected(orgCode, userId){
		...
		setTimeout(function() {
				obj = window.parent.analysisOfElementsCollectionList(elementsCollectionStr);
				window.parent.getDetailOnMapOfListClick(elementsCollectionStr,obj['menuSummaryWidth'],obj['menuSummaryHeight'],userId+'&orgCode='+orgCode);
		},1000);
	}
	//--定位
