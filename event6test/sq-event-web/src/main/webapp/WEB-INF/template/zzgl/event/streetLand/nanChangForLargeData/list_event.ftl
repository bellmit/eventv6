...
<div class="MainContent">
	<@block name="eventToolbar">
		<#include "/zzgl/event/streetLand/nanChangForLargeData/eventDataGridToolbar.ftl" />
	</@block>
</div>
...
<script type="text/javascript">
	...
	<#if extraParams??>
        ...
        $('#list').datagrid({
            ...
            idField:'eventId',
            url:'${rc.getContextPath()}/zhsq/event/streetLandEventController/listData.json',
	    ...
	    function showDetailRow(eventId,instanceId,workFlowId,bizPlatform,type){
	        if(!eventId){
	            ...
	            if(bizPlatform == "001"){
	                ...
	                var url = "${rc.getContextPath()}/zhsq/event/streetLandEventController/detailEvent.jhtml?eventType=${eventType}&instanceId="+instanceId+"&workFlowId="+workFlowId+"&eventId="+eventId+"&model=${model}&cachenum=" + Math.random();
    ...
    function attentionEvent(obj,isAttention,eventId){
        ...
        if(attention=='添加关注'){
            $.ajax({
                type: "POST",
                url:'${rc.getContextPath()}/zhsq/event/streetLandEventController/addAttention.json?t='+Math.random(),
                data: postData,
                dataType:"json",
            ...
            $.ajax({
                type: "POST",
                url:'${rc.getContextPath()}/zhsq/event/streetLandEventController/canclAttention.json?t='+Math.random(),
                data: postData,
                dataType:"json",
                success: function(data){
                    //alert(data);
    ...
    function remindEvent1(obj,eventId,instanceId){
        var url = "${rc.getContextPath()}/zhsq/event/streetLandEventController/toAddRemind.jhtml?eventId=" + eventId + "&instanceId=" +instanceId;
        showMaxJqueryWindow("督办信息", url, 480, 280);
    }

    function urgeEvent(obj,eventId,instanceId){
        var url = "${rc.getContextPath()}/zhsq/event/streetLandEventController/toAddUrge.jhtml?eventId=" + eventId + "&instanceId=" +instanceId;
