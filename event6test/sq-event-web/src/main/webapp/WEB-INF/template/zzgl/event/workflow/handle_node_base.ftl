	...
	(function($) {
		...
		$(function() {
			var defaultMsg = $("#otherMobileNums").attr("defaultValue"),
				...
				isChooseDefaultRadio = <#if isChooseDefaultRadio??> ${isChooseDefaultRadio?c} <#else> true </#if> ;

			//是否进行下一环节节点的默认选择
			if(isChooseDefaultRadio == 'true' || isChooseDefaultRadio == true){
				defaultRadioCheck();
			}
						...
						if (data.msg && data.msg != "") {
							...
							if (isDisplayUser) {//只展示人员信息，不可修改
								...
								if(htmlUserNames!=null && htmlUserNames.length>0){
									...
									htmlUserArray = htmlUserNames.split(',');
									htmlIdArray = htmlUserIds.toString().split(',');
									htmlUserOrgIdArray = htmlUserOrgIds.toString().split(',');
									len = htmlUserArray.length;
