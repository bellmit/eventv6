...
function CloseSearchBtn(){
	...
	function loadMessage(pageNo,pageSize,searchType){
		...
		var postData = 'page='+pageNo+'&rows='+pageSize+'&gridId='+gridId+'&corName='+corName+'&economicType='+economicType;
		$.ajax({
			type: "POST",
			url:'${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofregion/corEnvListData.json?t='+Math.random(),  
			data: postData,
			dataType:"json",
			success: function(data){
				...
				if(list && list.length>0) {
					for(var i=0;i<list.length;i++){
					  ...
					  tableBody+='<dl onclick="selected(\''+val.envId+'\',\''+(val.corName==null?'':val.corName)+'\')">';
					  tableBody+='<dt>';
...
function gisPosition(res){
    ...
    if($('#elementsCollectionStr').val() != "") {
        var url = "${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofregion/getArcgisCorEnvDataList.jhtml?ids="+res+"&showType=2";
