			...
			success : function(data) {
				...
				if(list && list.length>0) {
					for(var i=0;i<list.length;i++){
					  ...
					  tableBody+='<dl onclick="selected(\''+val.ORG_ID+'\',\''+(val.NAME_==null?'':val.NAME_)+'\')">';
					  tableBody+='<dt>';
					  tableBody+='<b class="FontDarkBlue">'+(val.NAME_==null?'':val.NAME_)+'</b>';
					  tableBody+='</dt>';
					  tableBody+='<dd>地址：'+(val.REGISTERED_ADDR==null?'':val.REGISTERED_ADDR)+'</dd>';
					  tableBody+='</dl>';
					  
					  results=results+","+val.ORG_ID;
