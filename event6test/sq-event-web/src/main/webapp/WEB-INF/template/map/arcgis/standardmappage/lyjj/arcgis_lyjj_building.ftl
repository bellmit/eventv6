...
function CloseSearchBtn(){
				...
				if(list && list.length>0) {
					for(var i=0;i<list.length;i++){
					  ...
					  if(buildingAddr==null){
					  	buildingAddr = "暂无";
					  }else if(buildingAddr!=null && buildingAddr!="" && buildingAddr.length>16){
						buildingAddr = buildingAddr.substring(0,16)+"...";
					  }else{
					  ...
					  tableBody+='</dt>';
					  tableBody+='<dd title=\''+(val.buildingAddr==null?'':val.buildingAddr)+'\'>'+buildingAddr+'</dd>';
					  tableBody+='</dl>';
	...
	function selected(id){
		setTimeout(function() {
			window.parent.localtionLyjjBuildingPoint(id);
		},1000);
	}
	...
	function gisPosition(res){
		if (res==""){
	  	...
	  	var buildingurl="${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataoflyjj/getArcgisLocateDataListOfBuilding.jhtml?buildingIds="+res;
	  	window.parent.currentLayerLocateFunctionStr="getArcgisDataOfLyjjBuilding('"+buildingurl+"','dwl')";
		window.parent.getArcgisDataOfLyjjBuilding(buildingurl,'dwl');
	}
</script>
</body>
</html>