...
function CloseSearchBtn(){
	...
	function loadMessage(pageNo,pageSize,searchType){
		...
		var postData = 'itype='+itype+'&page='+pageNo+'&rows='+pageSize+'&gridId='+gridId+'&disputeEventName='+disputeEventName+'&handleDateFlag='+handleDateFlag;
		$.ajax({
			type: "POST",
			...
			success: function(data){ 
				...
				if(list && list.length>0) {
					for(var i=0;i<list.length;i++){
					  ...
					  if(disputeEventName!=null && disputeEventName!="" && disputeEventName.length>7){
						  disputeEventName = disputeEventName.substring(0,7)+"...";
					  }
					  
					  var happenAddr = val.happenAddr;
					  if(happenAddr!=null && happenAddr!="" && happenAddr.length>18){
						  happenAddr = happenAddr.substring(0,18)+"...";
					  }
					  ...
					  tableBody+='<b class="FontDarkBlue" >'+(disputeEventName==null?'':disputeEventName)+'</b>' + '<span class="fr">'+happenTimeStr+'</span>';
					  tableBody+='</dt>';
