			...
			success : function(data) {
				...
				if(list && list.length>0) {
					for(var i=0;i<list.length;i++){
					  ...
					  tableBody+='<dl onclick="selected(\''+val.RENT_ID+'\',\''+(val.ROOM_NAME==null?'':val.ROOM_NAME)+'\')">';
					  tableBody+='<dt>';
					  tableBody+='<b class="FontDarkBlue">'+(val.ROOM_NAME==null?'':val.ROOM_NAME)+'</b>';
					  tableBody+='</dt>';
