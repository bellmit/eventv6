...
<title>框选资源列表（防栓、井盖、路灯、供水、供热、燃气）</title>
...
<body style="border:none">
	...
	function selected(id){
		...
		setTimeout(function() {
			if($('#elementsCollectionStr').val() != "") {
				...
				window.parent.localtionResourcePoint('${resTypeCode!''}',id);
			}
		},1000);
	}
	//--定位
	function gisPosition(res){
		if (typeof parent.MMApi != "undefined") {	// 判断是否是高德地图
			...
			opt.ecs = $('#elementsCollectionStr').val();
			opt.gridId = $('#gridId').val();
			opt.url = "${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofgoods/getArcgisLocateDataListOfResource.jhtml?resTypeCode=${resTypeCode!''}&ids="+res;
		...
		if($('#elementsCollectionStr').val() != "") {
			var gisDataUrl="${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofgoods/getArcgisLocateDataListOfResource.jhtml?resTypeCode=${resTypeCode!''}&ids="+res;
			...
			var gisDataUrl="${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofgoods/getArcgisLocateDataListOfResource.jhtml?resTypeCode=${resTypeCode!''}&ids="+res;
		  	window.parent.currentLayerLocateFunctionStr="getArcgisDataOfResource('${resTypeCode!''}','"+gisDataUrl+"')";
		  	window.parent.getArcgisDataOfResource('${resTypeCode!''}',gisDataUrl);
		}
