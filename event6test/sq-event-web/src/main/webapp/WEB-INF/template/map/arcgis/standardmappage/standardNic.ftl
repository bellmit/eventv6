    ...
    <div class="" style="display:block;">
        <div class="ListSearch">
        	<div class="condition">
            	<ul>
                	<li class="LC1">名称：</li>
                	<li class="LC2"><input id="nicName" name="name" type="text" class="inp1" /></li>
                </ul>
            	<ul>
                	<li class="LC1">类型：</li>
                	<li class="LC2">
                		<select name="level" id="orgLevel" class="sel1">
                			<option value="">---全部---</option>
							<#if orgLevelDict??>
								<#list orgLevelDict as l>
...
<script type="text/javascript">
	...
	function loadMessage(pageNo,pageSize,searchType){
		...
		var postData = 'page='+pageNo+'&rows='+pageSize+'&infoOrgId='+gridId+'&name='+nicName+'&level='+orgLevel;
		
		$.ajax({
			type: "POST",
			url: '${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofregion/listNicData.jhtml?t='+Math.random(),  
			data: postData,
			dataType:"json",
			success: function(data){
				...
				if(list && list.length>0) {
					for(var i=0;i<list.length;i++){
					  var val=list[i];
					  var nicAddress = val.addr_;
					  var nicName = val.name;
					  
					  if(!nicAddress) {
					  	nicAddress = "&nbsp;";//为了排版整齐
					  } else if(nicAddress.length>15){
					  	nicAddress = nicAddress.substring(0,15)+"...";
					  }
					  
					  if(!nicName) {
					  	nicName = "";
					  } else if(nicName.length>15){
					  	nicName = nicName.substring(0,15)+"...";
					  }
					  
					  tableBody+='<dl onclick="selected(\''+val.nicId+'\',\''+(val.name==null?'':val.name)+'\')">';
						  tableBody+='<dt>';
							  tableBody+='<span class="fr">'+
										  	(val.levelName==null?'':val.levelName)+
										 '</span>';
							  tableBody+='<b class="FontDarkBlue">'+ nicName +'</b>';
							  
							  tableBody+='<dd>';
							  
							  tableBody+='<span title="联系电话" class="fr"><img src="${uiDomain!''}/images/map/gisv0/special_config/images/dianhua.png" />'+ (val.centerContactTel==null?'':val.centerContactTel) +'</span>';
							  tableBody+='<span title="'+ (val.addr_==null?'':val.addr_) +'">'+ nicAddress +'</span>';
	...
	function gisPosition(res){
		if (typeof parent.MMApi != "undefined") {	// 判断是否是高德地图
			...
			opt.ecs = $('#elementsCollectionStr').val();
			opt.gridId = $('#gridId').val();
			opt.url = "${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofregion/getArcgisLocateDataListOfNic.jhtml?ids="+res;
		...
		if($('#elementsCollectionStr').val() != "") {
			var url = "${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofregion/getArcgisLocateDataListOfNic.jhtml?ids="+res;
