...
<style>
.ztree li span{font-size:12px; font-family:Microsoft YaHei;}
</style>
</head>
    ...
    <div class="" style="display:block;">
        ...
        <div class="ListSearch">
        	<div class="condition">
				...
				<ul>
                	<li >名称：</li>
                	<li ><input id="name" name="name" type="text" class="inp1" /></li>
                </ul>
                <ul>
	                <li >性质：</li>
	                <li >
	                    <input type="hidden" name="useNature" id="useNature"  />
						<input class="inp1" type="text" name="useNatureName" id="useNatureName"  style="width:185px;"/>
	                </li>
				</ul>
				<ul>
                	<li >地址：</li>
                	<li ><input id="addressKey" name="addressKey" type="text" class="inp1" /></li>
                </ul>
            	<ul>
                	<li class="LC1">&nbsp;</li>
...
AnoleApi.initListComboBox("useNatureName", "useNature", "${AREA_BUILDING_INFO_USE_NATURE}", null, null, {
	  ShowOptions : {
		EnableToolbar : true
	  }
});
...
function CloseSearchBtn(){
	...
	function loadMessage(pageNo,pageSize,searchType){
		...
		var postData = 'page='+pageNo+'&rows='+pageSize+'&gridId='+gridId+'&buildingName='+name+'&addressKey='+addressKey+'&useNature='+useNature;
		$.ajax({
			type: "POST",
			url: '${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofbuilding/listData.json?t='+Math.random(), 
			data: postData,
			dataType:"json",
			success: function(data){
				...
				if(list && list.length>0) {
					for(var i=0;i<list.length;i++){
					  ...
					  var standardAddress = val.standardAddress;
					  if(standardAddress!=null && standardAddress!=""){
					  }else{
						standardAddress = "";
					  }
					  
					  tableBody+='<dl onclick="selected(\''+val.buildingId+'\')">';
					  tableBody+='<dt>';
					  tableBody+='<span class="fr">'+
					   (val.adminPhone==null?'':('<img src="${uiDomain!''}/images/map/gisv0/special_config/images/people_06.png">'+val.adminPhone))
					  +'</span>';
					  tableBody+='<b class="FontDarkBlue" title=\''+(val.buildingName==null?'':val.buildingName)+'\'>'+(val.buildingName==null?'':val.buildingName)+'</b>';
					  tableBody+='</dt>';
					  if(val.standardAddress != null && val.standardAddress != ''){
					  	tableBody+='<dd title=\''+(val.standardAddress==null?'':val.standardAddress)+'\'>'+(standardAddress)+'</dd>';
					  }else{
					  	tableBody+='<dd title=\''+(val.buildingAddress==null?'':val.buildingAddress)+'\'>'+(buildingAddress)+'</dd>';
					  }
	...
	function selected(id){
		...
		setTimeout(function() {
			if($('#elementsCollectionStr').val() != "") {
				window.parent.getDetailOnMapOfListClick($('#elementsCollectionStr').val(),370,281,id)
			}else {
	...
	function gisPosition(res){
		if (typeof parent.MMApi != "undefined") {	// 判断是否是高德地图
			...
			opt.ecs = $('#elementsCollectionStr').val();
			opt.gridId = $('#gridId').val();
			opt.url = "${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofbuilding/getArcgisLocateDataListOfBuilding.jhtml?buildingIds="+res+"&showType=2";
			return parent.MMApi.markerIcons(opt, "building");
		}
		...
		var url = "${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofbuilding/getArcgisLocateDataListOfBuilding.jhtml?buildingIds="+res+"&showType=2";
		window.parent.currentLayerLocateFunctionStr="getArcgisDataOfZhuanTi('"+url+"','"+$('#elementsCollectionStr').val()+"')";
		window.parent.getArcgisDataOfZhuanTi(url,$('#elementsCollectionStr').val(),370,281);
	}
</script>
</body>
</html>