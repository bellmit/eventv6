...
function CloseSearchBtn(){
	...
	function loadMessage(pageNo,pageSize,searchType){
		...
		var postData = 'page='+pageNo+'&rows='+pageSize+'&gridId='+gridId+'&name='+name+'&order='+order+'&elementsCollectionStr='+$('#elementsCollectionStr').val();
		$.ajax({
			type: "POST",
				...
				if(list && list.length>0) {
					for(var i=0;i<list.length;i++){
					  ...
					  if(val.gender==null){
					  	...
					  	imageGenderSpan = "<span width='20px' height='20px' style='padding:0 0 0 15px;background-position-y:0px;' title='"+((gender=='M' || gender == '男')?'男':'女')+"'  class='"+((gender=='M' || gender == '男')?'PeopleMale':'PeopleFemale')+"'>";
					  	gender = "";
					  	imageGenderSpan += gender;
					  	imageGenderSpan += "</span>";
					  }
					  tableBody+='<dl onclick="selected(\''+val.ciRsId+'\')">';
					  tableBody+='<dt>';
					  tableBody+='<span class="fr">'+gender+'&nbsp;'+(val.birthday==null ? '' : val.birthday)+'</span>';
					  tableBody+='<b class="FontDarkBlue">'+(val.name==null?'':val.name)+'&nbsp;'+imageGenderSpan+'</b>';
					  tableBody+='</dt>';
					  tableBody+='<dd>'
					  +(val.identityCard==null?'':'<span class="PeopleID" width="20px" height="20px" style="padding:0 0 0 15px;background-position-y:0px;"></span>'+val.identityCard)+'</dd>';
					  tableBody+='</dl>';
					  results=results+","+val.ciRsId;
					}
	...
	function selected(id){
		...
		setTimeout(function() {
			window.parent.getPopDetailOnMapOfListClick('${type}',id, gridId);
		},1000);
	}
	//--定位
