...
<body>
        	...
        	<div class="MetterContent">
                <div class="ConList">
                    ...
                    <div class="ListShow">
                        ...
                        <div class="tabs2">
                        	<div class="NorForm ThreeColumn" style="overflow-y:auto;height:331px;">
	                            <table width="100%" border="0" cellspacing="0" cellpadding="0">
									...
									<tr id="poortr">
										<td class="LeftTd"><label class="LabName"><span>贫困户标识：</span></label><div class="Check_Radio FontDarkBlue"><#if homeInfo.poorHoldCN??>${homeInfo.poorHoldCN}</#if></div></td>
										<td><label class="LabName"><span>致贫原因：</span></label><div class="Check_Radio FontDarkBlue"><#if homeInfo.poorReasonMCN??>${homeInfo.poorReasonMCN}</#if></div></td>
									</tr>
									<tr>
                    	...
                    	<!-----------------------------------帮扶信息-------------------------------------->
                        <div class="tabs2 hide" style="margin:0;height:328px;width:100%;overflow:auto">
							<table id="homeHelpInfoList" border="false"></table>
                        </div>
                    </div>
                </div>
            </div>
	...
	<script type="text/javascript">
		...
		//帮扶人员信息列表
		$('#homeHelpInfoLi').click(function(){
			$('#homeHelpInfoList').datagrid({
				nowrap: true,
				striped: true,
				fit: true,
				idField:'homeHelpId',
				...
				singleSelect:true,
				scrollbarSize:0,
				url:'${rc.getContextPath()}/zhsq/map/arcgis/arcgisDataOfPrecisionPoverty/homeHelpListData.json',
                pageSize: 20,
				columns:[[
					{field:'helpDate',title:'帮扶时间',align:'center',width:20},
					{field:'helpDutyman',title:'帮扶责任人',align:'center',width:20},
					{field:'helpDutymanPhone',title:'帮扶责任人联系方式',align:'center',width:20},
					{field:'helpMeasure',title:'帮扶措施及成效',align:'center',width:50}
				]],
				toolbar:'#jqueryToolbar',
				pagination:true,  
				queryParams:{familyId:'${homeInfo.familyId}'},
				onLoadSuccess:function(data){
				   $('#homeHelpInfoList').datagrid('clearSelections'); //清除掉列表选中记录 
				   if(data.total==0){ 
				      var body = $(this).data().datagrid.dc.body2; 
				      body.append('<div style="text-align: center;"><img src="${uiDomain!''}/images/map/gisv0/special_config/images/nodata.png" title="暂无数据"/></div>'); 
				   }
			...
			//设置分页控件
            var p = $('#homeHelpInfoList').datagrid('getPager');
			$(p).pagination({
				pageSize: 20,//每页显示的记录条数，默认为20
				pageList: [20,30,40,50],//可以设置每页记录条数的列表
				beforePageText: '第',//页数文本框前显示的汉字
