...
function CloseSearchBtn(){
	...
	function loadMessage(pageNo,pageSize,searchType){
		...
		$.ajax({
			type: "POST",
			url: '${rc.getContextPath()}/zhsq/map/arcgis/arcgisDataOfSituationController/careRoadMembersListData.json?t='+Math.random(), 
			data: postData,
			dataType:"json",
	...
	function selected(id,imsi){
		...
		setTimeout(function() {
			if($('#elementsCollectionStr').val() != "") {
				...
				window.parent.localtionCareRoadMemberPoint(id,imsi);
			}
		},1000);
	}
	//地图定位
	...
	if("1" != window.parent.IS_ACCUMULATION_LAYER) {
		...
		if($('#elementsCollectionStr').val() != "") {
			var url = "${rc.getContextPath()}/zhsq/map/arcgis/arcgisDataOfSituationController/getArcgisCareRoadMembersLocateDataList.jhtml?ids="+res+"&showType=2";
			...
			var gridAdminurl="${rc.getContextPath()}/zhsq/map/arcgis/arcgisDataOfSituationController/getArcgisCareRoadMembersLocateDataList.jhtml?ids="+res+"&showType=2";
			window.parent.currentLayerLocateFunctionStr="getArcgisDataOfCareRoadMember('"+gridAdminurl+"','')";
			window.parent.getArcgisDataOfCareRoadMember(gridAdminurl,'');
		}
	}
</script>
</body>
