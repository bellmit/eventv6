    ...
    <div class="" style="display:block;">
        ...
        <div class="ListSearch">
        	<div class="MetterCondition">
                ...
                <ul class="content" style="height:26px;">
                	<li class="LC1" style="float:left; line-height:26px; width:36px;">队伍：</li>
                	<li class="LC2" style="float:left; line-height:26px;"><input id="teamName" name="teamName" type="text" class="inp1" /></li>
                </ul>
...
function CloseSearchBtn(){
	...
	function loadMessage(pageNo,pageSize,searchType){
		...
		var	postData = 'page='+pageNo+'&rows='+pageSize+'&gridId='+gridId+'&name='+name+'&bizType='+bizType+'&teamName='+teamName;
		$.ajax({
			type: "POST",
			url: '${rc.getContextPath()}/zhsq/map/arcgis/arcgisDataOfSituationController/teamMembersListData.json?t='+Math.random(), 
			data: postData,
			dataType:"json",
			success: function(data){
				...
				if(list && list.length>0) {
					...
					for(var i=0;i<list.length;i++){
					  ...
					  tableBody+='<dl onclick="selected(\''+val.memberId+'\',\''+(val.imsi==null?'':val.imsi)+'\')">';
					  tableBody+='<dt>';
					  tableBody+='<span class="fr" style="width: 100px;">' + (val.tel==null?'':('<img src="${uiDomain!''}/images/map/gisv0/special_config/images/people_06.png">'+val.tel))+'</span>';
					  tableBody+='<b class="FontDarkBlue">'+(val.name==null?'':val.name)+'</b>';
					  //((dutys==""||dutys.length>3)?(val.dutyLabel==null?'':'<label>【' + val.dutyLabel + '】</label>'):"");
					  tableBody+='</dt>';
					  tableBody+='<dd>'+(val.teamName==null?'':val.teamName)+'</dd>';
					  tableBody+='</dl>';
	...
	//地图弹窗
	function selected(id, imsi){
		if(window.parent.qnviccub != undefined) {
	  		$("#qnviccub", window.parent.document).remove();
	 	}
	 	
	 	var url = '${rc.getContextPath()}/zhsq/map/arcgis/arcgisDataOfSituationController/teamMemberDetail.jhtml?memberId='+id+'&imsi='+imsi;
	...
	if("1" != window.parent.IS_ACCUMULATION_LAYER) {
		...
		var url = "${rc.getContextPath()}/zhsq/map/arcgis/arcgisDataOfSituationController/getArcgisTeamMembersLocateDataList.jhtml?ids="+res+"&showType=2";
