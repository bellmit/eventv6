...
function CloseSearchBtn(){
	...
	function loadMessage(pageNo,pageSize){
		...
		$.ajax({
			type: "POST",
			url:'${rc.getContextPath()}/zhsq/map/arcgis/arcgisDataOfEfficiencySupervisionController/workingEventListData.json?t='+Math.random(),
			data: postData,
			dataType:"json",
				...
				if(list && list.length>0) {
					for(var i=0;i<list.length;i++){
					  ...
					  if(val.urgencyDegree == null || val.urgencyDegree == '01'){
						image1="<img alt=\"\" src=\"${rc.getContextPath()}/images/event_01.gif\">";
					  }else{
						image1="<img alt=\"\" src=\"${rc.getContextPath()}/images/event_"+val.urgencyDegree+".gif\">";
					  }
					  if(val.source == '03'){
					  ...
					  if(val.handleDateStatus == null || val.handleDateStatus == '1'){
							handleDateStatus="<img width=\"15\" height=\"15\" title=\"处理时限正常\" src=\"${rc.getContextPath()}/images/time_normal.png\" />";
						}else if(val.handleDateStatus == null || val.handleDateStatus == '2'){
							handleDateStatus="<img width=\"15\" height=\"15\" title=\"处理时限将到期\" src=\"${rc.getContextPath()}/images/time_to_expire.png\" />";
						}else if(val.handleDateStatus == null || val.handleDateStatus == '3'){
							handleDateStatus="<img width=\"15\" height=\"15\" title=\"处理时限已过期\" src=\"${rc.getContextPath()}/images/time_expired.png\" />";
						}
