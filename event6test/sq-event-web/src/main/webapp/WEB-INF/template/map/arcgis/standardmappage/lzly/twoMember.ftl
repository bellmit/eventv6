...
<input type="hidden" id="bizType" value="<#if bizType??>${bizType}</#if>"/>
    ...
    <div class="ListSearch">
        <div class="MetterCondition">
            <ul class="content" style="height:26px;">
                ...
                <li class="LC2" style="float:left; line-height:26px;"><input id="name" name="name" type="text"
                                                                             class="inp1"/></li>
            </ul>
            <div class="SearchBtn"><input name="" type="button" value="查询" class="NorBtn"
                                          onclick="loadMessage(1,$('#pageSize').val(),'searchBtn');"/></div>
            <div class="clear"></div>
        </div>
...
//var temp1= $(".ListSearch").is(":visible");//是否可见
    ...
    function loadMessage(pageNo, pageSize, searchType) {
        ...
        postData = 'page=' + pageNo + '&rows=' + pageSize + '&regionCode=' + infoOrgCode + '&name=' + name + '&bizType=' + bizType;
        if ($('#elementsCollectionStr').val() != "") {
            postData += '&elementsCollectionStr=' + $('#elementsCollectionStr').val();
        }
        $.ajax({
            type: "POST",
            url: '${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofpoplocal/twoMemberListData.json',
            data: postData,
            dataType: "json",
            success: function (data) {
                ...
                if (list && list.length > 0) {
                    for (var i = 0; i < list.length; i++) {
                        var val = list[i];
                        tableBody += '<dl onclick="selected(\'' + val.tmId + '\')">';
                        tableBody += '<dt>';
                        tableBody += '<span class="fr">' + (val.memberPhone == null ? '' : '<img src="${uiDomain!''}/images/map/gisv0/special_config/images/people_06.png">' + (val.memberPhone)) + '</span>';
                        tableBody += '<b class="FontDarkBlue">' + (val.memberName == null ? '' : val.memberName) + '</b>';
                        tableBody += '</dt>';
                        tableBody += '<dd>' + (val.twoStation.stationName == null ? '' : val.twoStation.stationName) + '</dd>';
                        tableBody += '</dl>';
                        results = results + "," + val.tmId;
                    }
    ...
    function selected(id) {
        ...
        var url = '${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofpoplocal/twoMemberDetail.jhtml?tmId=' + id;
    ...
    function gisPosition(res) {
        if (typeof parent.MMApi != "undefined") {	// 判断是否是高德地图
            ...
            opt.ecs = $('#elementsCollectionStr').val();
            opt.gridId = $('#gridId').val();
            opt.url = "${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofpoplocal/getLocateDataListOfTwoMember.jhtml?ids=" + res;
        ...
        if ($('#elementsCollectionStr').val() != "") {
            var url = "${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofpoplocal/getLocateDataListOfTwoMember.jhtml?ids=" + res;
            ...
            var gridAdminurl = "${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofpoplocal/getLocateDataListOfTwoMember.jhtml?ids=" + res + "&showType=2";
