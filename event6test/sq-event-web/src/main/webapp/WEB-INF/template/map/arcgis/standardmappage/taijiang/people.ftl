			...
			<div class="con">
            	<div class="SelectKind">
				    ...
				    <div class="AdvanceSearch" style="display:none;">
                    	<ul>
                        	<li>
                        	    排序：<select id="order" editable="false" class="easyui-combobox" data-options="panelHeight:40,width:70" onChange="searchData()">
...
<script type="text/javascript">
	...
	function loadMessage(pageNo,pageSize){
		...
		var name = $("#name").val();
		if(name=="==输入姓名==") name="";
		//var order = $("#order").combobox("getValue");
		var order="time";
		...
		var postData = 'type=${type}&page='+pageNo+'&rows='+pageSize+'&gridId='+gridId+'&name='+name+'&order='+order;
		$.ajax({
			type: "POST",
					...
					for(var i=0;i<list.length;i++){
					  var val=list[i];
					    if((i+1)%2==0){//基数
					       ...
					       tableBody+=' <img src="${uiDomain!''}/images/map/gisv0/map_config/unselected/people_${imgtype}.png"/></li>';
					       tableBody+='<li class="ListInfo" onclick="selected(\''+val.ciRsId+'\')">';
					       tableBody+='<p>姓名：'+(val.name==null?'':val.name)+'</p>';
					       tableBody+='<p>身份证：'+(val.identityCard==null?'':val.identityCard)+'</p>';
					       tableBody+='</li></ul>';
					    }else{
	...
	function selected(id){
	   ...
	   //window.parent.getPopDetailOnMapOfListClick('${type}',id);
	}
	//--定位
