...
gisPosition();
	...
	function showChar() {
    	...
    	$.ajax({   
			 url: '${rc.getContextPath()}/zhsq/map/gisstat/gisStat/getPoorHouseholdsStatCount.json',   
			 type: 'POST',
			 timeout: 300000, 
			 data:{gridId:gridId,lowIncome:lowIncome},
			 error: function(){   
...
function gisPosition(){
	var gridId = $('#gridId').val();
	var lowIncome = $('#lowIncome').val();
	var postData = '?gridId='+gridId+'&lowIncome='+lowIncome;
	var url="${rc.getContextPath()}/zhsq/map/gisstat/gisStat/getPrecisionPovertyLayer.json"+postData;
