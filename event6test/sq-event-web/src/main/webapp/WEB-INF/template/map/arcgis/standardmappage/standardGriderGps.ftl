...
<body style="border:none;scolling:yes">
    ...
    <input type="hidden" id="beginTime" value="<#if beginTime??>${beginTime}</#if>" />
    <input type="hidden" id="endTime" value="<#if endTime??>${endTime}</#if>" />
...
function CloseSearchBtn(){
	...
	function loadMessage(pageNo,pageSize,searchType){
		...
		var postData = 'page='+pageNo+'&rows='+pageSize+'&gridId='+gridId+'&resName='+name+'&elementsCollectionStr='+elementsCollectionStr+'&beginTime='+beginTime+'&endTime='+endTime;
		$.ajax({
			type: "POST",
			url: '${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofgoods/griderGpsListData.json?t='+Math.random(),
			data: postData,
			dataType:"json",
			success: function(data){
				...
				if(list && list.length>0) {
					for(var i=0;i<list.length;i++){
					  ...
					  tableBody+='</dt>';
					  tableBody+='<dd>'+(val.checkTime==null?'':val.checkTime)+'</dd>';
					  tableBody+='</dl>';
