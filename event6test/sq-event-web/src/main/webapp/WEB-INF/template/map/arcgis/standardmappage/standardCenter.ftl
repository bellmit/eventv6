...
<title>安监队伍、群防群治队伍、机构队伍列表</title>
...
function CloseSearchBtn(){
	...
	function loadMessage(pageNo,pageSize,searchType){
		...
		var postData = 'page='+pageNo+'&rows='+pageSize+'&orgCode='+orgCode+'&name='+name+'&bizType='+bizType;
		$.ajax({
			type: "POST",
			url: '${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofgoods/controlsafetyRanksListData.json?t='+Math.random(),
			data: postData,
			dataType:"json",
				...
				if(list && list.length>0) {
					for(var i=0;i<list.length;i++){
					  var val=list[i];
						...
						var orgLevelStr = "";
						if(val.orgLevelStr != null){
                            orgLevelStr=val.orgLevelStr;
						}
						var orgAddr = val.orgAddr;
						if(orgAddr!=null && orgAddr!="" && orgAddr.length>11){
							orgAddr = orgAddr.substring(0,11)+"...";
						}
						var unitContactTel = val.unitContactTel;
					  tableBody+='<dl onclick="selected(\''+val.hashId+'\',\''+(val.name==null?'':val.name)+'\')">';
					  tableBody+='<dt title="'+val.name+'">';
					  tableBody+='<b class="FontDarkBlue">'+(name==null?'':name)+'</b>' + '<span class="fr">'+orgLevelStr+'</span>';
					  tableBody+='</dt>';
					  tableBody+='<dd title="'+(val.orgAddr==null?'暂无':val.orgAddr)+'">'+(val.orgAddr==null?'暂无':val.orgAddr)+'<span class="fr">'+(unitContactTel==null?'':unitContactTel) +'</span>'+'</dd>';
					  tableBody+='</dl>';
					  
					  results=results+","+val.hashId;
					}
	...
	function selected(hashId, name){
		if (typeof parent.MMApi != "undefined") {	// 判断是否是高德地图
			...
			return parent.MMApi.clickOverlayById(hashId, opt);
		}
		
		setTimeout(function() {
			if($('#elementsCollectionStr').val() != "") {
				window.parent.getDetailOnMapOfListClick($('#elementsCollectionStr').val(),300,150,hashId)
			}else {
	...
	function gisPosition(res){
		if (typeof parent.MMApi != "undefined") {	// 判断是否是高德地图
			...
			opt.ecs = $('#elementsCollectionStr').val();
			opt.gridId = $('#gridId').val();
			opt.url = "${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofgoods/getArcgisLocateDataListOfControlsafetyRanks.jhtml?ids="+res+"&showType=2";
