...
<body style="border:none">
    ...
    <div class="" style="display:block;">
        ...
        <div class="ListSearch">
        	<div class="condition">
            	<ul>
                	<li class="LC1">楼栋名称：</li>
                	<li class="LC2"><input id="buildingName" name="buildingName" type="text" class="inp1" /></li>
                </ul>
            	<ul>
                	<li class="LC1">楼宇性质：</li>
                	<li class="LC2">
                		<select name="buildingType" id="buildingType" class="sel1">
                ...
                <ul>
                	<li class="LC1">楼宇状态：</li>
                	<li class="LC2">
                		<select name="buildingStatus" id="buildingStatus" class="sel1">
                			<option value="">---全部---</option>
							<option value="1">已建</option>
...
function CloseSearchBtn(){
	...
	function loadMessage(pageNo,pageSize){
		...
		var buildingStatus = $("#buildingStatus").val();
		var pageSize = $("#pageSize").val();
		$.blockUI({message: "加载中..." , css: {width: '150px',height:'50px',lineHeight:'50px',top:'40%',left:'20%',
    	background:'url(${rc.getContextPath()}/css/loading.gif) no-repeat',textIndent:'20px'},overlayCSS:{backgroundColor:'#fff'}});
		var postData = 'page='+pageNo+'&rows='+pageSize+'&orgCode='+orgCode+'&buildingName='+buildingName+'&buildingType='+buildingType+'&buildingStatus='+buildingStatus;
		$.ajax({
			type: "POST",
			url:'${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataoflyjj/getArcgisLyjjBuildingDataList.jhtml?t='+Math.random(), 
			data: postData,
			dataType:"json",
			success: function(data){
				...
				if(list && list.length>0) {
					for(var i=0;i<list.length;i++){
					  ...
					  var buildingAddr = val.buildingAddr;
					  if(buildingAddr!=null && buildingAddr!="" && buildingAddr.length>12){
						buildingAddr = buildingAddr.substring(0,15)+"...";
					  }else{
					  ...
					  tableBody+='</dt>';
					  tableBody+='<dd title=\''+(val.buildingAddr==null?'':val.buildingAddr)+'\'>'+(val.buildingAddr==null?'&nbsp;':val.buildingAddr)+'</dd>';
					  tableBody+='</dl>';
	...
	/*
		...
		var url = sq_zzgrid_url+"/zzgl/grid/areaBuildingInfo/detail.jhtml?gridId="+$('#gridId').val()+"&buildingId="+id;
		window.parent.showDetailLigerUI(title,url,900,480);
		*/
		gisPosition(id);
	...
	function gisPosition(res){
		if (res==""){
	  	...
	  	var corurl="${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataoflyjj/getArcgisLocateDataListOfBuilding.jhtml?ids="+res+"&showType=2";
		window.parent.getArcgisDataOfBuilding(corurl,"dwl");
	}
</script>
</body>
</html>