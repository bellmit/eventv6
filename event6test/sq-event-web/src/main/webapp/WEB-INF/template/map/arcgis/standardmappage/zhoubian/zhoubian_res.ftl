			...
			success : function(data) {
				...
				if(list && list.length>0) {
					for(var i=0;i<list.length;i++){
					  ...
					  tableBody+='<dl onclick="selected(\''+val.RES_ID+'\',\''+(val.RES_NAME==null?'':val.RES_NAME)+'\')">';
					  tableBody+='<dt>';
					  tableBody+='<span class="fr"></span>';
					  tableBody+='<b class="FontDarkBlue">'+(val.RES_NAME==null?'':val.RES_NAME)+'</b>';
					  tableBody+='</dt>';
					  tableBody+='<dd>'+(val.GRID_NAME==null?'':val.GRID_NAME)+'</dd>';
					  tableBody+='</dl>';
					  
					  results=results+","+val.RES_ID;
					}
