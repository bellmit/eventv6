...
<body style="border:none">
	...
	function loadMessage(pageNo, pageSize) {
		...
		$.ajax({
			type : "POST",
			url : '${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofthing/listImportUnitData.json?t=' + Math.random(),
			data : {
				'page' : pageNo,
				'rows' : $("#pageSize").val(),
				'name' : $('#name').val(),
			...
			success : function(data) {
				...
				if (list && list.length > 0) {
					for ( var i = 0; i < list.length; i++) {
						...
						tableBody += '<dl onclick="selected('+val.importUnitId+')">';
						tableBody += '<dt>';
						tableBody += '<span class="fr">'+val.typeLabel+'</span>';
						...
						results = results + "," + val.importUnitId;
					}
