...
<body style="border:none">
			...
			success : function(data) {
				...
				if (list && list.length > 0) {
					for ( var i = 0; i < list.length; i++) {
						...
						tableBody += '<b class="FontDarkBlue" title=\'' + (val.NAME == null ? '' : val.NAME) + '\'>' + (name == null ? '' : name) + '</b>';
						tableBody += '</dt>';
						tableBody += '<dd title=\'' + (val.ADDR_ == null ? '' : val.ADDR_) + '\'>' + (address == null ? '&nbsp;' : address) + '</dd>';
						tableBody += '</dl>';
	...
	function gisPosition(res) {
		...
		if ($('#elementsCollectionStr').val() != "") {
			var url = "${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofgoods/getArcgisLocateDataListOfFireTeam.jhtml?ids="+res+"&showType=2";
