...
function CloseSearchBtn(){
	...
	function loadMessage(pageNo,pageSize,searchType){
		...
		$.ajax({
			type: "POST",
			url: '${rc.getContextPath()}/zhsq/alarm/videoSurveillanceController/globalEyesListData.json?t='+Math.random(),
			data: postData,
			dataType:"json",
			success: function(data){
				...
				if(list && list.length>0) {
					for(var i=0;i<list.length;i++){
					  ...
					  tableBody+='<dl onclick="selected(\''+val.deviceNum+'\')">';
					  tableBody+='<dt>';
					  tableBody+='<span class="fr">'+'</span>';
					  tableBody+='<b class="FontDarkBlue">'+(val.deviceName==null?'':val.deviceName)+'</b>';
					  tableBody+='</dt>';
					  tableBody+='<dd>'+(val.deviceNum==null?'':val.deviceNum)+'</dd>';
					  tableBody+='</dl>';
					  
					  results=results+",'"+val.deviceNum+"'";
					}
...
//		},1000);
        setTimeout(function() {
//            window.parent.locationFFCSGlobalEyesPoint(id);
            window.parent.showGlobalEye(id);
		},1000);
        <#--setTimeout(function() {-->
			<#--$.ajax({-->
				<#--type: "POST",-->
				<#--url: '${rc.getContextPath()}/zhsq/alarm/videoSurveillanceController/globalEyesShowUrl.jhtml?deviceNums='+id,-->
				<#--dataType:"json",-->
				<#--success: function(data){-->
					<#--window.parent.locationFFCSGlobalEyesPoint(id,data.rtspUrl);-->
				<#--},-->
				<#--error:function(data){-->
					<#--$.unblockUI();-->
					<#--$.messager.alert('提示','数据读取错误！！！','error');-->
				<#--}-->
			<#--});-->
        <#--},1000);-->
	}
	...
	function gisPosition(res){
		if (typeof parent.MMApi != "undefined") {	// 判断是否是高德地图
			...
			opt.h = 406;
			opt.ecs = $('#elementsCollectionStr').val();
			opt.url = "${rc.getContextPath()}/zhsq/alarm/videoSurveillanceController/getArcgisLocateDataListOfGlobalEyes.jhtml?ids="+res+"&showType=2";
		...
		<#--if($('#elementsCollectionStr').val() != "") {-->
			<#--var url = "${rc.getContextPath()}/zhsq/alarm/videoSurveillanceController/getArcgisLocateDataListOfGlobalEyes.jhtml?ids="+res+"&showType=2";-->
