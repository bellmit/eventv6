    ...
    <div class="SecMenu" style="display:block;">
			<div class="con">
            	<div class="SelectKind">
                	<p><input name="corName"  id="corName" type="text" class="inp1" value="==输入法人名称=="  onfocus="if (this.value=='==输入法人名称==') {this.value=''}" onblur="if(this.value==''){this.value='==输入法人名称=='}"/><input name="search" type="button" class="btn1" onclick="loadMessage(1,10);"/><input name="moreSearch" id="moreSearch" type="button" class="btn3" /></p>
				    <div class="AdvanceSearch" style="display:none;">
                    	<ul>
                        	<li>法人类型：<select name="corType" id="corType">
                        	...
                        	<li>行业分类：<select name="category"  id="category">
                        	        <option value="">---全部---</option>
                                	<#if categoryDC??>
...
<script type="text/javascript">
	...
	function loadMessage(pageNo,pageSize){
		...
		var postData = 'page='+pageNo+'&rows='+pageSize+'&gridId='+gridId+'&corName='+corName+'&corType='+corType+'&category='+category;
			...
			success: function(data){
				...
				if(list && list.length>0) {
					for(var i=0;i<list.length;i++){
					  var val=list[i];
					    if((i+1)%2==0){//基数
					       tableBody+='<ul class="back"><li class="ListIcon"><img src="${uiDomain!''}/images/map/gisv0/map_config/unselected/region_company.png"/></li>';
					       tableBody+='<li class="ListInfo" onclick="selected(\''+val.cbiId+'\',\''+(val.corName==null?'':val.corName)+'\')">';
					       tableBody+='<p>法人名称：'+(val.corName==null?'':val.corName)+'</p>';
					       tableBody+='<p>法人代表：'+(val.representativeName==null?'':val.representativeName)+'</p>';
					       tableBody+='<p>电话：'+(val.telephone==null?'':val.telephone)+'</p>';
					       tableBody+='</li></ul>';
					    }else{
					       tableBody+='<ul ><li class="ListIcon"><img src="${uiDomain!''}/images/map/gisv0/map_config/unselected/region_company.png"/></li>';
					       tableBody+='<li class="ListInfo" onclick="selected(\''+val.cbiId+'\',\''+(val.corName==null?'':val.corName)+'\')">';
					       tableBody+='<p>法人名称：'+(val.corName==null?'':val.corName)+'</p>';
					       tableBody+='<p>法人代表：'+(val.representativeName==null?'':val.representativeName)+'</p>';
					       tableBody+='<p>电话：'+(val.telephone==null?'':val.telephone)+'</p>';
