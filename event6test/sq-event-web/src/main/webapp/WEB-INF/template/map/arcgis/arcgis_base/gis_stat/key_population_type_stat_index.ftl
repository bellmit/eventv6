...
showChar();
	...
	function showChar() {
    	...
    	$.ajax({   
			 url: '${rc.getContextPath()}/zhsq/map/gisstat/gisStat/getKeyPopulationTypeStatCount.json',   
			 type: 'POST',
			 timeout: 300000, 
			 data:{gridId:gridId},
			 error: function(){   
			 ...
			 success: function(result){
				...
				if(result.length>0) {
					...
					tabStr+='<table width="100%" border="0" cellspacing="0" cellpadding="0" class="rk-table">';
							tabStr+='<tr><td align="center" colspan="3">'+result[0].name+'</td></tr>';
							tabStr+='<tr class="rk-table-ys">';
							tabStr+='<td align="right" width="110px">统计项</td>';
							tabStr+='<td align="center">统计值（人）</td>';
							tabStr+='<td align="center">占比（%）</td>';
							tabStr+='</tr>';
					for(var i=0; i<result.length; i++) {
							tabStr+='<tr class="rk-table-ys">';
							tabStr+='<td align="right" width="110px">'+result[i].label+'</td>';
							tabStr+='<td align="center">'+result[i].count+'</td>';
							tabStr+='<td align="center">'+result[i].percent+'</td>';
							tabStr+='</tr>';
					} 
					tabStr+='</table>';
				}
