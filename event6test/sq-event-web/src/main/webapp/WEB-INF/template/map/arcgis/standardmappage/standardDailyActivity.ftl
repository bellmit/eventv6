...
<body style="border:none">
    	...
    	<div class="ListSearch">
        	<div class="MetterCondition">
            	<ul class="content" style="height:26px;">
                	<li class="LC1" style="float:left; line-height:26px; width:60px;">活动名称：</li>
                	<li class="LC2" style="float:left; line-height:26px;"><input id="activityName" name="activityName" type="text" class="inp1" /></li>
                </ul>
				<div class="SearchBtn"><input name="" type="button" value="查询" class="NorBtn" onclick="loadMessage(1,$('#pageSize').val());" /></div>
                <div class="clear"></div>
            </div>
	...
	function loadMessage(pageNo, pageSize) {
		...
		$('.type li').each(function(index) {
			if($(this).attr('class')=='current'){
				importUnitTypes = importUnitTypes + "," + $(this).attr('id');
			}
		});
		
		if (importUnitTypes!=null && importUnitTypes!="" && importUnitTypes.length > 0) {
			importUnitTypes = importUnitTypes.substring(1);
		}
		
		$.ajax({
			type : "POST",
			url : '${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofthing/listDailyActivityData.json?t=' + Math.random(),
			data : {
				'page' : pageNo,
				'rows' : $("#pageSize").val(),
				...
				'importUnitOrgId' : "${importUnitOrgId}",
			...
			success : function(data) {
				...
				if (list && list.length > 0) {
					for ( var i = 0; i < list.length; i++) {
						...
						var address = val.activityAddr;
						var addressTitle = "";
						if (address != null && address != "" && address.length >= 12) {
							addressTitle = "活动地址："+address;
							address = address.substring(0, 12) + "...";
						}
						var activityDate = val.activityDateStr;
						var activityDateStr = "";
						if (activityDate != null && activityDate != "") {
							activityDateStr = activityDate;
						}
						
						tableBody += '<dl onclick="selected('+val.activityId+')">';
						tableBody += '<dt>';
						tableBody += '<span class="fr">'+activityDateStr+'</span>';
						...
						results = results + "," + val.activityId;
					}
	...
	function gisPosition(res) {
		...
		if (res == "") {
			//window.parent.clearImportUnitLayer();
			return;
		}
		
		if($('#elementsCollectionStr').val() != "") {
			var url = "${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofthing/getArcgisDailyActivityList.jhtml?ids=" + res + "&showType=2";
			...
			var importUnitUrl = "${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofthing/getArcgisDailyActivityList.jhtml?ids=" + res + "&showType=2";
