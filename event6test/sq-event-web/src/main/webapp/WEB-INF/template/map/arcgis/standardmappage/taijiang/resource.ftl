	   ...
	   <div class="SecMenu" style="display:block;">
			<div class="con">
            	<div class="SelectKind">
                	<p><input name="name"  id="name" type="text" class="inp1"  value="==请输入名称=="  onfocus="if (this.value=='==请输入名称==') {this.value=''}" onblur="if(this.value==''){this.value='==请输入名称=='}"/><input name="search" type="button" class="btn1" onclick="loadMessage(1,10);"/></p>
				</div>
...
<script type="text/javascript">
	...
	function loadMessage(pageNo,pageSize){
       ...
       var postData = 'page='+pageNo+'&rows='+pageSize+'&gridId='+gridId+'&resName='+name+'&resTypeId='+resTypeId;
       
		$.ajax({
			type: "POST",
			url: '${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofgoods/resourceListData.json?t='+Math.random(),
			data: postData,
			dataType:"json",
					...
					for(var i=0;i<list.length;i++){
					  var val=list[i];
					    if((i+1)%2==0){//基数
					       ...
					       tableBody+='<img alt="" src="${uiDomain!''}/images/map/gisv0/map_config/unselected/things_fireHydrant.png"/></li>';
					       tableBody+='<li class="ListInfo" onclick="selected(\''+val.resId+'\',\''+(val.resName==null?'':val.resName)+'\')">';
					       tableBody+='<p>名称：'+(val.resName==null?'':val.resName)+'</p>';
					       tableBody+='<p>网格：'+(val.gridName==null?'':val.gridName)+'</p>';
					       tableBody+='<p>类型：<#if resType??>${resType.name!''}</#if></p>';
					       tableBody+='</li></ul>';
					    }else{
	...
	window.parent.clearMyLayer();
		if (res==""){
	  	...
	  	window.parent.currentLayerLocateFunctionStr="getArcgisDataOfKeyPlace('${resType.typeCode!''}','"+gisDataUrl+"')";
