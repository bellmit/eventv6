...
<body style="border:none;scolling:yes">
    ...
    <div class="" style="display:block;">
        ...
        <div class="ListSearch">
        	<div class="condition">
            	<ul>
                	<li class="LC1">事件名称：</li>
                	<li class="LC2">
                		<input id="disputeEventName"  type="text" class="inp1"/>
                	</li>
                </ul>
            	<ul>
                	<li class="LC1">&nbsp;</li>
        ...
        <div class="tushi2">
        <a href="#" onclick="statusSearch();"><img src="${uiDomain!''}/images/map/gisv0/special_config/images/icon_04.png" />所有</a>
        <a href="#" onclick="statusSearch(1);"><img src="${uiDomain!''}/images/map/gisv0/special_config/images/icon_08.png" />正常</a>
        <a href="#" onclick="statusSearch(3);"><img src="${uiDomain!''}/images/map/gisv0/special_config/images/icon_07.png" />超时</a>
        <a href="#" onclick="statusSearch(2);"><img src="${uiDomain!''}/images/map/gisv0/special_config/images/icon_09.png" />即将超时</a>
        </div>
...
function statusSearch(handleStatus){
	$("#handleDateFlag").val(handleStatus);
...
function CloseSearchBtn(){
	...
	function loadMessage(pageNo,pageSize,searchType){
		...
		var disputeEventName = $('#disputeEventName').val();
		$.blockUI({message: "加载中..." , css: {width: '150px',height:'50px',lineHeight:'50px',top:'40%',left:'20%',
    	background:'url(${rc.getContextPath()}/css/loading.gif) no-repeat',textIndent:'20px'},overlayCSS:{backgroundColor:'#fff'}});
		var postData = 'page='+pageNo+'&rows='+pageSize+'&gridId='+gridId+'&disputeEventName='+disputeEventName+'&handleDateFlag='+handleDateFlag;
		$.ajax({
			type: "POST",
			url:'${rc.getContextPath()}/zhsq/map/arcgis/arcgisDataOfSocietyController/disputeListData.json?t='+Math.random(),
			data: postData,
			dataType:"json",
			success: function(data){ 
				...
				if(list && list.length>0) {
					for(var i=0;i<list.length;i++){
					  ...
					  var happenTimeStr = val.happenTimeStr;
					  var disputeEventName = val.disputeEventName;
					  if(disputeEventName!=null && disputeEventName!="" && disputeEventName.length>6){
						  disputeEventName = disputeEventName.substring(0,6);
					  }
					  
					  var happenAddr = val.happenAddr;
					  if(happenAddr!=null && happenAddr!="" && happenAddr.length>17){
						  happenAddr = happenAddr.substring(0,17)+"...";
					  }
					  var involveType = val.involveType;
					  var imageGenderSpan="";
					  if(involveType=='TAIWANG'){
					  	imageGenderSpan += '<img title="涉台" src="${uiDomain!''}/images/map/gisv0/special_config/images/involvedTaiWan.png">';
					  }
					  tableBody+='<dl onClick="selected(\''+val.mediationId+'\')">';
					  tableBody+='<dt>';
					  //tableBody+='<span class="fr" title=\''+timeAndAddr+'\'>'+(val.crimeTime==null?'':val.crimeTime)+'&nbsp;'+crimeAddr+'</span>';
					  tableBody+='<b class="FontDarkBlue" >'+(disputeEventName==null?'':disputeEventName)+imageGenderSpan+'</b>' + '<span class="fr">'+happenTimeStr+'</span>';
					  tableBody+='</dt>';
					  tableBody+='<dd title=\''+(val.happenAddr)+'\'>'+ happenAddr+'</dd>';
					  tableBody+='</dl>';
					  results=results+","+val.mediationId;
					}
	...
	function gisPosition(res){
		if (typeof parent.MMApi != "undefined") {	// 判断是否是高德地图
			...
			opt.ecs = $('#elementsCollectionStr').val();
			opt.gridId = $('#gridId').val();
			opt.url = "${rc.getContextPath()}/zhsq/map/arcgis/arcgisDataOfSocietyController/getArcgisLocateDataListOfDispute.jhtml?ids="+res;
...
// 				return;
		...
		var url = "${rc.getContextPath()}/zhsq/map/arcgis/arcgisDataOfSocietyController/getArcgisLocateDataListOfDispute.jhtml?ids="+res;
		if($('#elementsCollectionStr').val() != "") {
			...
			window.parent.currentLayerLocateFunctionStr="getArcgisDataOfCases('"+url+"')";
