...
<style>
	...
	<script type="text/javascript">
		...
		function zhxxShow(){
			...
			$('#zhxxIframe').attr('src',"${SQ_BI_URL}/report/yq/ncbigScreen/index.jhtml");
		}
		function zhxxHide(){
		...
		//企业复工	qyfu
		function qyfuShow(){
			$('#qyfuDiv').css({"display":"block","height":"100%"});
			$('#qyfuDiv').addClass('event-sw-on');
			$('#qyfuIframe').attr('src',"${SQ_BI_URL}/report/multiplex/index.jhtml");
		}
		function qyfuHide(){
			$('#qyfuDiv').css({"display":"none","height":"0%"});
			$('#qyfuDiv').removeClass('event-sw-on');
			$('#qyfuIframe').attr('src',"");
		}
		//一人疫档	yryd
		function yrydShow(){
		...
		function sqgzShow(){
			...
			$('#sqgzIframe').attr('src',"${SQ_BI_URL}/report/nanChangBigScreen/index");
		}
		function sqgzHide(){
		...
		function wzbzShow(){
			...
			$('#wzbzIframe').attr('src',"${SQ_BI_URL}/report/medical/index");
		}
		function wzbzHide(){
...
<body>
	...
	<div class="container-fluid">
		<div class="wrap-container">
			...
			<!-- 企业复工start -->
			<div  id='qyfuDiv'   style="z-index:50;position: absolute;width:100%; height: 0%;" >
				<iframe id='qyfuIframe' src="" frameborder="0" width="100%" height="100%"></iframe>
			</div>
			<!-- 综合信息end -->
					...
					<div class="struc-wrap clearfix" data-count='0'>
						<div class="struc-cont " >
	                        <div class="swiper-container swiper1">
								...
								<#if (qyfu??)>	
									<div id="qyfu" class="swiper-slide struc-item" style="font-size:0.22rem">
										<a href="javascript:;"  >
								...
								<#if (lly??)>
									<div id="managerDiv" class="swiper-slide struc-item" style="font-size:0.22rem">
										<a href="${SQ_UAM_URL}/admin/nc_admin" target="_blank">
											<p style="color: white;">数据管理</p>
										</a>
	                                </div>
								</#if>
    ...
    function showPointData(data){
		...
		function initFn2 () {
			...
			$('.swiper1 .swiper-slide').on('click', function () {
                ...
                //if(navigator.userAgent.match("ua_ffcs_nc")){
				if(isAndroid){ 
                	if($("#aifxDiv").hasClass('nav-item-on')||$("#xffx").hasClass('nav-item-on')||$("#clsb").hasClass('nav-item-on')||$("#szfx").hasClass('nav-item-on')||$("#qsfx").hasClass('nav-item-on')||$("#zhxx").hasClass('nav-item-on')||$("#qyfu").hasClass('nav-item-on')||$("#yryd").hasClass('nav-item-on')||$("#lcry").hasClass('nav-item-on')||$("#sqgz").hasClass('nav-item-on')||$("#wzbz").hasClass('nav-item-on')){
                    	$("#gridNameClick").hide();
                    	$("#todoTotalClick").hide();
                    }else{
                    	$("#gridNameClick").show();
