...
<body style="border:none;scolling:yes">
	...
	<input type="hidden" id="partyMemberType" value="${partyMemberType}" />	
...
function CloseSearchBtn(){
	...
	function loadMessage(pageNo,pageSize,searchType){
		...
		var postData = 'page='+pageNo+'&rows='+pageSize+'&orgCode='+orgCode+'&partyMemberType='+partyMemberType+'&name='+name;
		$.ajax({
			type: "POST",
			url:'${rc.getContextPath()}/zhsq/map/arcgis/arcgisDataOfSpecialPartyController/specialPartyInfoListData.json?t='+Math.random(),
			data: postData,
			dataType:"json",
			success: function(data){
				...
				if(list && list.length>0) {
					for(var i=0;i<list.length;i++){
					  ...
					  //tableBody+='<dl onClick="selected(\''+val.ciRsId+'\',\''+(val.drId==null?0:val.drId)+'\')">';
					  tableBody+='<dl onclick="selected(\''+val.ciRsId+'\')">';
					  tableBody+='<dt>';
					  tableBody+='<span class="fr">'+(val.birthday==null ? '' : val.birthday)+'</span>';
					  tableBody+='<b class="FontDarkBlue">'+(val.name==null?'':val.name)+'&nbsp;'+imageGenderSpan+'</b>';
					  tableBody+='</dt>';
					  tableBody+='<dd>'+(val.identityCard==null?'':'<img src="${uiDomain!''}/images/map/gisv0/special_config/images/people_03.png">'+val.identityCard)+'</dd>';
					  tableBody+='</dl>';
					  results+=","+val.ciRsId;
					}
	...
	function selected(id) {
		...
		setTimeout(function() {
			if($('#elementsCollectionStr').val() != "") {
				window.parent.getDetailOnMapOfListClick($('#elementsCollectionStr').val(),400,190,id,gridId)
			}else {
				window.parent.getPopDetailOnMapOfListClick('partyMerber',id, gridId);
			}
		},1000);
	}
	...
	function gisPosition(res){
		...
		if($('#elementsCollectionStr').val() != "") {
			var url = "${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofpoplocal/getArcgisLocateDataListOfPeople.jhtml?userIds="+res+"&type=partyMerber&showType=2";
			window.parent.currentLayerLocateFunctionStr="getArcgisDataOfZhuanTi('"+url+"','"+$('#elementsCollectionStr').val()+"')";
			window.parent.getArcgisDataOfZhuanTi(url,$('#elementsCollectionStr').val(),400,190);
		}else {
			var url = "${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofpoplocal/getArcgisLocateDataListOfPeople.jhtml?userIds="+res+"&type=partyMerber&showType=2";
			window.parent.currentLayerLocateFunctionStr="getArcgisDataOfPeople('"+url+"','partyMerber')";
			window.parent.getArcgisDataOfPeople(url,'partyMerber');
		}
	}
</script>
</body>
