    ...
    <div class="" style="display:block;">
        ...
        <div class="ListSearch">
        	<div class="MetterCondition">
            	<ul>
                	<li>
                		<input name="keyWord" id="keyWord" type="text" class="inp1" onkeydown="if (event.keyCode==13) {}" onblur="if(this.value=='')value='输入事件名称、地点、校园名称进行查询';" onfocus="if(this.value=='输入事件名称、地点、校园名称进行查询')value='';" value="输入事件名称、地点、校园名称进行查询" style="width:240px;" />
                	</li>
                </ul>
                <ul class="time">
...
function CloseSearchBtn(){
	...
	function loadMessage(pageNo,pageSize,searchType){
		...
		if (keyWord == "输入事件名称、地点、校园名称进行查询") keyWord = "";
			...
			success: function(data){
				...
				if(list && list.length>0) {
					for(var i=0;i<list.length;i++){
					  ...
					  tableBody+='</dt>';
					  tableBody+='<dd style="overflow:hidden;height:20px;">学校：'+(val.PLA_NAME==null?'':val.PLA_NAME)+'</dd>';
