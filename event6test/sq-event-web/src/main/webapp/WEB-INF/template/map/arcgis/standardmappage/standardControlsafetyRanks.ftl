			...
			success: function(data){
				...
				if(list && list.length>0) {
					for(var i=0;i<list.length;i++){
					  ...
					  tableBody+='</dt>';
					  if(bizType!=0){
					  	tableBody+='<dd>负责人：'+(val.manager==null?'':val.manager)+'</dd>';
					  	tableBody+='<dd>联系电话：'+(val.managerTel==null?'':val.managerTel)+'</dd>';
					  }else{
					  	tableBody+='<dd>机构类型：'+(val.teamTypeStr==null?'':val.teamTypeStr)+'</dd>';
					  }
					  tableBody+='</dl>';
	...
	function selected(hashId, name){
		...
		setTimeout(function() {
			if($('#elementsCollectionStr').val() != "") {
				window.parent.getDetailOnMapOfListClick($('#elementsCollectionStr').val(),300,180,hashId)
			}else {
				window.parent.localtionControlsafetyRanksPoint('${markType!''}',hashId,'${bizType!''}');
			}
	...
	function gisPosition(res){
		...
		if (res==""){
	  	...
	  	if($('#elementsCollectionStr').val() != "") {
			...
			window.parent.getArcgisDataOfZhuanTi(url,$('#elementsCollectionStr').val(),300,180);
		}else {
