			...
			success : function(data) {
				...
				if(list && list.length>0) {
					for(var i=0;i<list.length;i++){
					  ...
					  tableBody+='<dl onclick="selected(\''+val.CI_RS_ID+'\')">';
					  tableBody+='<dt>';
					  //tableBody+='<span class="fr">'+(val.I_BIRTHDAY==null ? '' : val.I_BIRTHDAY)+'</span>';
					  tableBody+='<b class="FontDarkBlue">'+(val.I_NAME==null?'':val.I_NAME)+'&nbsp;'+imageGenderSpan+'</b>';
					  tableBody+='</dt>';
					  tableBody+='<dd>'
					  +(val.I_IDENTITY_CARD==null?'':'<img src="${uiDomain!''}/images/map/gisv0/special_config/images/people_03.png">'+val.I_IDENTITY_CARD)+'</dd>';
					  tableBody+='</dl>';
					  results=results+","+val.CI_RS_ID;
					}
