    ...
    <div class="" style="display:block;">
        ...
        <div class="ListSearch">
        	<div class="condition">
                ...
                <ul>
                	<li class="LC1">地址：</li>
                	<li class="LC2"><input id="roomAddress" name="roomAddress" type="text" class="inp1" /></li>
                </ul>
            	<ul>
                	<li class="LC1">网格：</li>
...
function CloseSearchBtn(){
	...
	function loadMessage(pageNo,pageSize,searchType){
		...
		//$('#gridId').val();
		var name = $('#name').val();
		var address = $('#roomAddress').val();
		var gridName = $('#gridName').val();
		if(name=="==输入查询内容==") name="";
		var pageSize = $("#pageSize").val();
		$.blockUI({message: "加载中..." , css: {width: '150px',height:'50px',lineHeight:'50px',top:'40%',left:'20%',
    	background:'url(${rc.getContextPath()}/css/loading.gif) no-repeat',textIndent:'20px'},overlayCSS:{backgroundColor:'#fff'}});
		var postData = 'page='+pageNo+'&rows='+pageSize+'&gridId='+gridId+'&campusName='+name+'&gridName='+gridName+'&address='+address;
		$.ajax({
			type: "POST",
			url: '${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofgoods/campusListData.json?t='+Math.random(),
			data: postData,
			dataType:"json",
			success: function(data){
				...
				if(list && list.length>0) {
					for(var i=0;i<list.length;i++){
					  ...
					  tableBody+='</dt>';
					  tableBody+='<dd>地址：'+(val.roomAddress==null?'':val.roomAddress)+'</dd>';
					  tableBody+='</dl>';
	...
	function gisPosition(res){
		if (typeof parent.MMApi != "undefined") {	// 判断是否是高德地图
			...
			opt.ecs = $('#elementsCollectionStr').val();
			opt.gridId = $('#gridId').val();
			opt.url = "${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofgoods/getArcgisLocateDataListOfCampus.jhtml?resTypeCode=${markType!''}&ids="+res+"&showType=2";
		...
		if($('#elementsCollectionStr').val() != "") {
			var url = "${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofgoods/getArcgisLocateDataListOfCampus.jhtml?resTypeCode=${markType!''}&ids="+res+"&showType=2";
			window.parent.currentLayerLocateFunctionStr="getArcgisDataOfZhuanTi('"+url+"','"+$('#elementsCollectionStr').val()+"')";
			window.parent.getArcgisDataOfZhuanTi(url,$('#elementsCollectionStr').val(),350,280);
		}else {
			var gisDataUrl="${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofgoods/getArcgisLocateDataListOfCampus.jhtml?resTypeCode=${markType!''}&ids="+res;
		  	window.parent.currentLayerLocateFunctionStr="getArcgisDataOfCampus('${markType!''}','"+gisDataUrl+"')";
		  	window.parent.getArcgisDataOfCampus('${markType!''}',gisDataUrl);
		}
	}
</script>
</body>
