...
function CloseSearchBtn(){
	...
	function loadMessage(pageNo,pageSize,searchType){
		...
		var postData = 'page='+pageNo+'&rows='+pageSize+'&orgCode='+orgCode+'&resName='+name+'&resTypeId='+resTypeId+'&gridName='+gridName+'&operateStatus='+operateStatus+'&address='+address;
		$.ajax({
			type: "POST",
			url: '${rc.getContextPath()}/zhsq/map/arcgis/arcgisdataofgoods/newResourceListData.json?t='+Math.random(),
			data: postData,
			dataType:"json",
			success: function(data){
				...
				if(list && list.length>0) {
					for(var i=0;i<list.length;i++){
					  ...
					  tableBody+='<dt>';
					  if(val.operateStatus!='1'){//判断是否异常
					  	tableBody+='<img src="${rc.getContextPath()}/ui/images/yichang.png"/>';
					  }
					  tableBody+='<span class="fr">';
					  if(val.fireType =='002'){//判断是室内还是室外
					  	tableBody+='室外';
					  }else if(val.fireType =='001'){
					  	tableBody+='室内';
					  }else if(val.fireType =='003'){
					  	tableBody+='旋转';
					  }else if(val.fireType =='004'){
					  	tableBody+='地上';
					  }
					  tableBody+='</span>';
					  
					  if (val.catalog == 5) {
					  	tableBody+='<span class="fr">'+ (val.capacity==null?'0':val.capacity) + '吨</span>';
					  }
					  ...
					  tableBody+='<b title=\''+(val.name)+'\' class="FontDarkBlue">'+(name==null?'':name)+'</b>';
					  tableBody+='</dt>';
					  tableBody+='<dd title=\''+(val.address==null?'暂无':val.address)+'\'>地址：';
					  if(val.address!=null){//判断是否异常
					  	tableBody+=(address==null?'':address)+'</dd>';
					  }else if(val.buildingName!=null){
					  	tableBody+= (val.buildingName==null?'':val.buildingName)+'第'+(val.floor==null?'':val.floor)+'层</dd>';
					  }
					  //tableBody+='<dd>管理单位：'+(val.gridName==null?'':val.gridName)+'</dd>';
					  tableBody+='</dl>';
