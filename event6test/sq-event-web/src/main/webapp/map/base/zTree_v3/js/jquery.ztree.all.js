        ...
        view = {
            ...
            makeDOMNodeIcon: function (html, setting, node) {
                ...
                html.push("<span id='", node.tId, consts.id.ICON,
                    ...
                    "' zh-xc='", view.makeNodeIcoStyle(setting, node), "'></span><span id='", node.tId, consts.id.SPAN,
            ...
            makeDOMNodeNameBefore: function (html, setting, node) {
                ...
                html.push("<a id='", node.tId, consts.id.A, "' class='", consts.className.LEVEL, node.level, "' treeNode", consts.id.A, " onclick=\"", (node.click || ''),
                    "\" ", ((url != null && url.length > 0) ? "href='" + url + "'" : ""), " target='", view.makeNodeTarget(node), "' zh-xc='", fontStyle.join(''),
                    "'");
            ...
            makeUlHtml: function (setting, node, html, content) {
                html.push("<ul id='", node.tId, consts.id.UL, "' class='", consts.className.LEVEL, node.level, " ", view.makeUlLineClass(setting, node), "' zh-xc='display:", (node.open ? "block" : "none"), "'>");
                html.push(content);
                html.push("</ul>");
            },
...
(function($){
	...
	_beforeA = function(setting, node, html) {
		if (setting.check.enable) {
			...
			html.push("<span ID='", node.tId, consts.id.CHECK, "' class='", view.makeChkClass(setting, node), "' treeNode", consts.id.CHECK, (node.nocheck === true?" zh-xc='display:none;'":""),"></span>");
		}
	},
...
(function($){
		...
		showIfameMask: function(setting, showSign) {
			...
			if (showSign) {
				...
				for (var i = 0, l = iframeList.length; i < l; i++) {
					...
					r = tools.getAbs(obj),
					dragMask = $$("<div id='zTreeMask_" + i + "' class='zTreeMask' zh-xc='top:" + r[1] + "px; left:" + r[0] + "px; width:" + obj.offsetWidth + "px; height:" + obj.offsetHeight + "px;'></div>", setting);
	...
	_view = {
		addEditBtn: function(setting, node) {
			...
			var aObj = $$(node, consts.id.A, setting),
			editStr = "<span class='" + consts.className.BUTTON + " edit' id='" + node.tId + consts.id.EDIT + "' title='"+tools.apply(setting.edit.renameTitle, [setting.treeId, node], setting.edit.renameTitle)+"' treeNode"+consts.id.EDIT+" zh-xc='display:none;'></span>";
			aObj.append(editStr);
		...
		addRemoveBtn: function(setting, node) {
			...
			var aObj = $$(node, consts.id.A, setting),
			removeStr = "<span class='" + consts.className.BUTTON + " remove' id='" + node.tId + consts.id.REMOVE + "' title='"+tools.apply(setting.edit.removeTitle, [setting.treeId, node], setting.edit.removeTitle)+"' treeNode"+consts.id.REMOVE+" zh-xc='display:none;'></span>";
