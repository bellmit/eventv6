...
TreeGrid = function(_config){
	...
	var expandLayer = _config.expandLayer != undefined?_config.expandLayer:1;
	...
	//递归显示数据行
	drowData = function(id){
		var rows = _config.data;
		var cols = _config.columns;
		drowRowData(rows, cols, 1, id);
	}
	...
	drowRowData = function(_rows, _cols, _level, _pid){
		...
		for(var i=0;i<_rows.length;i++){
			...
			s += "<tr id='TR" + id + "' pid='" + ((_pid=="")?"":("TR"+_pid)) +"' isOpen='"+open+"' data=\"" + 
				TreeGrid.json2str(row) + "\" rowIndex='" + (rownum++) + "' dataId='"+row.id+"'  style='display:" + dispaly + "'>";
			row.htmlHidden&&(s+=row.htmlHidden);
	...
	this.show = function(){
		...
		s += "<table id='" + this.id + "' cellspacing=0 cellpadding=0 width='" + (_config.width || "100%") + "' class='c-tab1'>";
		drowHeader();
		drowData(this.id);
		s += "</table>";
	...
	init = function(){
		...
		__root.find("img[folder='Y']").bind("click", function(){
			...
			showHiddenNode(__root,trid, isOpen);
		});
	}

	//显示或隐藏子节点数据
	showHiddenNode = function(__root,_trid, _open){
		if(_open == "N"){ //隐藏子节点
			...
			showSubs(__root,_trid);
		}
	}

	//递归检查下一级节点是否需要显示
	showSubs = function(__root,_trid){
