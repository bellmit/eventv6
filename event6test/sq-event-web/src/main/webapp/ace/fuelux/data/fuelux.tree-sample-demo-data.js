...
DataSourceTree.prototype.data = function(options, callback) {
	...
	if(options && !("name" in options) && !("type" in options)){
		$data = this._data;//the root tree
		callback({ data: $data });
		return;
	}
	else if(options && "type" in options && options.type == "folder" && options.additionalParameters != null) {
	...
	if($data != null)//this setTimeout is only for mimicking some random delay
		setTimeout(function(){callback({ data: $data });} , parseInt(Math.random() * 400) + 100);
	else{
		callback({ data: {}});
	}
