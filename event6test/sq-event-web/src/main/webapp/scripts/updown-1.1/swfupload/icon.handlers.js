 ...
 */
var default_config = {
	upload_url:'/upFileServlet?method=up&moudle=head',
...
function uploadSuccess(file, serverData) {
	...
	try {
		...
		file.filePath = $.parseJSON(serverData).filePath;
		//var row = document.getElementById(this.customSettings.tr_id);

		//$(row.cells[1]).html(['<a href="',default_config.download_url,'&attachmentId=',file.attachmentId,'">',displayFileName(file.name),'</a>'].join(""));
		$("#iconAddress").val(file.filePath);
		$("#avater").html(["<img src='",file.filePath,"' width='120', height='168'/>"].join(""));
