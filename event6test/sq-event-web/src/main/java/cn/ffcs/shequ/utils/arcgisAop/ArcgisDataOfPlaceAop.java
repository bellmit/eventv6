 ...
 */
public class ArcgisDataOfPlaceAop extends ZZBaseController {
    @Autowired
        ...
        try {
            ...
            if (object != null) {
                ...
                for (PlaceInfo placeInfo : placeInfoList) {
                    String baseAddr = placeInfo.getBaseAddr();
                    if (idYD) {//盐都楼宇执行隐匿功能
                        if (StringUtils.isNotBlank(baseAddr)) {
                            baseAddr = baseAddr.replaceAll(gridPath, "");
                            placeInfo.setBaseAddr(baseAddr);
                        }
                    }
                }
