...
/**
 * 延平区工作流使用接口实现 对应流程图为 鼓楼事件流程图
 * @author zhangls
 *
 */
@Service(value = "eventDisposalWorkflow4YPQService")
public class EventDisposalWorkflow4YPQServiceImpl extends
	...
	//延平区工作流决策类
	private final String YPQ_DECISION_SERVICE = "workflowYPQDecisionMakingService";
	...
	public String startFlowByWorkFlow(Long eventId, Long workFlowId, String toClose, UserInfo userInfo, Map<String, Object> extraParam) throws Exception {
		...
		if(String.valueOf(ConstantValue.ORG_TYPE_UNIT).equals(orgType) && (StringUtils.isBlank(orgProfessionCode) || ConstantValue.GOV_PROFESSION_CODE.equals(orgProfessionCode))) {
			...
			userMap.put("decisionService", YPQ_DECISION_SERVICE);
	...
	public boolean subWorkFlowForEndingAndEvaluate(Long instanceId, String taskId, String nextNodeName, String nextStaffId, String curOrgIds, String advice, UserInfo userInfo, String smsContent, Map<String, Object> extraParam) throws Exception{
		...
		if(flag) {
			...
			if(ConstantValue.ACCEPT_TASK_CODE.equals(nextNodeName) || DISTRICT_DEPARTMENT_NODE_CODE.equals(curNodeName) || COMMUNITY_NODE_CODE_SEND.equals(curNodeName) || STREET_NODE_CODE_SEND.equals(curNodeName)) {
				Long userId = -1L;
				...
				}else if(DISTRICT_DEPARTMENT_NODE_CODE.equals(curNodeName) || COMMUNITY_NODE_CODE_SEND.equals(curNodeName) || STREET_NODE_CODE_SEND.equals(curNodeName)) {
					//添加 结案 工作圈记录
	...
	public Map<String, Object> initFlowInfo(String taskId, Long instanceId,
		...
		if(DISTRICT_COMMAND_CENTER_NODE_CODE.equals(curNodeName) || DISTRICT_DEPARTMENT_NODE_CODE.equals(curNodeName)) {
			...
			if(taskMapList != null) {//判断当前事件是否受理过
				...
				for(Map<String, Object> taskMap : taskMapList) {
					...
					if((DISTRICT_COMMAND_CENTER_NODE_CODE.equals(mainTaskCode) || DISTRICT_DEPARTMENT_NODE_CODE.equals(mainTaskCode)) && CommonFunctions.isNotBlank(taskMap, "TASK_ID")) {
						try {
