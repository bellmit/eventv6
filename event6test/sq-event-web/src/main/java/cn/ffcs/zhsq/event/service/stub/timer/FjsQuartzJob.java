 ...
 **/
public class FjsQuartzJob implements Job {
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        timerParams.put("exchangeType","003");//省厅
        ...
        params.put("type", "31");
        params.put("infoOrgCode", fjsOrgCode);
        ...
        logger.info("=========================环保省环保厅数据推送==========================");
        String gridadmin_flag = funConfigurationService.changeCodeToValue(ConstantValue.THREE_PART_JOINT_UNOIN, "gridadmin_flag", IFunConfigurationService.CFG_TYPE_FACT_VAL);
        if(StringUtils.isNotBlank(gridadmin_flag) && gridadmin_flag.equals("true")){
            logger.info("=========================环保省环保厅网格数据推送==========================");
            gridadminUrl = funConfigurationService.changeCodeToValue(ConstantValue.THREE_PART_JOINT_UNOIN, "fjs_gridadmin_url", IFunConfigurationService.CFG_TYPE_FACT_VAL);
            String adminGridId = funConfigurationService.changeCodeToValue(ConstantValue.THREE_PART_JOINT_UNOIN, "admin_grid_id", IFunConfigurationService.CFG_TYPE_FACT_VAL);
            if(StringUtils.isNotBlank(adminGridId) && StringUtils.isNotBlank(gridadminUrl)){
                Map<String, Object> maps = new HashMap<String, Object>();
                maps.put("startGridId",adminGridId);
                cn.ffcs.common.EUDGPagination gridAdminPagination = gridAdminService.findGridAdminPagination(1, 10000, maps);
                List<GridAdmin> listByGridId = (List<GridAdmin>) gridAdminPagination.getRows();
                for(GridAdmin gridAdmin : listByGridId){
                    syncGridAdmin(gridAdmin);
                }
            }
            logger.info("=========================环保省环保厅网格数据推送==========================");
        }
    ...
    private void syncGridAdmin(GridAdmin gridAdmin){
        JSONArray jsonArray = new JSONArray();
        JSONObject jsonObject = new JSONObject();
        jsonObject.put("sourceID",gridAdmin.getUserId());
        jsonObject.put("name",gridAdmin.getPartyName());
        jsonObject.put("sex",1);
        Long gridId = gridAdmin.getGridId();
        String infoOrgCode = gridAdmin.getInfoOrgCode();
        MixedGridInfo mixedGridInfo = null;
        if(StringUtils.isNotBlank(infoOrgCode)){
            mixedGridInfo = mixedGridInfoService.getDefaultGridByOrgCode(infoOrgCode);
        }
        if(gridId != null){
            mixedGridInfo = this.mixedGridInfoService.findMixedGridInfoById(gridId, false);
        }

        String regionCode = getRegionCode(mixedGridInfo.getInfoOrgCode());
        jsonObject.put("regionCode",regionCode);

        jsonObject.put("inspArea",mixedGridInfo.getGridName());
        jsonObject.put("inspFreq",4);//每月巡查频率
        jsonObject.put("training","");
        jsonArray.add(jsonObject);
        JSONObject params = new JSONObject();
        if(StringUtils.isNotBlank(gridadminUrl)){
            ...
            String s = HttpUtil.jsonPost(gridadminUrl, params.toString());
